class Session extends CObject {
	...
	async afterLogin(e) {
		...
		if (user) {
			try {
				...
				if (params) {
					let proc = async e => {
						let promise = app.promise_param ??= MQParam.topluYukleVerisiOlustur(e)
						let params = await promise
						for (let inst of values(params))
							await inst.topluYukle?.(e)
					}
	...
	async rolleriYukle(e) { this.rol = (await RolYapi.oku(e)) ?? new RolYapi({ adimsal: {}, iceriksel: {} }); return this }
