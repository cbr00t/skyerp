class Session extends CObject {
	...
	static DefaultLoginTipi = 'login'; static get extraArgs() { return ['userDesc', 'pass', 'useSessionIDFlag', 'isSSO', 'hasSSO', 'eMails', '_isAdmin', 'yetki', 'dbName', 'subeKod'] }
	...
	constructor(e) {
		e = e || {}; super(e); const loginTipi = e.loginTipi || e.sessionMatch || e.ozelTipKod;
	...
	async afterLogin(e) { const {user} = this; if (user) { try { this.encUser = await app.xenc(user); await this.rolleriYukle(e) } catch (ex) { console.error(ex) } } }
	...
	buildAjaxArgs(e) {
		...
		const {hasSession} = this; if (hasSession) {
			...
			if (!(useSessionIDFlag || isSSO) && (user || session)) {
				...
				else { args.loginTipi = loginTipi || Session.DefaultLoginTipi; args.user = user; if (pass) args.pass = pass }
			}
			else { const {sessionID} = this; if (sessionID) { args.sessionID = sessionID } }
		}
	}
	useSessionID() { this.useSessionIDFlag = true; return this } useUser() { this.useSessionIDFlag = false; return this } sso() { this.isSSO = true; return this }
}
