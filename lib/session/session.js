class Session extends CObject {
	...
	static get extraArgs() {
		return [
			'userDesc', 'pass', 'useSessionIDFlag', 'isSSO', 'hasSSO', 'eMails', '_isAdmin',
			'yetki', 'dbName', 'subeKod', 'remoteIP', 'testId',
			'modulBilgi'
		]
	}
	...
	constructor(e = {}) {
		...
		this.user = e.user; this.pass = e.pass;
		this.encUser = e.encUser; this.userDesc = e.userDesc
	...
	async dbKisitYukle(e) {
		...
		let { yil: kisitYil } = dbKisit
		let dbYil = asInteger(dbName?.substr(2, 2))
		{
	...
	async ekTanimYukle(e) {
		let { encUser } = this
		if (!encUser || MQCogul.isOfflineMode)
			return this

		let { DelimEnc: delim } = this.class
		let rec
		;{
			let sent = new MQSent(), {where: wh, sahalar} = sent
			sent.fromAdd('ORTAK..xuser k')
			wh.degerAta(encUser, 'k.xkod')
			sahalar.addWithAlias('k', 'xtanim tanim', 'xmodulbilgi modulBilgi')
			try { rec = await sent.execTekil() }
			catch (ex) { cerr(ex) }
		}
		if (rec) {
			let dec = await app.xdec(values(rec))
			...
			;{
				let { tanim: v } = this
				this.tanim = v ? v.split(delim) : null
			}
			;{
				let { modulBilgi: mb } = this
				this.modulBilgi = mb
					? fromEntries(
						mb.split(delim)
							.map(v => {
	...
	static async getSession(e = {}) {
		let { user, encUser, tanim, xtanim, basit } = e
		if (!user && encUser)
			user = await app.xdec(encUser)
		else if (user && !encUser)
			encUser = await app.xenc(user)

		if (!tanim && xtanim)
			tanim = await app.xdec(xtanim)
		
		if (empty(tanim)) {
			if (!encUser)
				return null
			let sent = new MQSent(), { where: wh, sahalar } = sent
			sent.fromAdd('ORTAK..xuser u')
			wh.degerAta(encUser, 'u.xkod')
			sahalar.add('RTRIM(u.xtanim) xtanim')
			xtanim = await sent.execTekilDeger()
			tanim = xtanim ? await app.xdec(xtanim) : null
		}
		if (empty(tanim))
			return null
		
		let { DelimEnc: delim } = this
		let [ yetki, pass, userDesc ] = tanim.split(delim)
		yetki = (
			yetki == 'A' ? 'admin' :
			yetki == 'S' ? 'sef' :
			''
		)
		let res = new this({ user, encUser, pass, userDesc, yetki })
		if (!basit) {
			await promiseAllSet([
				await res.rolleriYukle(e),
				await res.dbKisitYukle(e),
				await res.ekTanimYukle(e)
			])
		}
		return res
	}
	static getSessionBasit(e = {}) { return this.getSession({ ...e, basit: true }) }
	...
	buildAjaxArgs(e) {
		...
		if (hasSession) {
			...
			if (!(useSessionIDFlag || isSSO) && (user || session)) {
				if (config.dev && session) {
					session = JSON.parse(Base64.decode(session))
					loginTipi = session.loginTipi
					user = session.user
					pass = session.pass
					extend(this, { loginTipi, user, pass })
