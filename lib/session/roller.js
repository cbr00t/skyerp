class Rol extends CKodVeAdi {
	...
	static get araSeviyemi() { return true } static get tip() { return null }
	...
	constructor(e) { e = e || {}; super(e); $.extend(this, { userKod: e.userKod ?? e.user, kod: e.kod, ozelmi: asBool(e.ozelmi ?? e.ozel), belirtec2Detay: e.belirtec2Detay ?? {} }) }
	...
	static async oku(e) {
		...
		for (const rec of recs) {
			...
			const {tip} = inst.class, tipSelector = tip == 'A' ? 'adimsal' : tip == 'I' ? 'iceriksel' : tip;
		...
		const tip2Belirtec2DetListe = {}; for (const [tip, instListe] of Object.entries(tip2InstListe)) {
			...
			for (const inst of instListe) {
				const {belirtec2Detay} = inst; if ($.isEmptyObject(belirtec2Detay)) { continue }
				for (const [belirtec, det] of Object.entries(belirtec2Detay)) { (belirtec2DetListe[belirtec] = belirtec2DetListe[belirtec] || []).push(det) }
			}
		}
		const result = Object.values(kod2Inst); return { kod2Inst, tip2InstListe, tip2Belirtec2DetListe, result }
	}
	...
	static async detaylariYukle(e) {
		...
		for (const rec of recs) {
			...
			det.setValues({ rec }); const belirtec2Detay = inst.belirtec2Detay = inst.belirtec2Detay || {};
			const {belirtec} = det; belirtec2Detay[belirtec] = det
		}
		return true
	}
...
class Rol_Iceriksel extends Rol {
	static { window[this.name] = this; this._key2Class[this.name] = this } static get tip() { return 'I' } static get araSeviyemi() { return false }
...
class Rol_AdimsalDetay extends Rol_Detay {
	...
	get islemGirismi() { return !this.islem } get adim() { return this.belirtec }
...
class Rol_IcerikselDetay extends Rol_Detay {
	...
	constructor(e) { e = e || {}; super(e); $.extend(this, { belirtec: e.belirtec }) }
}
