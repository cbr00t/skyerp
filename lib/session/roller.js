class RolYapi extends CObject {
	...
	icerikselUygunmu(e, _value) {
		...
		let {iceriksel} = this.tip2InstListe; return empty(iceriksel) ? true : !!iceriksel.find(kisit => kisit.uygunmu(e))
	}
	icerikselClauseDuzenle(e) {
		let {iceriksel} = this.tip2InstListe; if (empty(iceriksel)) { return this }
...
class Rol extends CKodVeAdi {
	...
	static get sinifAdi() { return 'Rol' } static get table() { return 'puserrol' }
	static get tableAlias() { return 'urol' } static get kodSaha() { return 'rolkod' }
	...
	static async oku(e) {
		...
		let encKodListe = e.encKodListe = keys(encKodSet)
		await this.detaylariYukle({ ...e, tip2InstListe, kod2Inst, encKodSet })
		let tip2Belirtec2DetListe = {}
		for (let [tip, instListe] of entries(tip2InstListe)) {
			let belirtec2DetListe = tip2Belirtec2DetListe[tip] = tip2Belirtec2DetListe[tip] || {};
			for (let {belirtec2Detaylar} of instListe) {
				if (empty(belirtec2Detaylar)) { continue }
				for (let [belirtec, detaylar] of entries(belirtec2Detaylar)) {
	...
	static loadServerData(e = {}) {
		let queryYapi = this.loadServerData_queryOlustur(e)
		return app.sqlExecSelect(queryYapi)
	}
	static loadServerData_queryOlustur(e) {
		let {tableAndAlias} = this, uni = new MQUnionAll()
		let stm = new MQStm({ sent: uni })
		let _e = { ...e, stm, uni }
		this.loadServerData_queryDuzenle(_e); stm = _e.stm
		return { query: stm }
	}
	static loadServerData_queryDuzenle(e) {
		let {table} = this, {stm, stm: { orderBy }, uni, encUser} = e
		let sent = new MQSent({
			from: `${table} urol`, where: `urol.rolkod <> ''`,
			sahalar: ['urol.kullanicikod userKod', 'urol.tip', 'urol.rolkod kod', 'urol.ozelmi']
		}), {where: wh} = sent
		if (encUser)
			wh.degerAta(encUser, 'urol.kullanicikod')
		uni.add(sent)
		orderBy.add('userKod', 'tip', 'rolkod', 'ozelmi')
	}
	static async detaylariYukle(e = {}) {
		let recs = await this.loadServerData_detaylar(e)
		if (!recs)
			return false
		await this.decryptRecs({ recs })
		let {kod2Inst} = e
		for (let rec of recs) {
			let {_tip, kod} = rec, inst = kod2Inst[kod]
			if (inst == null)
				continue
			let cls = Rol_Detay.getClass(_tip), det = cls ? new cls() : null
			if (det == null)
				continue
			det.setValues({ rec })
			let belirtec2Detaylar = inst.belirtec2Detaylar ??= {}
			let {belirtec} = det
			if (det.bosDegilmi)
				(belirtec2Detaylar[belirtec] ??= []).push(det)
		}
		return true
	}
	static loadServerData_detaylar(e) {
		let queryYapi = this.loadServerData_detaylar_queryOlustur(e)
		return app.sqlExecSelect(queryYapi)
	}
	static loadServerData_detaylar_queryOlustur(e) {
		let {tableAndAlias} = this
		let uni = new MQUnionAll()
		let stm = new MQStm({ sent: uni })
		let _e = { ...e, stm, uni }
		this.loadServerData_detaylar_queryDuzenle(_e)
		stm = _e.stm
		return { query: stm }
	}
	...
	setValues({ rec }) {
		$.extend(this, {
			userKod: rec.userKod ?? rec.user,
			kod: rec.kod, ozelmi: asBool(rec.ozelmi)
		})
	}
	uygunmu({ belirtec } = {}) {
		let {belirtec2Detaylar} = this
		if (!belirtec2Detaylar)
			return true
		let detaylar = belirtec2Detaylar[belirtec]
		return detaylar ? !!detaylar.find(det => det.uygunmu(...arguments)) : true
   }
	static async decryptRecs(e = {}) {
		let {encKeySet} = this
		if (empty(encKeySet))
			return this
		let recs = makeArray(e.recs)
		if (!recs?.length)
			return this
		let valueSet = {}
		for (let rec of recs)
		...
		if (empty(valueSet))
			return this
		let enc2Dec
		try { enc2Dec = await app.xdec(keys(valueSet)) }
...
class Rol_Iceriksel extends Rol {
	...
	static get icerikselmi() { return true } static get sinifAdi() { return `${super.sinifAdi}: İçeriksel` }
	static get detaySinif() { return Rol_IcerikselDetay }
	clauseDuzenle(e) {
		...
		for (let det of detaylar) {
			...
			if (basiSonumu) { if (value.bosDegilmi) { bs = value } }
			else if (birKismimi) { values.push(value) }
		}
...
class Rol_Detay extends CObject {
	static { window[this.name] = this; this._key2Class[this.name] = this }
	static get detaymi() { return true } static get araSeviyemi() { return true }
	static get tableAlias() { return 'det' }
	static get tableAndAlias() {
		let {table, tableAlias} = this
		return tableAlias ? table : `${table} ${tableAlias}`
	}
	static get tip2Sinif() {
		let result = this._tip2Sinif; if (result == null) {
			result = {}; let {subClasses} = this
			for (let cls of subClasses) {
				let {araSeviyemi, tip} = cls
				if (!araSeviyemi && tip)
					result[tip] = cls
			}
			this._tip2Sinif = result
	...
	get bosmu() { return empty(this.value) } get bosDegilmi() { return !this.bosmu }
	constructor(e = {}) { super(e); $.extend(this, { kod: e.kod, seq: e.seq, belirtec: e.belirtec ?? e.adim }) }
	static getClass(e) { let tip = typeof e == 'object' ? e.tip : e; return this.tip2Sinif[tip] }
	static newFor(e) { if (typeof e != 'object') { e = { tip: e } } let cls = this.getClass(e); return cls ? new cls(e) : null }
...
class Rol_AdimsalDetay extends Rol_Detay {
	static { window[this.name] = this; this._key2Class[this.name] = this }
	static get tip() { return 'A' } static get araSeviyemi() { return false }
	...
	constructor(e = {}) { super(e); $.extend(this, { adim: e.adim, islem: e.islem, index: e.index ?? e.indis }) }
