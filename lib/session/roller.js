...
class Rol extends CKodVeAdi {
	...
	static async oku(e) {
		...
		let encKodListe = Object.keys(encKodSet); await this.detaylariYukle({ ...e, tip2InstListe, kod2Inst, encKodSet });
	...
	static loadServerData_detaylar_queryDuzenle(e) {
		...
		let sentEkle = (detaySinif, duzenleyici) => {
			let {tip, table} = detaySinif;
			let sent = new MQSent({ from: `${table} det`, where: `det.kod <> ''`, sahalar: [`'${tip}' _tip`, 'det.kod'] }), {where: wh} = sent;
			if (encKodListe) { wh.inDizi(encKodListe, 'det.kod') }
			if (duzenleyici) { getFuncValue.call(this, duzenleyici, { ...e, sent, wh }) }
			uni.add(sent); return sent
		};
		sentEkle(Rol_AdimsalDetay, ({ sent, wh }) => {
		...
		sentEkle(Rol_IcerikselDetay_BasiSonu, ({ sent, wh }) => {  sent.sahalar.add('det.seq', 'det.belirtec', 'det.basi', 'det.sonu', 'NULL deger', 'NULL islem', 'NULL indis') });
		sentEkle(Rol_IcerikselDetay_BirKismi, ({ sent, wh }) => { sent.sahalar.add('10000 seq', 'det.belirtec', 'NULL basi', 'NULL sonu', 'det.bkod deger', 'NULL islem', 'NULL indis') });
...
class Rol_Iceriksel extends Rol {
	...
	clauseDuzenle(e) {
		e = e || {}; let {belirtec} = e, {belirtec2Detaylar} = this, detaylar = belirtec2Detaylar[belirtec]; if (!detaylar?.length) { return this }
		let values = [], bs; for (let det of detaylar) {
			if (det.bosmu) { continue } let {basiSonumu, birKismimi} = det.class, {value} = det;
			if (basiSonumu) { if (value.bosDegilmi) { bs = value } } else if (birKismimi) { values.push(value) }
		}
		if (!(bs || values.length)) { return this } let {saha} = e, wh = e.where ?? e.sent?.where;
