class LayoutBase extends CObject {
	...
	run(e) {
		if (this.asyncRunFlag) {
			return new $.Deferred(async p => {
				try { await this.init(); const result = await this.runDevam(); await this.afterRun();
					p.resolve(result === undefined ? this : result)
				}
				catch (ex) { p.reject(ex) }
			})
		}
		this.init(e); const result = this.runDevam(e); this.afterRun(e);
		return result === undefined ? this : result
	}
	init(e) {
		let {content, layout, _css} = this; const hasContent = content?.length;
		...
		if (layout?.length && this.class.slowAnimationFlag) { layout.addClass('slow-animation') }
	}
	runDevam(e) { }
	afterRun(e) {
	...
	show(e) { const {layout} = this; layout.css('opacity', 1); layout.removeClass('jqx-hidden basic-hidden'); return this }
	...
	getLayout(e) {
		...
		if (selector) {
			...
			const {content} = this; if (content?.length) {
	...
	onResize(e) {
		...
		const {_lastEventTimestamp_resize} = this, _now = this._lastEventTimestamp_resize = now();
		if (_lastEventTimestamp_resize && _now - _lastEventTimestamp_resize < 100) return; this._inEventFlag = true;
		try {
			const {layout} = this; if (!this.isDestroyed && layout?.length) {
				const elms_dockBottom = layout.find('.dock-bottom');
				if (elms_dockBottom.length) {
					for (let i = 0; i < elms_dockBottom.length; i++) {
						const elm = elms_dockBottom.eq(i);
						/* let elmPos = elm.offsetParent(); */ let elmPos = elm;
						if (!elmPos?.length) elmPos = elm
						if (elmPos?.length && !elmPos.height()) elmPos = elm.parent()
						const y = elmPos.position().top; elm.height(`calc(var(--full) - (${y}px + 0px))`)
					}
				}
