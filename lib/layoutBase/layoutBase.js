class LayoutBase extends CObject {
	...
	constructor(e = {}) {
		...
		this.layoutSelector = e.layoutSelector
		this.cssSelector = e.cssSelector
	}
	run(e) {
	...
	getLayout(e = {}) {
		let layout = this.getLayoutInternal(e)
		if (layout)
			return layout
		
		let selector = e.selector || this.layoutSelector || this.defaultLayoutSelector
		delete e.selector
		...
		if (selector) {
			...
			if (content?.length) {
				...
				if (layout?.length) {
					if (layout[0].tagName.toUpperCase() == 'TEMPLATE')
						layout = layout.eq(0).contents(':not(text)').clone(true)
				}
				if (layout?.length)
					return layout
			}
	...
	getCSS(e = {}) {
		let css = this.getCSSInternal(e)
		if (css)
			return css
		
		let selector = e.selector || this.cssSelector || this.defaultCSSSelector
		delete e.selector
		...
		if (selector) {
			...
			if (content?.length) {
				...
				if (css?.length) {
					if (css[0].tagName.toUpperCase() == 'TEMPLATE')
						css = css.eq(0).contents(':text').clone(true)
				}
