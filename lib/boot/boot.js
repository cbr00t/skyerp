class Boot {
	...
	run() {
		const {layoutName} = this.class, template = document.querySelector(`template#${layoutName}`);
		const layout = this.layout = template.content.querySelector('div'); this.progress = layout.querySelector('progress');
		layout.classList.add(layoutName, 'part'); document.body.appendChild(layout)
	}
