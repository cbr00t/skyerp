class App extends AppBase {
	...
	async init(e) {
		let {offlineMode: offline} = this;
		for (let key of ['queryCache', 'reqCache']) {
			let cache = this[key]; if (!cache) { continue }
			if (offline) {
				try { await cache.yukle(e) }
				catch (ex) { console.error(key, 'yukle()', ex) }
			}
			else {
				if (qs[key] || qs[key.replace('Cache', 'Record')])
					cache.record()
			}
		}
		await super.init(e);
	...
	async runDevam(e) {
		...
		if (!this._ilkIslemlerYapildimi) {
			...
			if (dbMgr) {
				await this._promise_dbMgr_tablolariOlustur_fetchAndSaveQueries
				try { await dbMgr.yukle() }
				finally { this.dbMgr_createDefaultDBs(e) }
			}
	...
	dbMgr_tablolariOlustur(e = {}) {
		...
		for (let db of dbMgr.iterValues()) {
			...
			if (!queries?.length)
				continue
			if (!internal) { try { db.execute('BEGIN TRANSACTION') } catch (ex) { } }
			for (let query of queries) {
				try { db.execute(query) }
				catch (ex) { console.error({ islem: 'dbMgr_tablolariOlustur', query, ex }) }
			}
			if (!internal) { try { db.execute('COMMIT') } catch (ex) { } }
			let _e = { ...e, db, name }
	...
	dbMgr_tablolariOlustur_fetchAndSaveQueries(e) {
		...
		let db2InitQueries = this.dbMgr_tablolariOlustur_queriesOlustur(e)
		if (empty(db2Urls)) { return }
		let promises = []
		for (let [dbName, url] of Object.entries(db2Urls)) {
			if (!url.includes('?')) { url += '?' }
			...
			promises.push(ajaxPost({ dataType: 'text', ajaxContentType: 'text/x-sql', url }).then(data => {
				(db2InitQueries[dbName] = db2InitQueries[dbName] ?? [])
					.push(...(data || []).split(';').filter(x => !!x))
			} ))
		}
	...
	async sysInfo(e) {
		let delim = '@', {class: { DefaultWSHostName_SkyServer: host }, session: { remoteIP: networkIP } = {}, ws} = config;
	...
	*getModulIter() { let {moduller} = this; if (moduller) { for (let modul of Object.values(moduller)) { yield modul } } }
	...
	sqlExecTekilDeger(e) {
		...
		if (promise) { promise = promise.then(rec => Object.values(rec || {})[0]) }
		return promise
	}
	async sqlExec(e) {
						...
						try {
							...
							for (let execTip in execTip2Id2SqlExecArgs) {
								try {
									...
									if ($.isEmptyObject(args) || $.isEmptyObject(args.queries))
										return
