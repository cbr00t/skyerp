class App extends AppBase {
	...
	async init(e) {
		let {queryCache, offlineMode: offline} = this;
		if (queryCache) {
			if (offline) {
				try { await queryCache.yukle(e) }
				catch (ex) { console.error('query-cache', 'yukle()', ex) }
			}
			else { if (qs.queryCache || qs.queryRecord) { queryCache.record() } }
		}
		await super.init(e);
	...
	*getModulIter() { let {moduller} = this; if (moduller) { for (let modul of Object.values(moduller)) { yield modul } } }
	...
	sqlExecTekil(e) {
		e = e || {}; if (!(e.query ?? e.queries)) { e = { query: e } } e.maxRow = 2
	...
	_sqlExec(e) {
		...
		let _execQuery = async _then => {
			try {
				...
				if (_then) { result = await _then(result) ?? result }
				return result
			}
			catch (ex) {
		...
		if (queryCache) {
			...
			if (query) {
				if (offline) {
					return (async () => {
						let result = await queryCache.get(queryYapi);
						if (result == null) {
							switch (execTip) {
				...
				if (queryCache.recording) {
					return _execQuery(result => queryCache.onQueryExec({ ...e, execTip, queryYapi, result }))
				}
			}
		}
		return _execQuery()
	...
	wsSqlExec(e) {
		...
		let {args} = e, {params} = args; delete args.params;
		...
		if (deferFlag) {
			let {execTip, queries} = args, uygunmu = !$.isEmptyObject(queries);
			if (uygunmu) {
				for (let query of queries) {
					if (!(query && typeof query == 'object' && $.isEmptyObject(query.params))) { uygunmu = false; break }
				}
			}
			if (uygunmu) {
