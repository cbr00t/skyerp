class App extends AppBase {
	...
	gecerliModulleriBelirle(e) {
		e = e ?? {}; const {tumModuller} = this.class, moduller = e.moduller ?? this.moduller;
		...
		if (menuId) {
			const {frMenu} = this; if (frMenu) {
				...
				if (menuItem) {
					...
					setTimeout(async () => {
						await app.promise_ready; const menuPart = frMenu.part;
	...
	async barkodBilgiBelirle(e) {
		...
		if (barkod.length > 2) {
			...
			if (kural) {
				const parser = await kural.parseSonucu(e);
	...
	*getModulIter() { const {moduller} = this; if (moduller) { for (const modul of Object.values(moduller)) { yield modul } } }
	...
	async sqlSiraliExec(e) {
		...
		const delim = 'UNION ALL'; let query = e.query ?? e.queries;
		if ($.isArray(query)) { query = query.flat().join(delim) }
		let queries = query?.split(delim); if (!queries.length) { return [] }
	...
	_sqlExec(e) {
		...
		const _e = $.extend({}, e),  keys = ['offlineMode', 'isOfflineMode', 'offline', 'wsArgs', 'query', 'queries', /*'params'*/]; for (const key of keys) { delete _e[key] }
	...
	wsVIOQueryYapi(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
	...
	wsWebRequest(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
			timeout: 300000, processData: false, dataType: e.type, ajaxContentType: wsContentTypeVeCharSet, data: toJSONStr(data),
	...
	wsMultiUpload(e) {
		e = e || {}; let data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
	...
	wsEMailGonder(e) {
		...
		const url = app.getWSUrl({ session: null, wsPath: 'ws/eMail', api: 'send', args });
		const timeout = 120 * 1000, processData = false, ajaxContentType = wsContentTypeVeCharSet;
		return ajaxPost({ timeout, processData, ajaxContentType, url, data })
	}
	wsEMailQueue_add(e) {
		let args = e || {}, {data} = args;
		if (typeof data == 'object') { data = toJSONStr(data) } delete args.data;
		const url = app.getWSUrl({ session: null, wsPath: 'ws/eMailQueue', api: 'add', args });
		const timeout = 120 * 1000, processData = false, ajaxContentType = wsContentTypeVeCharSet;
		return ajaxPost({ timeout, processData, ajaxContentType, url, data })
	}
