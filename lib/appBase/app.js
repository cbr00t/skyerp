class App extends AppBase {
	...
	constructor(e) {
		...
		$.extend(this, { mqGlobals: {}, params: {}, sabitTanimlar: {}, temps: {}, promise_login: new $.Deferred(), promise_ready: new $.Deferred(), trnId2Count: {} })
	}
	...
	async sqlTrnDo(e, _proc) {
		...
		const {defTrn} = e; let {trnId} = e, trnResult;
		if (trnId == null) { trnResult = await this[defTrn ? 'sqlDefTrnBegin' : 'sqlTrnBegin']({ trnId }), trnId = trnResult?.trnId }
		if (trnId == null) { throw { isError: true, rc: 'noTransaction', errorText: 'Transaction yok veya geçersiz' } }
		if (trnResult == null) { const {trnId2Count} = this; trnSayi = trnId2Count[trnId]; trnResult = { trnId, trnSayi } }
		let rollback = false;
		try {
			const proc = typeof e == 'object' ? e.proc ?? e.block : _proc;
			const _e = $.extend({}, e, trnResult); let result = trnResult.result = await getFuncValue.call(this, proc, _e);
			if (result === false) { rollback = true }
		}
		catch (ex) { rollback = true; throw ex }
		finally { let result = await this.sqlTrnEnd({ trnId, rollback }); $.extend(trnResult, { rollback }, result || {}) }
		return trnResult
	}
	...
	async sqlTrnBegin(e) {
		e = e || {}; const {trnId2Count} = this; let trnId = typeof e == 'object' ? e.trnId : e;
		let trnResult; if (trnId == null) { trnResult = await this.wsSqlTrnBegin({ trnId }); trnId = trnResult?.trnId } if (trnId == null) { return null }
		const trnSayi = trnId2Count[trnId] = (trnId2Count[trnId] || 0) + 1;
		const result = $.extend({}, { trnId, trnSayi }, trnResult ?? {}); return result
	}
	async sqlTrnEnd(e) {
		e = e || {}; const trnId = typeof e == 'object' ? e.trnId : e; if (trnId == null) { return null }
		const {trnId2Count} = this, {rollback} = e; let trnSayi = trnId2Count[trnId] || 0;
		if (trnSayi) { trnSayi = rollback ? 0 : Math.max(trnSayi - 1, 0); trnId2Count[trnId] = trnSayi }
		const result = { trnId, trnSayi }; if (!trnSayi) {
			let _result; try { _result =  await this.wsSqlTrnEnd({ trnId, rollback }) }
			catch (ex) { const err = ex?.responseJSON ?? ex; if ((err?.rc || err?.code) != 'InvalidOperationException') { throw ex } _result = err }
			delete trnId2Count[trnId]; $.extend(result, _result)
		}
		return result
	}
	...
	_sqlExec(e) {
		...
		if (wsArgs) { $.extend(_e, wsArgs) } if (offset != null) { _e.offset = offset } if (maxRow != null) { _e.maxRow = maxRow || limit }
