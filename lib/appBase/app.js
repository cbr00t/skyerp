class App extends AppBase {
    static { window[this.name] = this; this._key2Class[this.name] = this } static get dbMgrClass() { return null }
	...
	constructor(e) {
		...
		const {dbMgrClass} = this.class; if (dbMgrClass) { this.dbMgr = dbMgrClass.default ?? new dbMgrClass() }
	}
	...
	*getModulIter() { const {moduller} = this; if (moduller) { for (const modul of Object.values(moduller)) { yield modul } } }
	...
	wsSqlExec(e) {
		...
		this.sqlType = ''; if (args.query) { args.queries = [args.query]; delete _args.query }
		if (args.queries) {
			const {queries} = args;
