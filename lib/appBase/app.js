class App extends AppBase {
	...
	async runDevam(e) { await super.runDevam(e); if (this.isLoginRequired) { await this.loginIstendi(e) } else { this.promise_login?.resolve() } }
	async afterRun(e) {
		await super.afterRun(e); await this.promise_login;
		let promises = []; if (!this._ilkIslemlerYapildimi) {
			const {params, sabitTanimlar, moduller} = this;
			...
			if (promises?.length) { this.class.tumModuller; try { await Promise.all(promises) } catch (ex) { console.error(ex); debugger } promises = [] }
