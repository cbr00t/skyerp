class App extends AppBase {
	...
	*getModulIter() { const {moduller} = this; if (moduller) { for (const modul of Object.values(moduller)) { yield modul } } }
	...
	_sqlExec(e) {
		...
		return this.wsSqlExec(_e).catch(ex => { if (ex.statusText == 'parsererror') { return null } else { throw ex } })
	}
	wsSqlExec(e) {
	...
	wsDosyaListe(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
			timeout: 60000, processData: false, ajaxContentType: wsContentType, data: toJSONStr(data), url: app.getWSUrl({ /* wsPath: 'ws/yonetim', */ api: 'dosyaListe', args: e }) })
	}
	wsSqlServerListe(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
	...
	wsYardimIcerik(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
			timeout: 30000, processData: false, dataType: 'text', ajaxContentType: wsContentType,
	...
	wsStokResimParam(e) {
		e = e || {}; const data = e.data ?? e.args ?? {}; for (const key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
			timeout: 13000, processData: false, ajaxContentType: wsContentType, data: toJSONStr(data), url: app.getWSUrl({ wsPath: 'ws/genel', api: 'stokResimParam', args: e }) })
	}
	...
	wsEIslemYap(e) {
		...
		return ajaxPost({ timeout: 60 * 60 * 1000, processData: false, ajaxContentType: wsContentType, url: app.getWSUrl({ api: 'eIslemYap', args }), data })
	}
	wsEMailGonder(e) {
