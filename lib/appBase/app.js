class App extends AppBase {
	...
	get superDefaultWSPath() { return super.defaultWSPath }
	get defaultWSPath() { return `${super.defaultWSPath}/skyERP` }
	get testBaseClass() { return TestBase }
	get configParamSinif() { return MQYerelParamConfig } get yerelParamSinif() { return MQYerelParamApp }
	get ortakParamSinif() { return window.MQOrtakParam } get isLoginRequired() { return true }
	get autoExecMenuId() { return this._autoExecMenuId } get sqlDeferWaitMS() { return 100 }
	get dbMgr_initDBNames() { return [] }
	get noCache() { return true }
	get noQueryCache() { return this.noCache }
	get noReqCache() { return this.noCache }
	get ntfyWSRootUrl() { return 'https://ntfy.sh' }
	get ntfyWSRootUrl_cloud() { return 'https://cloud.vioyazilim.com.tr:2090' }
	get ntfyWSUrl() { return this.ntfyWSRootUrl_cloud }
	// get ntfyDefTopic() { return 'sky' }
	get ntfyDefTopic() { return null }
	get ntfyTopic() { return [this.ntfyDefTopic].filter(Boolean) }
	get ntfyTopicStr() {
		let { ntfyTopic } = this
		return ( isArray(ntfyTopic) ? ntfyTopic.filter(Boolean).join('-') : ntfyTopic?.trim() ) || this.ntfyRootTopic
	}
	get ntfyPriority() { return 5 }    // (1..5)
	get buDB() {
		let { dbName } = config.session ?? {}
		return dbName
	}
	get raporEkSahaDosyalari() {
		let { _raporEkSahaDosyalari: result } = this
		if (!result) {
			let _e = { liste: [] }
			this.raporEkSahaDosyalariDuzenle(_e)
			result = _e.liste
		}
		return result
	}
	static get tumModuller() {
		let { _tumModuller: result } = this
		if (result == null) {
			let e = { liste: [] }
			this.tumModulleriDuzenle(e)
			result = {}
			for (let modul of e.liste) {
				let {kod} = modul
				if (kod)
					result[kod] = modul
			}
	...
	constructor(e = {}) {
		...
		$.extend(this, {
			...
			promise_login: defer(),
			promise_ready: defer(),
			promise_importLibs: defer(),
			trnId2Count: {},
			reqCache: new ReqCache(), queryCache: new QueryCache()
		})
	}
	async init(e) {
		let { offlineMode, dbMgrClass, noQueryCache, noReqCache } = this
		for (let key of ['queryCache', 'reqCache']) {
			let cache = this[key]
			if (!cache)
				continue
			let uygunmu = (
				( key == 'queryCache' && !noQueryCache ) ||
				( key == 'reqCache' && !noReqCache )
			)
			if (!uygunmu)
				continue
			if (offlineMode) {
	...
	async runDevam(e) {
		let pr_mustKod = this._promise_portalMustKod = this.portalMustBelirle(e)
		pr_mustKod.finally(() =>
			this.uzakScript_startTimer(e))
		
		delay(100).then(async () => {
			await promiseAllSet(
				self.importLib_pdfMake?.(),
				self.importLib_html2pdf?.(e),
				self.importLib_jsZip?.(),
				self.importLib_tablulator?.()
			)
			this.promise_importLibs?.resolve()
		})
	...
	paramsDuzenle({ params }) {
		let { yerelParamSinif, ortakParamSinif } = this
		extend(params, {
			prot: MQProtParam.getInstance(),
			ticariGenel: MQTicariGenelParam.getInstance()
		})
		if (yerelParamSinif)
			params.yerel = yerelParamSinif.getInstance()
		if (ortakParamSinif)
			params.ortak = ortakParamSinif.getInstance()
		if (window?.MQLocalData)
			MQLocalData._instance = undefined
	...
	async sabitTanimlarDuzenleSonrasi(e) {
		...
		for (let [key, tanim] of entries(sabitTanimlar)) {
			...
			catch (ex) { sabitTanimlar[key] = null }
		}
	}
	...
	async anaMenuOlustur(e) {
		...
		this.frMenu_nav = frMenu.deepCopy()
		if (frMenu && divMenu?.length)
			await frMenu.menuLayoutOlustur({ parent: divMenu })
		await this.anaMenuOlusturSonrasi(e)
	}
	async anaMenuOlusturSonrasi(e) {
		let frMenu_nav = await this.getNavBarMenu(e)
		if (frMenu_nav)
			extend(this, { frMenu_nav })
		await this.navBarDuzenle(e)
		return await this.anaMenuOlusturSonrasiDevam(e)
	}
	async getNavBarMenu(e) { return null }
	async navBarDuzenle(e) {
		let {frMenu_nav, mainNav} = this
		if (frMenu_nav)
			await frMenu_nav.navLayoutOlustur({ parent: mainNav })
	}
	...
	anaMenuOlusturSonrasiDevam(e) {
		...
		if (mainNav?.length) {
			...
			mainNav.jqxResponsivePanel({
				...
				toggleButton: btnMainNavToggle, autoClose: true
			})
			if (btnMainNavToggle?.length)
		...
		let sonIslemlerExecFlag = false
		let menuId = qs.menuAdim || qs.menuId || qs.menuID || qs.menu || this.autoExecMenuId
		if (menuId) {
			let {frMenu} = this
			if (frMenu) {
				...
				if (menuItem) {
					...
					setTimeout(async () => {
						...
						let menuPart = frMenu.part
						if (menuPart) {
							let parentItem = menuItem
							if (parentItem.choicemi)
	...
	dbMgr_tabloEksikleriTamamla({ db, name }) { /*e ??= {}; let {db} = e */ }
		...
		let fillQueries = async (src, urlmi) => {
			for (let [dbName, data] of entries(src)) {
				if (urlmi && (isString(data) || isArray(data))) {
					...
					try { data = await ajaxGet({ dataType: 'text', ajaxContentType: 'text/x-sql', url }) }
					catch (ex) { data = null; console.warn(ex) }
				}
				if (!data)
					continue
				...
				if (isString(data))
					data = data.split(';')
				let queries = data.filter(Boolean).map(_ => _?.trim())
	...
	async sonDegisenDosyalar(e) {
		e ??= {}; let gunSayi = e.gunSayi ?? 0, {dir} = e, recursiveFlag = e.recursive ?? e.recursiveFlag ?? false;
	...
	*getModulIter() { let {moduller} = this; if (moduller) { for (let modul of values(moduller)) { yield modul } } }
	...
	async sqlGetVersion() {
		...
		if (offlineMode)
			return ({ text: 'sqlite3', patchLevel: '', edition: 'sqlite.js (64-bit)', major: 3, minor: 0 })
		
		let query = `SELECT  
		    SERVERPROPERTY('ProductVersion')                   AS  text,
		...
		`
		return await app.sqlExecTekil(query)
	}
	...
	async sqlHasConstraint(e, _typeDizi, _tableLike) {
		return !empty(await this.sqlGetConstraints(e, _typeDizi, _tableLike))
	}
	...
	async sqlGetColumns(e, _nameDizi, _typeDizi, _nameLike) {
		...
		let stm = new MQStm({ sent, orderBy: ['name'] })
	...
	sqlDefTrnDo(e, _proc) { e ??= {}; if (typeof e != 'object') { e = { trnId: e, proc: _proc } } return this.sqlTrnDo(e) }
	async sqlTrnDo(e = {}, _proc) {
		if (!isObject(e))
			e = { trnId: e, proc: _proc }
		
		let { defTrn, trnId } = e
		let trnResult, trnOlustumu = false
		let isMainTrn = !trnId
		trnId ||= this.trnId
		if (!trnId) {
			trnResult = await this[defTrn ? 'sqlDefTrnBegin' : 'sqlTrnBegin']({ trnId })
			trnId = trnResult?.trnId
			trnOlustumu = true
		}
		if (trnId == null)
			throw { isError: true, rc: 'noTransaction', errorText: 'Transaction yok veya geçersiz' }
		
		let { trnId2Count } = this, trnSayi = trnId2Count[trnId] || 0
		if (!trnOlustumu) {
			trnId2Count[trnId] = ++trnSayi
			trnResult = { trnId, trnSayi }
		}
		
		let rollback = false
		try {
			let proc = isObject(e) ? e.proc ?? e.block : _proc
			let _e = { ...e, ...trnResult }
			if (isMainTrn) {
				this.trnId = _e._trnId = trnId
				delete _e.trnId
			}
			
			let result = trnResult.result = await proc.call(this, _e)
			if (result === false)
				rollback = true
		}
		catch (ex) {
			...
			let result = await this.sqlTrnEnd({ trnId, rollback, isMainTrn })
			if (isMainTrn)
				delete this.trnId
			extend(trnResult, { rollback }, result || {})
		}
		
		return trnResult
	}
	async sqlTrnList(e) {
		e ??= {}; let trnId = e.trnId ?? e.trnIdList; let db = e.db ?? e.dbName; if (db === true) { db = app.wsSQL.db }
		return await this.wsSqlTrnList({ trnId })
	}
	async sqlDefTrnBegin(e) {
		e ??= {}; let {trnId2Count} = this; let trnId = typeof e == 'object' ? e.trnId : e;
	...
	async sqlTrnBegin(e) {
		e ??= {}; let {trnId2Count} = this; let trnId = typeof e == 'object' ? e.trnId : e; let trnResult;
	...
	async sqlTrnEnd(e) {
		e ??= {}; let trnId = typeof e == 'object' ? e.trnId : e, isMainTrn = typeof e == 'object' ? e.isMainTrn : null;
	...
	sqlTrnCommit(e) { e ??= {}; if (typeof e != 'object') { e = { trnId: e } } e.rollback = false; return this.sqlTrnEnd(e) }
	sqlTrnRollback(e) { e ??= {}; if (typeof e != 'object') { e = { trnId: e } } e.rollback = true; return this.sqlTrnEnd(e) }
	wsSqlTrnList(e) { e ??= {}; return ajaxPost({ timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, url: app.getWSUrl({ api: 'sqlTrnList', args: e }) }) }
	wsSqlTrnBegin(e) { e ??= {}; return ajaxPost({ timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, url: app.getWSUrl({ api: 'sqlTrnBegin', args: e }) }) }
	wsSqlTrnEnd(e) { e ??= {}; return ajaxPost({ timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, url: app.getWSUrl({ api: 'sqlTrnEnd', args: e }) }) }
	wsSqlTrnCommit(e) { e ??= {}; return ajaxPost({ timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, url: app.getWSUrl({ api: 'sqlTrnCommit', args: e }) }) }
	wsSqlTrnRollback(e) { e ??= {}; return ajaxPost({ timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, url: app.getWSUrl({ api: 'sqlTrnRollback', args: e }) }) }
	async sqlSiraliExec(e) {
		e ??= {}; if (!(e.query ?? e.queries)) { e = { query: e } };
	...
	wsSqlExec(e) {
						...
						try {
							...
							for (let execTip in execTip2Id2SqlExecArgs) {
								try {
									...
									let _e = {
										timeout: _e.timeout ?? 500000, processData: false, contentType: wsContentTypeVeCharSet,
		...
		let _e = {
			timeout: e.timeout ?? 500000, processData: false, contentType: wsContentTypeVeCharSet,
			data, url: this.getWSUrl({ session, wsPath: this.sqlExecWSPath, api: 'sqlExec', args: e })
		}
		console.debug('sqlExec', { defer: false, args: args })
		return ajaxPost(_e)
	}
	wsVIOQueryYapi(e) {
		e ??= {}; let data = e.data ?? e.args ?? {}; for (let key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
	...
	wsWebRequest(e) {
		...
		if (isObject(data))
			data = toJSONStr(data)
		return ajaxPost({
			timeout: 300000, processData: false, dataType: type, ajaxContentType: wsContentTypeVeCharSet, data,
	...
	wsCRUT(e) {
		e ??= {}
		let data = e.data ?? e.args ?? e.queries ?? e.query ?? e
		deleteKeys(e, 'data', 'args', 'queries', 'query')
		if (e == data)
			e = {}
		if (isObject(data) && !(data.query ?? data.queries)) {
			let isArr = isArray(data)
			data = isArr ? { queries: data } : { query: data }
		}
	...
	wsMultiUpload(e) {
		...
		deleteKeys(e, 'data', 'args')
		return ajaxPost({
			timeout: 60000, processData: false, ajaxContentType: wsContentTypeVeCharSet,
			url: app.getWSUrl({ /* wsPath: 'ws/yonetim', */ api: 'multiUpload', args: e }),
			data
		})
	}
	wsTempUpload(e, _data) {
		e = isObject(e) ? e : { localFile: e, remoteFile: e }
		let { data = e.args ?? {} } = e
		deleteKeys(e, 'data', 'args')
		return ajaxPost({
			timeout: 60000, processData: false, ajaxContentType: `application/octet-stream; charset=utf-8`,
			url: app.getWSUrl({ /* wsPath: 'ws/yonetim', */ api: 'tempUpload', args: e }),
			data
		})
	}
	wsYardimIcerik(e) {
	...
	wsResimData(e) { e ??= {}; return ajaxGet({ timeout: 30000, processData: false, dataType: null, url: app.getWSUrl({ api: 'resimData', args: e }) }) }
	wsResimDataKaydet(e) {
		e ??= {}; let {data} = e, mimeType = e.mimeType ?? e.contentType ?? e.type; for (let key of ['args', 'data', 'mimeType', 'contentType', 'type']) { delete e[key] }
	...
	wsDBResimData(e) { e ??= {}; return ajaxGet({ timeout: 30000, processData: false, dataType: null, url: app.getWSUrl({ api: 'dbResimData', args: e }) }) }
	wsStokResimParam(e) {
		e ??= {}; let data = e.data ?? e.args ?? {}; for (let key of ['data', 'args']) { delete e[key] }
		return ajaxPost({
	...
	wsResimDataSil(e) { e ??= {}; return ajaxPost({ timeout: 10000, url: app.getWSUrl({ api: 'resimDataSil', args: e }) }) }
	...
	wsEIslemXMLData(e) { e ??= {}; return ajaxGet({ timeout: 60000, dataType: 'text', url: app.getWSUrl({ api: 'eIslemXMLData', args: e }) }) }
	wsEIslemXSLTData(e) { e ??= {}; return ajaxGet({ timeout: 60000, dataType: 'text', url: app.getWSUrl({ api: 'eIslemXSLTData', args: e }) }) }
	wsEIslemXSLTScript(e) { e ??= {}; return ajaxGet({ timeout: 10000, dataType: 'text', url: app.getWSUrl({ api: 'eIslemXSLTScript', args: e }) }) }
	wsEIslemGoruntu(e) { e ??= {}; return ajaxGet({ timeout: 60000, dataType: 'text', url: app.getWSUrl({ api: 'eIslemGoruntu', args: e }) }) }
	wsXSLTTransform(e = {}) {
		let data = e.data ?? e.args
		if (isObject(data))
			data = toJSONStr(data)
		deleteKeys(e, 'data', 'args')
		let outputType = e.outputType ||= 'data'
		let url = app.getWSUrl({ api: 'xsltTransform', args: e })
		let processData = false, ajaxContentType = wsContentTypeVeCharSet
		let dataType = outputType == 'stream' ? 'text' : 'json'
		return ajaxPost({ timeout: 60 * 60 * 1000, processData, ajaxContentType, dataType, url, data })
	}
	wsXSLTTransformAsStream(e = {}) {
		e = { ...e, outputType: 'stream' }
		return this.wsXSLTTransform(e)
	
	}
	wsLogo(e = {}) {
		let { processData, dataType, wsPath } = e
		let api = 'logoBilgileri', args = { ...e }
		deleteKeys(args, 'processData', 'dataType', 'wsPath')
		let url = app.getWSUrl({ wsPath, api, args })
		return ajaxGet({ processData, dataType, url })
	}
	wsLogoAsStream(e = {}) {
		e = { ...e, stream: true, processData: false, dataType: null }
	...
	wsTSHash(e = {}) {
		...
		if (isObject(data))
			data = toJSONStr(data)
		deleteKeys(e, 'data', 'args')
		let api = 'tsHash', args = e
		let url = app.getWSUrl({ api, args })
		let processData = false, ajaxContentType = wsContentTypeVeCharSet
		return ajaxPost({ processData, ajaxContentType, url, data })
	}
	wsMutabakatBilgi(e = {}) {
		let wsPath = 'ws/eMutabakat', api = 'mutabakatBilgileri', args = e
		let url = app.getWSUrl({ wsPath, api, args })
		return ajaxPost({ url })
	}
	wsMutabakatOnayKoduGonder(e = {}) {
		let wsPath = 'ws/eMutabakat', api = 'mutabakatOnayKoduGonder', args = e
		let url = app.getWSUrl({ wsPath, api, args })
		return ajaxPost({ url })
	}
	wsMutabakatCevapKaydet(e = {}) {
		let data = e.data ?? e.args
		if (isObject(data))
			data = toJSONStr(data)
		deleteKeys(e, 'data', 'args')
		let wsPath = 'ws/eMutabakat', api = 'mutabakatCevapKaydet', args = e
		let url = app.getWSUrl({ wsPath, api, args })
		let processData = false, ajaxContentType = wsContentTypeVeCharSet
		return ajaxPost({ processData, ajaxContentType, url, data })
	}
	wsCariListe(e) { e ??= {}; return ajaxGet({ url: app.getWSUrl({ api: 'cariListe', args: e }) }) }
	wsPlasiyerListe(e) { e ??= {}; return ajaxGet({ url: app.getWSUrl({ api: 'plasiyerListe', args: e }) }) }
	wsPlasiyerIcinCariler(e) { e ??= {}; return ajaxGet({ url: app.getWSUrl({ api: 'plasiyerIcinCariler', args: e }) }) }
	...
	wsFiyatListe(e) { e ??= {}; return ajaxGet({ url: app.getWSUrl({ api: 'fiyatListe', args: e }) }) }
	wsCariKaydet(e = {}) {
		let data = e.data ?? e.args
		if (isObject(data))
			data = toJSONStr(data)
		deleteKeys(e, 'data', 'args')
		let url = app.getWSUrl({ api: 'cariKaydet', args: e })
		let processData = false, ajaxContentType = wsContentTypeVeCharSet
		return ajaxPost({ timeout: 60 * 60 * 1000, processData, ajaxContentType, url, data })
	}
	wsIPCTest(e) {
		let args = e || {}, data = args.data ?? args.args; if (isObject(data)) { data = toJSONStr(data) } delete args.data; delete args.args;
	...
	wsEIslemYap(e) {
		let args = e ?? {}
		let { data = args.args, eLogin, ekArgs } = args
		if (isObject(data))
			data = toJSONStr(data)
		deleteKeys(args, 'data', 'args')

		if (isObject(eLogin))
			eLogin = args.eLogin = toJSONStr(eLogin)
		if (isObject(ekArgs))
			ekArgs = args.ekArgs = toJSONStr(ekArgs)
		
		return ajaxPost({
			timeout: 60 * 60 * 1000, processData: false,
			ajaxContentType: wsContentTypeVeCharSet,
			url: app.getWSUrl({ api: 'eIslemYap', args }),
			data
		})
	}
	wsEMailGonder(e) {
		let args = e || {}, {data} = args;
		if (isObject(data)) { data = toJSONStr(data) } delete args.data;
	...
	async wsFirewall_x(e = {}) {
		...
		data = isObject(data) && empty(data) ? null : toJSONStr(data)
	...
	wsSmsGonder(e = {}) {
		...
		let api = 'smsGonder'
		let url = app.getWSUrl({ api, args })
		let timeout = 60_000, processData = false, ajaxContentType = wsContentTypeVeCharSet
		return ajaxPost({ timeout, processData, ajaxContentType, url, data })
	}
	wsNTFY(e = {}) {
		let data = isObject(e) ? { ...e } : { message: e?.toString() ?? null }
		let { topic = this.ntfyTopic } = data
		if (isArray(topic))
			topic = data.topic = topic.filter(Boolean).join('-')
		let { url = this.ntfyWSUrl } = data
		data.priority ??= this.ntfyPriority
		if (data.tags != null)
			data.tags = makeArray(data.tags)
		data.message ??= data.text ?? data.content
		data.sequence_id ??= data.seqId ?? data.seq ?? data.sequenceId
		deleteKeys(data, 'url', 'text', 'content', 'seqId', 'seq', 'sequenceId')
		...
		if (isObject(data))
			data = toJSONStr(data)
		let processData = false, timeout = 3_000, ajaxContentType = wsContentType
		return ajaxPost({ processData, timeout, ajaxContentType, url, data })
	}

	async uzakScript_startTimer(e = {}) {
		let { rconKey: key } = this
		if (!key)
			return null

		let { es } = this
		// if (es && es.readyState != WebSocket.OPEN) {
		...
		let topic = ['rcon', key].filter(Boolean).join('-')
		let priority = 1
		if (!es) {
			es = this.es = new EventSource(`${app.ntfyWSUrl}/${topic}/sse`)
			es.onmessage = async ({ data: _ }) => {
				let json = false
				if (_ && isString(_))
					_ = JSON.parse(_) ?? {}
				
				let { id: msgId, time, tags, message: msg } = _
				if (!msg || msg == 'triggered')
					return
				
				if (time && this._remoteCode_lastTime && time - this._remoteCode_lastTime < 1)
					return

				let type = tags?.find(x => x[0] == '_')?.toLowerCase()?.slice(1)
				let data
				try {
					data = JSON.parse(msg)
					data = data?.data ?? data
				...
				this._remoteCode_lastTime = time
				...
				if (!type || type == 'code') {
					try {
						...
						let callback = data => {
							return ntfy({
								topic, priority,
								tags: ['o', '_callback', '_msgId', msgId],
								message: (
									json
										? toJSONStr({ data })
										: ( isObject(data) ? toJSONStr(data) : data?.toString() ?? 'null' )
								)
							})
						}
						let _e = { ...e, key, es, callback }
						if (isString(func))
							func = eval(func)
						if (isClass(func))
							func = new func()
						...
						let res = await (
							isFunction(func) ? func.call(this, _e) :
							func.run ? func.run.call(this, _e) :
							func
						)
						...
						setTimeout(() => {
							ntfy({
								topic, priority,
								tags: ['white_check_mark', '_result', '_msgId', msgId],
								message: (
									json
										? toJSONStr({ data: res })
										: ( isObject(res) ? toJSONStr(res) : res?.toString() ?? 'null' )
								)
							})
						}, 1)
					}
						...
						try {
							let _ex = ex
							if (isObject(_ex)) {
								_ex = _ex.responseJSON ?? _ex.responseText ?? _ex
								let tmp = toJSONStr(_ex)
								if (tmp == {}.toString() || tmp == toJSONStr({}.toString()))
									tmp = getErrorText(_ex)
								_ex = tmp
							}
							cerr(ex)
							await ntfy({
								topic,
								tags: ['x', '_error', '_msgId', msgId],
								message: (
									json
										? toJSONStr({ data: _ex })
										: ( isObject(_ex) ? toJSONStr(_ex) : _ex?.toString() ?? 'null' )
								)
							})
						}
						catch (ex2) { cerr(ex2) }
			...
			es.onerror = ({ data }) => {
				cerr(data)
				this.ws = null
				setTimeout(() => this.uzakScript_startTimer(...arguments), 1_000)
			}
			es.onclose = evt => {
				this.ws = null
				setTimeout(() => this.uzakScript_startTimer(...arguments), 100)
			}
		}
	}
	async uzakScript_startTimer_old(e = {}) {
		let { config: param } = this.params || {}
		let { uzakScriptURL: key } = param
		if (!key)
			return null

		let callbackKey = `${key}.callback`
		let contentType = wsContentType
		...
		if (!es) {
			es = this.es = new EventSource(app.getEventStreamURL({ key }))
			es.onmessage = async ({ data: _ }) => {
				if (!isObject(_))
					_ = JSON.parse(_ ?? {})
				let { type, data } = _
		...
		/*
			...
			let { esr } = self
			if (!esr) {
				esr = self.esr = new EventSource(app.getEventStreamURL({ key: callbackKey }))
				esr.onmessage = ({ data: _ }) => {
					if (isString(_))
						_ = JSON.parse(_)
					let { type, data } = _
					switch (type) {
			...
			;{
				...
				app.wsSignal({ key, data })
			}

		*/

		
		/*e ??= {}; super.uzakScript_startTimer(e)
	...
	/*async uzakScript_proc(e) {
		e ??= {}; let param = (this.params || {}).config || {}, {uzakScriptURL} = param; if (!uzakScriptURL) return false
	...
	async getEMailAuth(e) {
		let result = this._eMailAuth
		if (result === undefined) {
			let encData = e?.encData || encEMailAuthStr
			if (!encData)
				return null
	...
	online() { this._offlineMode = false; return this }
	offline() { this._offlineMode = true; return this }
	resetOfflineStatus() { this._offlineMode = null; return this }
}
...
try {
	...
	let orj = JSON.parse( await app.wsDownloadAsStream({ remoteFile: `f:/tmp/kdv1.json` }) )
	ctable(orj)
	
	arrays = {
		ihracKaydiylaTeslimler_ihracKaydiylaTeslimler: orj.ihracKaydiylaTeslimler.ihracKaydiylaTeslimler,
		indirimler_digerIndirimler: orj.indirimler.digerIndirimler,
		indirimler_indirilecekKdvDagilimi: orj.indirimler.indirilecekKdvDagilimi,
		istisnalar_tamIstisna: orj.istisnalar.tamIstisna
	}
	for (let [key, data] of entries(arrays)) {
		//console.group(key)
		//ctable(data)
		//console.groupEnd()

		let elm = $(`<table id="${key}" class="full-width" style="margin-bottom: 3em"/>`)
		elm.appendTo(content)
		let _keys = keys(data[0])
		...
		let tbl = new Tabulator(elm[0], {
		    ...
		    columns: _keys
				.map(k => ({
					field: k, title: k,
					editor: k.toLowerCase().includes('bedel') || k.toLowerCase().includes('oran') || k.toLowerCase().includes('tutar')
						? 'number' : 'input'
				}))
		...
		setTimeout(() => {
			tbl.addData([
				{ istisnaTuru: 'DENEME', teslimBedeli: 10.30, kdvOrani: 0.1 },
				{ istisnaTuru: 'DENEME 2', kdvOrani: 0.0 }
			], true)
		})
...
let data = `clog('test')`
let args = { stream: true, type: 'application/json', data }
await importLib(app.getWSUrl({ api: 'echo', args, session: null }), true, 't1')


let data = toJSONStr({ a: 1 })
await app.wsWriteTemp({ key: 'k', temp: true, contentType: wsContentType, data })
await app.wsReadTempAsStream({ key: 'k' })
data = `$$temp-k`
let args = { data }
await app.wsEchoAsStream({ args })
...
(async () => {
	let launched = true
	let key = 'test'
	let temp = true, counter = 0
	
	if (!launched) {
		let data = toJSONStr({ foo: 'bar' })
		if (launched)
			await app.wsSignal({ key, data })
		else {
			launched = true
			await app.wsWriteTemp({ key, data, temp })
		}
	}
	...
	;{
		async function handleEvent({ data }) {
			...
			data = { result: ++counter }
			await app.wsWriteTemp({ key: `${key}-result`, data, temp })
		}
		try {
			let { base64, data } = await app.wsReadTemp({ key }) ?? {}
			if (data) {
				if (base64)
		...
		let es = new EventSource(app.getEventStreamURL(key))
		es.onmessage = handleEvent
		es.onerror = evt => cerr(evt.data)
	}
})()
