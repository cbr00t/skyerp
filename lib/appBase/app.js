class App extends AppBase {
	...
	async ilkIslemler(e) {
		let { params, sabitTanimlar, _paramYuklendi: paramYuklendi, class: { tumModuller } } = this
		let { hasSession } = config
		
		try {
			let lastError, { promise_param: promise } = this
			try { await promise }
			catch (ex) { lastError = ex }
			if (hasSession && (!promise || lastError))
		...
		let withPromisesDo = async block => {
			...
			if (promises?.length) {
				try { await promiseAllSet(promises) }
				catch (ex) { console.error(ex); debugger }
			}
			return promises
		}
	...
	async anaMenuOlustur(e) {
		...
		let frMenu = await this.getAnaMenu(e)
		
		;{
			let args = { ...e, divMenu, frMenu }
			await ir?.anaMenuDuzenle?.(args)
			divMenu = this.divMenu = args.divMenu
			frMenu = args.frMenu
		}
		this.frMenu = frMenu
	...
	async anaMenuOlusturSonrasi(e) {
		let { injectResult: ir } = this
		if (await ir?.anaMenuOlusturSonrasi?.(e) === false)
			return false
	...
	async navBarDuzenle(e) {
		let { frMenu_nav, mainNav, injectResult: ir } = this
		let res = ir?.navBarDuzenle?.({ ...e, frMenu_nav, mainNav })
		if (frMenu_nav)
	...
	navLayoutOlustur_araIslem(e) {
		...
		;items.forEach(r =>
			r.navLayoutOlustur(e))

		let { injectResult: ir } = this
		ir?.navLayoutOlustur_araIslem?.({ ...e, items })
	}
	...
	async sonIslemler(e) {
		...
		;{
			let { afterRun: afterRunBlock } = qs
			if (isString(afterRunBlock))
				afterRunBlock = getFunc(afterRunBlock)
			if (!afterRunBlock) {
				afterRunBlock = qs.afterRunBase64
				if (isString(afterRunBlock) && Base64.isValid(afterRunBlock))
					afterRunBlock = Base64.decode(afterRunBlock)
			...
			if (afterRunBlock) {
				let { injectResult: ir } = this
				if (await ir?.afterRun?.({ ...e, afterRunBlock }) === false)
					return false
				
				try {
					await delay(1)
					let args = {
						...e, sender: this, app: this,
						activePart: this.activePart, frMenu: this.frMenu
					}
					getFuncValue.call(this, afterRunBlock, args)
				}
				catch (ex) {
	...
	*getModulIter() { let {moduller} = this; if (moduller) { for (let modul of values(moduller)) { yield modul } } }
	...
	async sqlSiraliExec(e) {
		e ??= {}
		if (!(e.query ?? e.queries))
			e = { query: e } 
		
		let delim = 'UNION ALL'
		let { query: queries = e.queries } = e
		queries = queries?.liste ?? queries
		if (!isArray(queries))
			queries = queries.split(delim)
		if (isArray(queries))
		...
		for (let query of queries) {
			...
			try {
				_e.query = query
				let result = await app.sqlExec(_e)
			...
			extend(item, { seq, query, queryYapi })
			results.push(item)
		}
		return results
	}
	...
	wsDosyaListe(e = {}) {
		...
		deleteKeys(e, 'data', 'args')
		return ajaxPost({
			timeout: 60000, processData: false, 
			ajaxContentType: wsContentTypeVeCharSet,
			data: toJSONStr(data),
			url: app.getWSUrl({ /* wsPath: 'ws/yonetim', */ api: 'dosyaListe', args: e })
		})
	}
	async wsSqlServerListe(e = {}) {
		let { data = e.args = {} } = e
		deleteKeys(e, 'data', 'args')

		let { injectResult: ir } = this
		let res = await ir?.sqlServerListe?.(e)
		res = ajaxPost({
			timeout: 8000, processData: false, ajaxContentType: wsContentTypeVeCharSet, data: toJSONStr(data),
			url: app.getWSUrl({ api: 'sqlServerListe', args: e })
		})
		if (ir?.sqlServerListeDuzenle) {
			let args = { ...e, result: res }
			await ir.sqlServerListeDuzenle(args)
			res = args.result
		}
	}
	async wsDBListe(e = {}) {
		...
		let { injectResult: ir } = this
		let res = await ir?.dbListe?.(e)
		if (res === undefined) {
			res = await ajaxPost({
				timeout: 13_000, processData: false,
				ajaxContentType: wsContentTypeVeCharSet, data: toJSONStr(data),
				url: app.getWSUrl({ api: 'dbListe', args: e })
			})
			if (ir?.dbListeDuzenle) {
				let args = { ...e, result: res }
				await ir.dbListeDuzenle(args)
				res = args.result
			}
		}
		return res
