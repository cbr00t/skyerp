class AppBase extends LayoutBase {
	...
	async init(e) {
		...
		super.init(e); this.promise_inject = this.wsInjectCode(e)
			.then(text => {
		...
		const $root = $(':root'); $root.css('--appTitle-height', config.noHeader ? '3px' : 'var(--appTitle-height-mini)'); $root.css('--nav-top', 'var(--nav-top-mini)');
	...
	async openNewWindow(e) {
		e = e || {}; const _qs = { ...qs, inNewWindow: true }, session = config.session ?? {}, {sessionID, user, pass} = session, _session = session.session;
		const removeKeys = ['sessionID', 'session', 'sessionMatch', 'loginTipi', 'user', 'pass', 'menuAdim', 'menuId', 'menuID', 'menu', 'afterRun', 'newWindow', 'inNewWindow', '#nbb'];
		...
		let menuId; const {activeWndPart, lastMenuId} = app; if (activeWndPart && activeWndPart != app && lastMenuId) { menuId = lastMenuId }
		if (!menuId) { menuId = qs.menuAdim || qs.menuId || qs.menuID || qs.menu } if (menuId) { _qs.menuAdim = menuId }
	...
	wsInjectCode(e) { let wsURLBase = app.getWSUrlBase({ wsPath: '' }); if (wsURLBase?.endsWith('/')) { wsURLBase = wsURLBase.slice(0, -1) } return ajaxPost({ dataType: 'text', url: `${wsURLBase}/skyERP.${this.appName}.override.js` }) }
	...
	async wsLogin(e) {
		...
		if (infoOnly) args.infoOnly = true; const _session = e.session, {loginTipi} = _session; if (!loginTipi) { loginTipi = Session.defaultLoginTipi }
