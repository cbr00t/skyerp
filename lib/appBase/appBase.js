class AppBase extends LayoutBase {
	...
	async init(e = {}) {
		...
		this.promise_inject = this.wsInjectCode(e)
			.then(text => {
				let injectResult;
				try { injectResult = this.injectResult = eval(text) }
				catch(ex) { console.error('inject-code-init', ex, text); if (config.dev) { hConfirm(getErrorText(ex), 'Inject Code Error') } }
	...
	runDevam(e = {}) {
		let { injectResult } = this
		window.boot?.step()
		injectResult?.beforeRun({ sender: this, ...e })
		injectResult?.run?.({ sender: this, ...e })
		super.runDevam(e)
		window.boot?.step()
	}
	afterRun(e) {
		...
		(async () => {
			...
			injectResult?.afterRun?.({ sender: this, ...e })
			delay(10).then(() =>
				window.boot?.end?.())
		})()
	}
	...
	wsInjectCode(e) {
		let wsURLBase = app.getWSUrlBase({ wsPath: '' })
		if (wsURLBase?.endsWith('/'))
			wsURLBase = wsURLBase.slice(0, -1)
		try {
			return Promise.any([
