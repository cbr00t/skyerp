class AppBase extends LayoutBase {
	...
	async wsMD5(e) {
		...
		const {delimXEnc, md5Dec2Enc, md5Enc2Dec} = this.class, anah = keys.join(delimXEnc);
	...
	async _wsMD5(e) {
		e = e || {}; if ($.isArray(e)) { e = { dataArr: e } } else if (typeof e != 'object') { e = { data: e } }
		const result = await ajaxPost({ ajaxContentType: wsContentTypeVeCharSet, url: this.getWSUrl({ api: 'md5' }), processData: false, data: toJSONStr(e) });
	...
	async _wsMD5AsStream(e) {
		...
		const result = await ajaxPost({ ajaxContentType: wsContentTypeVeCharSet, dataType, url: this.getWSUrl({ api: 'md5' }), processData: false, data: toJSONStr(e) });
		return result == null || $.isArray(result) ? result : (result.result ?? result)
	}
	async wsXEnc(e) {
		e = e || {}; if ($.isArray(e)) { e = { dataArr: e } } else if (typeof e != 'object') { e = { data: e } }
	...
	wsRawSocket_readAny(e) {
		let {key} = e; let url = this.getWSUrl({ api: 'rawSocket_readAny', args: { key } });
		return ajaxPost({ url })
	}
