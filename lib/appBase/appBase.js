class AppBase extends LayoutBase {
	...
	get kioskmuDogrudan() { return false } get kioskmu() { return this.kioskmuDogrudan || config.kiosk } get inKioskMode() { return $('body').hasClass('kiosk') }
	...
	async init(e) {
		...
		super.init(e); if (config.darkThemeFlag) { $('body').addClass('dark-theme' )}
	...
	wsEventStream(e) {
		const {key} = e; let dataType = e.dataType || wsDataType, contentType = e.contentType || wsContentType;
		const {wsKey2Callbacks} = this, callbacks = wsKey2Callbacks[key] = wsKey2Callbacks[key] || [];
		const promise = ajaxGet({ dataType, ajaxContentType: contentType, url: this.getWSUrl({ api: 'eventStream', args: { key } }) }); callbacks.push(promise);
		promise.always(xhr => { const {wsKey2Callbacks} = this, callbacks = wsKey2Callbacks[key]; if (callbacks) { callbacks.splice() } delete wsKey2Callbacks[key] });
		return promise
	}
