...
function timeToString(value, noSecsFlag, msFlag) {
	if (value == null || typeof value == 'string') { return value }
	if (typeof value == 'number') { value = new Date(0).clearTime().addMilliseconds(value) }
	let result = value.toString(noSecsFlag && !msFlag ? TimeFormat_Saniyesiz : TimeFormat);
	if (msFlag) { const fra = 1; result += ('.' + value.getMilliseconds().toString().substr(0, fra).toString().padStart(fra, '0') ) }
	return result;
}
