...
function displayMessage(e, __title, __buttons, __default) {
	...
	const hasBGModal = $('body').hasClass('bg-modal'), wnd = createJQXWindow(e); if (wnd) {
		...
		if (hasBGModal) { $('body').removeClass('bg-modal') }
		setTimeout(() => wnd.jqxWindow('focus'), 100)
	}
...
function asSaniyeKisaString(value, saniyesizFlag) {
	...
	let result = ''; if (sa > 0) result += `${sa.toLocaleString()}sa`
	if (dk > 0) { if (result) result += ' '; result += `${dk.toLocaleString()}dk` }
	if (sn > 0) { if (result) result += ' '; result += `${roundToFra(sn, 1).toLocaleString()}sn` }
	return result
}
...
function arrayAggregate(getter, action, ...items) {
	let result = 0; for (const _item of items) { let item = getter ? getter(_item) : _item; if (item != null) { result = action(result, item) } }
	return result
}
