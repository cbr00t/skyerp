...
async function importLib(e) {
	let url = isObject(e) ? e.url : e
	if (!url)
		return null
	let { force } = e
	if (!force && document.querySelector(`script[src = "${url}"]`))
		return { isError: true, rc: 'alreadyLoaded', errorText: `Modül zaten yüklenmiş: [${url}]` }
	return await new Promise((r, f) => {
		let script = document.createElement('script')
		script.src = url
		script.onload = evt => r(evt)
		script.onerror = evt => f(evt)
		document.head.append(script)
	})
	// $(`<script src="${webRoot}/lib_external/etc/xslt-polyfill.min.js"></script>`).appendTo('head')
}
function importLib_xslt(e) {
	e = isObject(e) ? e : { url: e }
	return importLib({ ...e, url: `${webRoot}/lib_external/etc/xslt-polyfill.min.js` })
}
function importLib_pdfMake(e) {
	e = isObject(e) ? e : { url: e }
	return importLib({ ...e, url: `${webRoot}/lib_external/etc/pdfmake.min.js` })
}
function importLib_html2pdf(e) {
	e = isObject(e) ? e : { url: e }
	return importLib({ ...e, url: `${webRoot}/lib_external/etc/html2pdf.bundle.min.js` })
}
function importLib_jsZip(e) {
	e = isObject(e) ? e : { url: e }
	return importLib({ ...e, url: `${webRoot}/lib_external/etc/jszip.min.js` })
}
