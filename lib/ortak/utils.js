	...
	let wnd = e.wnd = createJQXWindow({
		...
		buttons: {
			TAMAM: async _e => {
				...
				if (await validate?.(e) === false) { return false }
...
function readQSDict() {
	let vars = {}; let hashes = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&'); let hash;
...
function toJSONStr(obj, space) {
	obj = obj ?? null;
	if (obj != null && obj instanceof Map) { let _obj = obj; obj = {}; for (let [key, value] of _obj) { obj[key] = value } }
