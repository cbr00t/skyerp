...
function displayMessage(e, __title, __buttons, __default) {
	...
	let {title} = e; const args = e.args = e.args = e.args || {}; /* const title = e.title = e.title == null ? ' ' : e.title; */
	$.extend(args, {
	...
	let hasBGModal = $('body').hasClass('bg-modal'), wnd = createJQXWindow(e); if (wnd) {
		...
		setTimeout(() => { for (const key of ['mouseup', 'touchend']) { $(document).on(key, focusCloseHandler) } }, 500);
...
function gA(t) {
  const {namesAbbr: kisaAy} = localizationObj.months;
...
function asSet(anArray) {
	...
	const result = {}; for (const key in anArray) { const item = anArray[key]; result[item] = true }
	return result
}
...
function gridDipIslem_avg_minus1(toplam, value, belirtec, rec, _) {
	if (value?.constructor?.name == 'Number') { value = asFloat(value) }
	toplam = (toplam || 0) + (value || 0);
	const count = rec?.boundindex; if (toplam && count && typeof count == 'number') { toplam = toplam / count }
	return isNaN(toplam) ? 0 : roundToBedelFra(toplam)
}
async function getFS(temp, e) {
	e = e || {}; temp = temp ?? e.temp;
	const {storage} = navigator; /*if (temp == null) { temp = !(await storage.persist()) }*/
	const fs = await storage.getDirectory({ type: temp == null ? undefined : (temp ? 'temporary' : 'persistent') });
