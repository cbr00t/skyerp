...
var enc2Dec, dec2Enc
var minDate = asDate('01.01.1900 00:00:00')
...
var libs = {}
var _isVirtualKeyboardSupported
...
function ajaxCall(e = {}) {
	...
	let proxyServerURL = fixProtocol(e.proxyServerURL ?? config?.ws?.proxyServerURL)
	let proxyURL = fixProtocol(e.proxyURL)
	let { url } = e
	if (proxyServerURL || proxyURL) {
		e.url = proxyURL
			? (`${app.getWSUrlBase({ wsPath: 'ws/proxy' })}/send/?url=${proxyURL}` )
			: app.getWSUrl({ wsPath: 'ws/proxy', api: 'send', args: { url: `${url.replace(config.wsURLBase, proxyServerURL)}` } })
	}
	return ajaxCallDirect(e)
}
function ajaxCallDirect(e = {}) {
	let { offlineMode: offline, reqCache } = app ?? {}
	/*if (offline && reqCache?.length)
		return reqCache.get(e)*/
	let promise = lastAjaxObj = defer(async p => {
		try {
			if (!navigator.onLine)
				throw { rc: 'offline', statusText: 'error' }
			let result = await $.ajax({
				cache: false, processData: e.processData ?? false,
			...
			clearTimeout(this._timer_onAjaxEnd)
			this._timer_onAjaxEnd = setTimeout(() =>
				app?.onAjaxEnd?.(false), .5)
			p.resolve(result)
		}
		catch (ex) {
			let { rc: r = ex.code, readyState: s } = ex
			if (r == 'offline' || s < 4) {
			...
			clearTimeout(this._timer_onAjaxEnd)
			this._timer_onAjaxEnd = setTimeout(() => app?.onAjaxEnd?.(true), 8)
			p.reject(ex)
		}
	})
	app?.onAjaxStart?.(false)
	return promise
}
function ajaxGet(e) { return lastAjaxObj = ajaxCall({ type: 'GET', ...e }) }
function ajaxPost(e) { return lastAjaxObj = ajaxCall({ type: 'POST', ...e }) }
function ajaxPut(e) { return lastAjaxObj = ajaxCall({ type: 'PUT', ...e }) }
function ajaxPUT(e) { return ajaxPut(e) }
function findScript(e, _url) {
	let isObj = isObject(e)
	let id = isObj ? e.id : e
	let url = _url ?? isObj ? e.url ?? e.src : null
	let result
	if (id)
		result = document.querySelector(`script[id = "${id}"]`)
	if (!result && url)
		result = document.querySelector(`script[src = "${url}"]`)
	return result
}
function getLib(e) {
	let isObj = isObject(e)
...
async function importLib(e, _force, _id, _temp) {
	let isObj = isObject(e)
	let url = isObj ? e.url ?? e.src : e
	if (!url)
		return null
	
	let force = _force ?? ( isObj ? e.force : null )
	let id = _id ?? ( isObj ? e.id : null )
	if (!force && document.querySelector(`script[src = "${url}"]`))
		return { isError: true, rc: 'alreadyLoaded', errorText: `Modül zaten yüklenmiş: [${url}]` }

	libs ??= {}
	let temp = _temp ?? ( isObj ? e._temp : null ) ?? false
	let pr = promise((r, f) => {
		let { head } = document
		let script = document.createElement('script')
		async function withCleanup(block) {
			try {
				return await block()
			}
			finally {
				if (temp && script)
					setTimeout(() => script?.remove(), 1)
			}
		}
		script.setAttribute('crossorigin', 'anonymous')
		if (id)
			script.setAttribute('data-lib', id)
		script.src = url
		script.onload = event =>
			withCleanup(() => r({ script, url, event }))
		script.onerror = event =>
			withCleanup(() => f({ script, url, event }))
		head.append(script)
	})
	if (id)
		libs[id] = pr
	return await pr
	// $(`<script src="${webRoot}/lib_external/etc/xslt-polyfill.min.js"></script>`).appendTo('head')
}
function importLib_xslt(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'xslt', url: `${webRoot}/lib_external/etc/xslt-polyfill.min.js` })
}
function importLib_pdfMake(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'pdfMake', url: `${webRoot}/lib_external/etc/pdfmake.min.js` })
}
function importLib_html2pdf(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'html2pdf', url: `${webRoot}/lib_external/etc/html2pdf.bundle.min.js` })
}
function importLib_jsZip(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'jsZip', url: `${webRoot}/lib_external/etc/jszip.min.js` })
}
function importLib_tablulator(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'tabulator', url: `${webRoot}/lib_external/tabulator/js/tabulator.min.js` })
}
function importLib_eruda(e) {
	e = isObject(e) ? e : {}
	return importLib({ ...e, id: 'eruda', url: `${webRoot}/lib_external/etc/eruda.js` })
...
function ntfy(e = {}, _title, _seqId, _priority, _tags, _actions, _icon, _markdown, _email, _click, ..._args) {
	e = isObject(e) ? { ...e } : {
		message: e, title: _title,
		sequence_id: _seqId, priority: _priority,
		tags: _tags, actions: _actions,
		icon: _icon, markdown: _markdown,		
		email: _email, click: _click,
		..._args
	}
	return app.wsNTFY(e)
}
function rcon(e, _key, _callbacks) {
	return app.rcon(e, _key, _callbacks)
}
...
function createJQXWindow(e) {
	...
	let _e = {
		theme, autoOpen: true, isModal: true, showCloseButton: true,
		showCollapseButton: true, closeButtonAction: args.closeButtonAction, closeButtonSize: 20, minWidth: 100, minHeight: 40,
		//keyboardCloseKey: ''
		...args
	}
	if (_e) {
...
function showProgress(message, title, showProgressFlag, abortBlock, ekBilgi, isModal) {
	...
	}); let content = wndProgress.find('div > .content')
...
function hideNotify() {
	return $('.sn-notifications-container').remove()
}
function jqxPrompt(e = {}) {
	let {
		title, args, etiket, placeHolder = e.placeholder, validate,
		inst = {}, promise, ioAttr = 'value', value, maxLength,
		duzenle, init, buildEk, afterRun,
		numerik, fbdSelector
	} = e
	fbdSelector ||= numerik ? 'addNumberInput' : 'addTextInput'
	if (value != null)
		inst.value = value
	
	e.promise = (promise ??= defer())
	let mini = isMiniDevice()
	let width = min(mini ? 400 : 600, $(window).width() - 50)
	let height = min(mini ? 200 : 250, $(window).height() - 20)
	if (title)
		height += 20
	...
	let rfb = e.rfb = new RootFormBuilder()
		.setInst(inst)
		.setLayout(wnd.find('.jqx-window-content > .subContent'))
		.addStyle(
			`$elementCSS { margin: 20px 0 -10px 0 }
			 $elementCSS > [data-builder-id = value] { margin-bottom: 10px }
			 $elementCSS > [data-builder-id = value] > label { color: royalblue; margin-bottom: 10px }
		`)
	let fbd_value = e.fbd_value = rfb[fbdSelector](ioAttr, etiket, undefined, placeHolder, maxLength)
	...
	rfb.onInit(_e =>
		init?.call?.(this, { ...e, ..._e }))
	rfb.onBuildEk(_e =>
		buildEk?.call?.(this, { ...e, ..._e }))
	rfb.onAfterRun(_e => {
		fbd_value.input.focus()
		afterRun?.call?.(this, { ...e, ..._e })
	})
	if (duzenle?.call(this, e) === false)
		return null
...
function readQSDict(url) {
	...
	for (let i = 0; i < hashes.length; i++) {
		...
		if (!key || key == '#')
			continue
		let value = hash[1]
		if (value == null && key?.endsWith('/'))
			key = key.slice(0, -1)
		value ??= true
		let lv = isString(value) ? value?.toLowerCase() : undefined
		value = (
			lv == 'true' ? true :
			lv == 'false' || lv == '' ? false :
			lv ? decodeURIComponent(value) : value
		)
		result[key] = value
	}
	...
	if (base64) {
		try {
			let data = window.Base64?.decode?.(base64) ?? atob(base64)
			if (data?.[0] == '?')
				data = data.slice(1)
			if (data) {
				let _qs = readQSDict(`?${data}`)
				if (!empty(_qs))
					extend(result, _qs)
			}
		}
...
function getErrorText(error, _wsURLBase) {
	if (error) {
		let { code = error.rc, ABORT_ERR } = error
		if (code == 'userAbort' || code == 'userClose' || (ABORT_ERR && code == ABORT_ERR))
			return null
	}
	...
	if (error?.errorText ?? error?.code) {
		let code = config?.dev ? ( error.rc ?? error.code ) : null
	...
	if (error) {
		if (isArray(error))
			error = error.join(CrLf)
		let {statusText} = error, wsURLBase = _wsURLBase ?? app?.wsURLBase
		if (statusText == 'error' || statusText == 'timeout')
			error = `<h4>Merkez ile iletişim sorunu</h4><div style="padding-left: 10px;">(<b><a href="${wsURLBase}/getSessionInfo" target="_blank">${wsURLBase}</a></b>)</div>`
	...
	return isObject(error) ? null : error
}
...
function string2Numeric(value) {
	return value && typeof value == 'string' ? value.replaceAll(/[^0-9]+/g, '') : value
}
function turkcesiz(value) {
...
function numberToString(value, options) {
	if (value == null || !isNumber(value))
		return null
	return Number(value).toLocaleString(culture, extend({ minimumIntegerDigits: 1, useGrouping: true }, options || {}))
}
function toStringWithFra(value, fra, options) {
	if (value == null || !isNumber(value) || fra < 0)
		return null
	if (typeof fra == 'object' || isFunction(fra) || fra < 0 || fra > 20)
		fra = null
	return Number(value).toLocaleString(culture, $.extend({ minimumIntegerDigits: 1, minimumFractionDigits: fra, maximumFractionDigits: fra, useGrouping: true }, options || {}))
}
function toFileStringWithFra(value, fra, options) {
	if (value == null || !isNumber(value) || fra < 0)
		return null
	if (typeof fra == 'object' || isFunction(fra) || fra < 0 || fra > 20) { fra = null }
	return Number(value).toLocaleString('en-US', $.extend({ minimumIntegerDigits: 1, minimumFractionDigits: fra, maximumFractionDigits: fra, useGrouping: false }, options || {}))
}
function fra0ToString(value, options) { return toStringWithFra(value, 0, options) }
function fra1ToString(value, options) { return toStringWithFra(value, 1, options) }
function fra2ToString(value, options) { return toStringWithFra(value, 2, options) }
function fra3ToString(value, options) { return toStringWithFra(value, 3, options) }
function fra4ToString(value, options) { return toStringWithFra(value, 4, options) }
function fra5ToString(value, options) { return toStringWithFra(value, 5, options) }
function fra6ToString(value, options) { return toStringWithFra(value, 6, options) }
function bedelToString(value, options) { return toStringWithFra(value, (app?.params?.zorunlu?.bedelFra) || 2, options) }
function fiyatToString(value, options) { return toStringWithFra(value, (app?.params?.zorunlu?.fiyatFra) ?? 5, options) }
function dvBedelToString(value, options) { return toStringWithFra(value, (app?.params?.zorunlu?.dvBedelFra) ?? 2, options) }
function dvFiyatToString(value, options) { return toStringWithFra(value, (app?.params?.zorunlu?.dvFiyatFra) ?? 5, options) }
function dvKurToString(value, options) { return toStringWithFra(value, (app?.params?.zorunlu?.dvKurFra) ?? 6, options) }
function fra1Str(value) { return fra1ToString(value) }
function fra2Str(value) { return fra2ToString(value) }
function fra3Str(value) { return fra3ToString(value) }
function roundToFra(value, fra) {
	if (!value || fra < 0)
		return value
	if (isNumber(value))
		return Number(fra == null ? value : value.toFixed(fra))
	return fra == null ? Number(value) : roundToFra(Number(value), fra)
}
function roundToFra1(value) { return roundToFra(value, 1) }
function roundToFra2(value) { return roundToFra(value, 2) }
function roundToFra3(value) { return roundToFra(value, 3) }
...
function evalOrSelf(value) {
	return isString(value) ? eval(value) : value
}
function classifyValue(value) {
	...
	if (type == 'function' && value.prototype instanceof CObject)
		return 'class'
	...
	if (type == 'function') {
		// ES6 class
		let desc = Object.getOwnPropertyDescriptor(value, 'prototype')
		return (
			desc &&
			desc.writable == false &&
	...
	if (type == 'object') {
		...
		if (typeof ctor == 'function') {
			let desc = Object.getOwnPropertyDescriptor(ctor, 'prototype')
...
function isObject(value) { return value != null && typeof(value) == 'object' && !(value instanceof Date) }
// function isDate(value) { return value instanceof Date && !isNaN(value.getTime()) }
function isCLDT(value) {
	let t = classifyValue(value)?.toLowerCase()
	return [null, 'string', 'number', 'boolean', 'date'].includes(t)
}
...
function coalesceNull(value, aBlock) {
	if (value != null)
		return value
	return getFuncValue(aBlock, value)
}
...
function sifirlaDoldur(value, length) { return value?.toString()?.padStart(length, '0') }
...
function asSaniyeKisaString(value, saniyesizFlag, dtFlag) {
	...
	value = Number(value)
	let kalan = value
	let gun = asInteger(kalan / 86400); kalan -= (gun * 86400);
	let sa = asInteger(kalan / 3600)
	kalan -= (sa * 3600)
	let dk = 0, sn = 0
	if (saniyesizFlag) { dk = roundToFra(asFloat(kalan / 60), 0) }
	else { dk = asInteger(kalan / 60); sn = kalan - (dk * 60) }
	if (dtFlag) {
		let result = gun ? `${gun}g ` : ''
		result += `${sifirlaDoldur(sa, 2)}:${sifirlaDoldur(dk, 2)}:${sifirlaDoldur(sn, 2)}`
		return result
	}
	else {
		let result = []
...
function min() { return Math.min(...arguments) }
function pow(value, exp) { return Math.pow(Number(value), exp) || 0 }
function sqrt(value) { return Math.sqrt(Number(value) || 0) }
function abs(value) { return Math.abs(Number(value) || 0) }
function random() { return Math.random() }
function floor(value) { return Math.floor(Number(value) || 0) }
function ceil(value) { return Math.ceil(Number(value) || 0) }
function round(value) { return Math.round(Number(value) || 0) }
function trunc(value) { return Math.trunc(Number(value) || 0) }
function negated(value) { return -(Number(value) || 0) }
...
function asDate(value) {
	if (value == null)
		return value
	if (isNumber(value))
		return new Date().clearTime().addSeconds(value)
	if (!isString(value))
		return value
	...
	if (dtParts?.length) {
		...
		if (datePart.includes('-')) { let parts = datePart.split('-'); info = { year: parseInt(parts[0]), month: parseInt(parts[1]), day: parseInt(parts[2]) } }
		else if (datePart.includes('.')) { let parts = datePart.split('.'); info = { day: parseInt(parts[0]), month: parseInt(parts[1]), year: parseInt(parts[2]) } }
		else if (datePart.includes('/')) { let parts = datePart.split('/'); info = { month: parseInt(parts[0]), day: parseInt(parts[1]), year: parseInt(parts[0]) } }
		...
		if (info && timePart?.length > 1) {
			if (timePart.includes(':')) {
				...
				Object.assign(info, { hours: parseInt(parts[0]), minutes: parts.length > 1 ? parseInt(parts[1]) : 0, seconds: parts.length > 2 ? parseInt(parts[2]) : 0, milliseconds: milliseconds})
			}
			else { Object.assign(info, { hours: timePart.substring(0, 2), minutes: timePart.length >= 2 ? timePart.substring(2, 4) : 0, seconds: timePart.length >= 4 ? timePart.substring(4, 6) : 0, milliseconds: timePart.length >= 6 ? timePart.substring(6) : 0}) }
		}
	}
	if (info) {
...
function dateTimeAsKisaString(value, secsFlag) {
...
function asDateTimeAndToKisaString(value, secsFlag) { return dateTimeAsKisaString(asDate(value), secsFlag) }
...
function gA(t) {
  ...
  return `${sifirlaDoldur(t.gun, 2)}.${kisaAy[t.ay - 1]}.${sifirlaDoldur(t.yil % 100, 2)}`
}
function isVirtualKeyboardSupported() {
	_isVirtualKeyboardSupported
	if (_isVirtualKeyboardSupported == null)
		_isVirtualKeyboardSupported = !!navigator.virtualKeyboard
	return result
}
...
function asSet(...anArray) {
	if (!anArray)
		return null
	if (anArray.length == 1 && anArray[0] == null)
		return null
	if (!isArray(anArray))
		return anArray

	return fromEntries(anArray.flat().map(_ => [_, true]))
}
function dictToKAListe(e = {}) {
	...
	if (isArray(dict))
		return dict
	let kodGetter = e.kodGetter ?? (_ => _)
	let adiGetter = e.adiGetter ?? (_ => _)
	return entries(dict).map(([k, a]) =>
		new CKodVeAdi({ kod: kodGetter(k), aciklama: adiGetter(a) }))
}
function asBool(value) {
	let falsy = (
		value == null || value == false || (isString(value) && !value) || value === 0 || value === "0" ||
		value == "H" || value === "h" || value == "N" || value === "n" ||
...
function sqlAliasmi(value) { return value && !isDigit(value[0]) && hepsiUygunmu(value, ch => sqlAliasmiCh(ch)) }
function sqlAliasmiCh(value) { return (value == '_') || isLetterOrDigit(value) }
...
function isGUID(value) { return isString(value) && value?.length == guidLength && Array.from(value).filter(ch => ch == '-').length == 4 }
...
function shuffle(coll) {
	if (!coll) { return coll }
	if (typeof coll == 'object' && !isArray(coll))
		coll = keys(coll)
	coll = [...coll]
	let result = []
	while (coll.length) {
		let index = asInteger(Math.random() * 1000000) % coll.length
		result.push(...coll.splice(index, 1))
	}
	return result
}
...
function getExpressions(e) {
	if (typeof e != 'object')
		e = { text: e }
	let text = e?.text?.toString()?.trim()
	if (!text || isArray(text))
		return text ? [...text] : []
	let getSubExps = regex => {
		let _result = [], parts
		while ((parts = regex.exec(text)) !== null) {
			if (parts[1])
				_result.push(parts[1].trim())
		}
		return _result
	}
...
function asReverseDict(value) {
	return value ? fromEntries(entries(value).map(([k, v]) => [v, k])) : null
}
...
function isBuffer(value) { return value?.constructor?.__proto__.name == 'TypedArray' }
function makeArray(value) { return $.makeArray(value) }
function arrayFrom(value) { return Array.from(value) }
function arrayFromAsync(value) { return Array.fromAsync(value) }
function arrayOf(value) { return Array.of(value) }
...
function os2HTMLColor(value) {
	return rgb2HTMLColor(osColor2RGB(value))
}
...
function html2RGB(value) {
	...
	if (!isString(value))
		return null
	return osColor2RGB(hexToInt(value))
}
function html2OSColor(value) {
	return rgb2OSColor(html2RGB(value))
}
function html2STOSColor(value) {
	return rgb2STOSColor(html2RGB(value))
}
...
function osColor2RGB(value) {
	...
	return ({
		r: ( (value & 0xff0000) >> 16 ),
		g: ( (value & 0x00ff00) >> 8 ) * 257,
		b: ( (value & 0x0000ff) ) * 257
	})
}
...
function stOSColor2RGB(value) {
	if (!isNumber(value))
		return value
	return ({
		r: ( value % 256 ),
		g: ( trunc(value / 256) % 256 ),
		b: ( trunc(value / 65536) % 256 )
	})
}
function rgb2STOSColor(rgb, _green, _blue) {
	if (rgb == null)
		return rgb 
	if (!isObject(rgb))
		rgb = { r: rgb, g: _green, b: _blue }
...
function stOS2HTMLColor(value) {
	return rgb2HTMLColor(stOSColor2RGB(value))
}
...
function arrayAggregate(getter, action, ...items) {
	let result = 0
	for (let _item of items.flat()) {
		let item = getter ? getter(_item) : _item
		if (item != null)
...
function numberAsLiraVeKurusText(e) {
	...
	return (
		`${numberAsText(tl, true)}TL` +
		(kr ? ` ${numberAsText(kr, true)}KR` : ``)
	)
}
function numberAsText(e, _noSpace) {
	let value = e == null || e.value == null ? e : e.value
	let noSpace = e?.noSpace ?? _noSpace
	if (!isNumber(value))
		return value || ''
	...
	return empty(result) ? 'Sıfır' : result.join('')
}
...
function parantezli(value) {
	...
	if (!/[()]/.test(value))
		return value
	/* !! başta parantez yoksa mutlaka sona da ekle.
		Yoksa, `SUM(alias.saha)` şeklinde veri geldiğinde yamulur */
	if (value[0] != '(')
		value = `(${value})`
	/*if (value[0] != '(')
		value = '(' + value
...
function virgulStr2Array(value) { return strDelim2Array(value, ',') }
function array2VirgulStr(value) { return array2StrDelim(value, ',') }
function noktaliVirgulStr2Array(value) { return strDelim2Array(value, ';') }
function array2NoktaliVirgulStr(value) { return array2StrDelim(value, ';') }
function pipeStr2Array(value) { return strDelim2Array(value, '|') }
function array2PipeStr(value) { return array2StrDelim(value, '|') }
...
function openFile(e) {
	...
	let promise = new $.Deferred(); try {
		let elm = $(`<input hidden ${coklumu ? 'multiple ' : ''}type="file"${captureHTML}${acceptHTML}>`).appendTo('body');
...
function delay(waitMS, returnValue) {
	return promise(resolve =>
		setTimeout(() => resolve(returnValue), waitMS))
}
function debounce(key, proc, delayMS, ...args) {
	if (!key)
		return proc(...args)
	
	let globStat = globalThis._debounceState ??= {}
	let st = globStat[key] ?? { key }
	let { timestamp: ts } = st
	if (ts && now() - ts >= delayMS) {
		delete globStat[key]
		return st.result
	}

	let result = proc?.(...args)
	extend(st, { timestamp: now(), result })
	globStat[key] = st

	return result
}
function deferExec(key, proc, delayMS, ...args) {
	let globStat = globalThis._deferExecState ??= {}
	let st = globStat[key] ?? { key }
	clearTimeout(st.timer)
	let timer = st.timer = setTimeout(async () => {
		try { await proc?.(...args) }
		finally { delete globStat[key] }
	}, delayMS)
	globStat[key] = st
	return timer
}
function waitUntil(proc, opts) {
	...
	let id = newGUID()
	let info = {
		promise: new defer(), steps: 0, aborted: false,
...
function isMiniDevice() { return $(window).width() <= 1400 }
function isMicroDevice() { return $(window).width() <= 800 }
...
function mixin(midCls, superCls) {
	let midClsDef = midCls.toString()
	midClsDef = midClsDef
		.replace('class $', 'class ')
		.replace(' extends CObject', ` extends ${superCls.name}`)
		.replace(' extends Object', ` extends ${superCls.name}`)
	return eval(`(${midClsDef})`)
	
    /*class Mix extends superCls {}
    // setProto(Mix, superCls)
    copyStaticMethods(Mix, midCls)
	copyInstanceMethods(Mix, midCls)
    return Mix*/
}
function setProto(dst, src) {
	Object.setPrototypeOf(dst.prototype, src.prototype)
	Object.setPrototypeOf(dst, src)
}
function copyStaticMethods(dst, src) {
...
function copyInstanceMethods(dst, src) {
    let {prototype: srcProto} = src
    let {prototype: dstProto} = dst
    for (let name of Object.getOwnPropertyNames(srcProto)) {
        if (name == 'constructor')
			continue
        let desc = Object.getOwnPropertyDescriptor(srcProto, name)
        Object.defineProperty(dstProto, name, desc)
    }
}
async function ctable(value) { return console.table(await value) }
async function clog(value) { return console.log(await value) }
async function cinfo(value) { return console.info(await value) }
async function cwarn(value) { return console.warn(await value) }
async function cdebug(value) { return console.debug(await value) }
function cerr(ex) {
	let errText = isObject(ex) ? getErrorText(ex) : undefined
	return console.error(errText, ex)
}

function remoteProc(e = {}, proc, args) {
	let saved = app.saved ??= { ws: config.ws, session: config.session, sql: app.wsSQL }
	let { class: cfgCls } = config
	let { ssl = true, url, host, port } = e
	/*let { host = cfgCls.DefaultWSHostName_SkyServer } = e
	let { port = ssl ? cfgCls.DefaultSSLWSPort : cfgCls.DefaultWSPort } = e*/
	url ||= ( host || port ? `http${ssl ? 's' : ''}://${host}:${port}` : undefined ) ?? saved.ws.url
	let ws = url ? { url } : undefined
	
	let { sql = e.wsSQL, server, db = e.dbName, sqlUser, sqlPass, session } = e
	if (server || db || sqlUser || sqlPass) {
		sql ??= {}
		if (server)
			sql.server = server
		if (db)
	...
	if (!session) {
		let cnv = { user: 'wsUser', pass: 'wsPass' }
		session = new Session()
		;['loginTipi', 'user', 'pass'].forEach(k =>
			session[k] = e[cnv[k]] ?? e[k] ?? saved.session?.[k])
	}
	if (isPlainObject(session))
		session = new Session(session)
	if (session) {
		session.loginTipi ??= saved.session?.loginTipi
		session.user ??= saved.session?.user
		session.pass ??= saved.session?.pass
	}

	let current = { ws, session, sql }
	if (ws)
		config.ws = ws
	if (session)
	...
	proc ??= e.proc ?? e.block
	args ??= e.args
	try { return proc?.({ ...e, remote: true, saved, current, args }) }
	finally {
		config.ws = saved.ws
		config.session = saved.session
		app.wsSQL = saved.sql
	}
}

function promise(procOrResult) {
	let isFunc = isFunction(procOrResult)
	let proc = isFunc ? procOrResult : undefined
	let result = isFunc ? undefined : procOrResult
	
	if (isFunc && !proc.length)
		return (async () => proc())()
...
function defer(procOrResult) {
	let isFunc = isFunction(procOrResult)
	let proc = isFunc ? procOrResult : undefined
	let result = isFunc ? undefined : procOrResult

	if (isFunc && !proc.length) {
		return new $.Deferred(async d => {
			try { d.resolve(await(proc())) }
			catch (ex) { d.reject(ex) }
		})
	}
	
	let promise = new $.Deferred(proc)
	if (result !== undefined)
		promise.resolve(result)
	return promise
}
function promiseAll(...promises) { return Promise.all(promises.flat()) }
function promiseAllSet(...promises) { return Promise.allSettled(promises.flat()) }
function promiseAny(...promises) { return Promise.any(promises.flat()) }
function promiseRace(...promises) { return Promise.race(promises.flat()) }

function getPdf_blob(content, args = {}) { return getPdfOutput('blob', content, args) }
function getPdf_buffer(content, args = {}) { return getPdfOutput('arraybuffer', content, args) }
function getPdf_string(content, args = {}) { return getPdfOutput(null, content, args) }
function getPdf_url(content, args = {}) { return getPdfOutput('bloburl', content, args) }
function getPdf_blobUrl(content, args = {}) { return getPdf_url(content, args) }
function getPdfOutput(type, content, args = {}) {
	return getPdf(content, args)
		.output(type)
}
function getPdf(content, args = {}) {
	let e = {
		margin: 5,
		// filename: 'rapor.pdf',
	...
	;{
		...
		;['scale'].forEach(k => {
			let v = e[k]
			if (v != null) {
				e.html2canvas[k.toLowerCase()] = v
				delete e[k]
			}
		})
		;['unit', 'format', 'orientation'].forEach(k => {
			let v = e[k]
			if (v != null) {
				e.jsPDF[k.toLowerCase()] = v
				delete e[k]
			}
		})
	}
...
function docFrg() { return $(_docFrg()) }
function _docFrg() { return document.createDocumentFragment() }

function getLeaf(rec) {
	while (rec?.detaylar?.length)
		rec = rec.detaylar[0]
	return rec
}
function walkToLeafUntil(rec, cond) {
	if (!rec)
		return null
	
	cond = getFunc(cond)   // string eval or function
	if (!cond)
		return null
...
function walkTreeUntil(rec, cond) {
	if (!rec)
		return null
	...
	let { detaylar } = rec
	for (let child of detaylar ?? []) {
		let res = walkTreeUntil(child, cond)
		if (res)
			return res
	}
...
function getAdiSoyadi(v) {
	v = v?.trim()
	if (!v)
		return v

	const Delim = ' '
	let tokens = v.split(Delim).filter(Boolean)
	return {
		adi: tokens.slice(0, -1).join(Delim),
		soyadi: tokens.at(-1)
	}
}
function normalizePostaKod(posta, ilKod, ihracatmi) {
	return MQCariIl?.normalizePostaKod?.(posta, ilKod, ihracatmi)
}
function getPostaKodu(ilKod, ihracatmi) {
	return MQCariIl?.getPostaKodu?.(ilKod, ihracatmi)
}

function getMergedText(prefix, arr, postfix) {
	return (empty(arr)) ? '' : [
		...
		...arr
			...
			.map(v => `<li>${v}</li>`),
		`</ul>`,
		( postfix ? `<p>${postfix}</p>` : null ),
	].filter(Boolean).join('\n')
}

function mergedToTokens(v, maxLen) {
    return v ? uygunKelimeliParcalaDogrudan(v, maxLen) : v
}
function tokensToMerged(arr, delim) {
	delim = ' '
    return arr?.filter(Boolean)?.join(delim)
}
...
function shell_lines(cmd) {
	return app.shell_lines(cmd)
}
function shell_firstLine(cmd) {
	return app.shell_firstLine(cmd)
}

function gridKolon(...args) {
	return GridKolon.from(...args)
}

async function* enumDirs(e = {}) {
	if (e.kind !== undefined)
		e = { dh: e }
	
	let { dh = e.fh, recursive, includeDirs, parent, withFile, mask } = e
	if (!(dh = await dh))
		return
	...
	withFile ??= false
	parent = (parent ?? '').trim('', '/')
	for await (let fh of dh.values()) {
		let { kind, name: n } = fh		
		let relPath = parent ? `${parent}/${n}` : n
		let dir = kind == 'directory'
		if (dir) {
			if (recursive) {
				let args = { ...e, dh: fh, parent: relPath }
				delete args.fh
				for await (let res of enumDirs(args))
					yield res
			}
		}
		...
		if (mask && !matchMask(relPath, mask))
			continue
		
		let file = withFile && !dir ? await fh.getFile() : null
		if (dir)
			n += '/'
		
		let { size: s, type: t, lastModifiedDate: m } = file ?? {}
		let ext = n.split('.').at(-1)?.trimEnd() ?? ''
		yield { dir, relPath, parent, fh, file, n, ext, t, s, m }
	}
}
function enumDirsRecur(e = {}) {
	if (e.kind !== undefined)
		e = { dh: e }
	return enumDirs({ ...e, recursive: true, withFile: false })
}
function enumDirsWFile(e = {}) {
	if (e.kind !== undefined)
		e = { dh: e }
	return enumDirs({ ...e, recursive: false, withFile: true })
}
function enumDirsRecurWFile(e = {}) {
	if (e.kind !== undefined)
		e = { dh: e }
	return enumDirs({ ...e, recursive: true, withFile: true })
}
...
(function() {
	...
	let { virtualKeyboard: kb } = navigator
	if (kb)
		kb.overlaysContent = true
