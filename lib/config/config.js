class Config extends CObject {
	...
	constructor(e = {}) {
		...
		let ws = this.ws = inverseCoalesce(e.ws, value => JSON.parse(value)) || {
			...
			url: e.url || e.wsURL || qs.url || qs.wsURL || undefined,
			ssl: asBoolQ(e.ssl ?? qs.ssl),
			proxyServerURL: (
	...
	applyColorScheme(e) {
		...
		colorScheme = colorScheme?.char ?? colorScheme
