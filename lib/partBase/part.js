class Part extends LayoutBase {
	...
	destroyPart(e) {
		...
		super.destroyPart(e); setTimeout(() => {
			const sender = this, _e = { ...e, sender };
			this.triggerKapanincaEvent(_e); if (injectResult?.kapaninca) { injectResult.kapaninca(_e) }
		}, 10);
		const subParts = this._subParts; if (subParts) {
			const parentPart = this.parentPart ?? this.sender;
			for (const key in subParts) {
				const subPart = subParts[key]; if (subParts == parentPart || subParts == parentPart?.parentPart) { continue }
