class Part extends LayoutBase {
	...
	get canDestroy() { return this._canDestroy ?? this.class.canDestroy }
	set canDestroy(v) { this._canDestroy = v }
	static get isCloseable() { return true }
	get isCloseable() { return this._closeable ?? this.class.isCloseable }
	set isCloseable(v) { this._closeable = v }
	get defaultLayoutSelector() { return `#${this.partName}` } get formDeferMS() { return 50 }
	static get wndClassNames() { return [this.partName, 'part'] } get wndClassNames() { return this.class.wndClassNames }
	...
	init(e) {
		super.init(e)
		if (!this.isDestroyed && !this.isSubPart)
			app._activePartStack.push(this)
		let { isWindowPart, partName, content, layout } = this
		if (isWindowPart) {
			if (!layout?.length || layout == content) {
				let wndContent = $(`div#${partName}.part`)
				if (!wndContent?.length) {
					wndContent = $(`template#${partName}.part`)
					if (wndContent?.length)
						wndContent = wndContent.contents('div').clone(true)
				}
				layout = wndContent
			}
			...
			if (!layout?.length)t
				this.layout = layout
			if (layout?.length)
				layout.addClass('wnd-content dock-bottom')
		}
		let parentPart = this.parentPart || this.sender
		if (parentPart) {
			let _subParts = parentPart._subParts ??= {}
			_subParts[partName || newGUID()] = this
		}
		if (layout?.length)
			layout.addClass('basic-hidden')
		let { injectResult } = app
		injectResult?.partInit?.({ sender: this, ...e })
	}
	run(e) {
	...
	afterRun(e) {
		...
		super.afterRun(e); let {layout, isWindowPart} = this
		if (isWindowPart) {
			if (!layout?.length || !layout.hasClass('wnd-content'))
				layout = this.layout = this.wndContent
			let {wndPart, wnd} = this
			if (!wndPart) {
				let {title, wndArgs, builder, parentPart} = this
				wndPart = this.wndPart = createTabbedWindow({ layout, title, widgetArgs: wndArgs, builder, asilPart: this, parentPart })
				wnd = this.wnd = wndPart.layout; this.wndOnOpen(e)
				wndPart.kapaninca?.(e => this.wndOnClose(e))
			}
			if (wnd) {
				let wndCSSClass = (this.wndClassNames || []).join(' ')
				wnd.addClass(wndCSSClass); layout.addClass(wndCSSClass)
				/* if (!this.noFullHeightFlag) layout.css('height', `calc(var(--full) - (var(--appTitle-bottom) + var(--tabs-header-bottom) + 5px))`) */
				let parentPart = this.parentPart || this.sender
				parentPart?.kapaninca?.(e => this.close())
			}
		}
		...
		if (layout?.length)
			layout.removeClass('basic-hidden')
		;{
			let handler = this._activatedHandler = evt =>
				this.activated({ event: evt })
			window.addEventListener('focus', handler)
		}
		;{
			let handler = this._deactivatedHandler = evt =>
				this.deactivated({ event: evt })
			window.addEventListener('blur', handler)
		}
		if (injectResult?.partAfterRun) { injectResult.partAfterRun({ sender: this, ...e }) }
	}
	updateWndTitle(e) {
		let title = this._lastTitle = e?.title ?? e ?? ''
		let {wndPart} = this, {layout: wndLayout} = wndPart ?? {}
		if (wndLayout?.length) {
			let {id} = wndLayout[0], wndParent = wndLayout.parents('.skyTabs.part')
			let elmTitle = wndParent.find(`.tabs > #${id}.tabPage > .header`)
			elmTitle.html(title)
		}
		return this
	}
	show(e) {
		let {wndPart, wnd, formDeferMS} = this
		let {injectResult, _initTitle, _initTitle: title} = app || {}
		if (wndPart && wnd) {
		...
		super.show(e)
		if (!$('body').hasClass('no-wnd')) {
			title = this._lastTitle ?? this.title
			if (title) {
				if (title.includes('<'))
					title = `${_initTitle}: ${$(`<div>${title}</div>`).text()}`
				$('head').children('title').html(title)
			}
		}	
		if (injectResult?.partShow)
			injectResult.partShow({ sender: this, ...e })
	}
	hideBasic(e) {
		super.hideBasic(e)
		let {wndPart, wnd} = this
		let {injectResult, _initTitle: title} = app || {}
		if (wndPart && wnd)
			wnd.css('opacity', .01)
		if (injectResult?.partHideBasic)
			injectResult.partHideBasic({ sender: this, ...e })
	}
	hide(e) {
		super.hide(e)
		let {wndPart} = this
		...
		if (wndPart)
			wndPart.hide()
		if (injectResult?.hide)
			injectResult.hide({ sender: this, ...e })
	}
	...
	activated(e) {
		...
		this.triggerActivatedEvent({ ...e, sender: this })
		injectResult?.activated?.(_e)
		clearTimeout(this._timer_scrollTop)
		this._timer_scrollTop = setTimeout(() => {
			let { activeWndPart } = app ?? {}, { body } = document
			if (activeWndPart && body.scrollTop)
				body.scrollTop = 0
			delete this._timer_scrollTop
		}, 100)
	}
	deactivated(e) {
		let {injectResult, _initTitle: title} = app ?? {}
		setTimeout(() => {
			let sender = this
			let _e = { ...e, sender } /*if (this.hasBGModal) { $('body').addClass('bg-modal') }*/
			this.triggerDeactivatedEvent({ ...e, sender: this })
			if (title && $('body').hasClass('no-wnd'))
				$('head').children('title').html(title)
			injectResult?.deactivated?.(_e)
		}, 1)
	}
	destroyPart(e) {
	...
	clearEvents(eventName, handler) {
		let {events} = this
		delete events[eventName]
		return this
	}
	async raiseEvent(eventName, ...args) {
		let { events, asilPart } = this
		let handlers = events?.[eventName]
		let ekHandlers = events?.[`${eventName}Ek`]
		if (!empty(ekHandlers))
			(handlers ??= []).push(...ekHandlers)
		if (handlers) {
			for (let i in handlers)
				await handlers[i]?.call?.(this, ...args)
		}
		let eventSelector = `event_${eventName}`
		await this[eventSelector] ?? window[eventSelector]?.call?.(this, func, ...args)
		let {injectResult} = app || {}
		if (injectResult?.raiseEvent) {
			let _e = { sender: this, eventName, args }
			await injectResult.raiseEvent(_e)
		}
		return this
	}
