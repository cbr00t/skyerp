...
- Run locally with PHP’s built-in server (serves the whole www root so /skyerp routes work):
  $env:PORT=81; php -S localhost:$env:PORT -t F:\AppServ\www
  Then open http://localhost:81/skyerp/app/index.php
...
- Access a module directly via browser, e.g., http://localhost:81/skyerp/app/<module>/index.php when using the built-in server setup above.
