class ProgressManager extends CObject {
	static{window[this.name] = this;
	this._key2Class[this.name] = this
	} get wnd() {
	...
	get layouts() {
		...
		if (!result) {
			...
			result = this._layouts = {
				...
				get content() {
					const {wnd} = this;
					return wnd?.length ? wnd.find('div > .content') : null
				},
				get layout() {
					const {content} = this;
					return content?.length ? content.find('.subContent') : null
				},
				get subLayout() {
					const {layout} = this;
					return layout?.length ? layout.find('div') : null
				},
				get buttons() {
					const {content} = this;
					return content?.length ? content.children('.buttons') : null
				},
				get abortButton() {
					const {buttons} = this;
					return buttons?.length ? buttons.children('button:eq(0)') : null
				},
				get img() {
	...
	get text() {
		const {layouts} = this
		  , elm = layouts.text;
		return elm?.length ? elm.html() : null
	}
	set text(value) {
		const {layouts} = this
		  , elm = layouts.text;
	...
	get imgSrc() {
		const {layouts} = this
		  , elm = layouts.img;
		return elm?.length ? elm.attr('src') : null
	}
	set imgSrc(value) {
		const {layouts} = this
		  , elm = layouts.img;
	...
	get imgVisible() {
		const {layouts} = this
		  , elm = layouts.img;
		return elm?.length ? !(elm.hasClass('jqx-hidden') || elm.hasClass('basic-hidden')) : false
	}
	...
	get progressMax() {
		const {layouts} = this
		  , elm = layouts.progress;
		return elm?.length ? inverseCoalesce(elm.attr('max'), value => asFloat(value)) : null
	}
	...
	get progressValue() {
		const {layouts} = this
		  , elm = layouts.progress;
		return elm?.length ? inverseCoalesce(elm.attr('value'), value => asFloat(value)) : null
	}
	...
	get abortBlock() {
		return this._abortBlock
	}
	set abortBlock(value) {
		return this._abortBlock = value
	}
	...
	constructor(e) {
		...
		$.extend(this, {
			_wnd: e.wnd,
			ekBilgiHandler: e.ekBilgiHandler,
			abortBlock: e.abortBlock ?? e.onAbort
		})
	}
	destroyPart(e) {
		this._wnd = this._layouts = this._ekBilgiHandler = this._abortBlock = null
	}
	progressStep(value) {
		const {layouts} = this
		  , elm = layouts.progress;
	...
	setEkBilgiText(value) {
		this.ekBilgiText = value;
		return this
	}
	setEkBilgiVisible(value) {
		this.isEkBilgiVisible = value;
		return this
	}
	setEkBilgHandler(value) {
		this.ekBilgHandler = value;
		return this
	}
	setAbortText(value) {
		this.abortText = value;
		return this
	}
	setAbortButtonVisible(value) {
		this.isAbortButtonVisible = value;
		return this
	}
	setAbortBlock(value) {
		this.abortBlock = value;
		return this
	}
}
