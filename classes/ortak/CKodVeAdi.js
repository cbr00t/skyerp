class CKod extends CObject {
	...
	constructor(e) {
		...
		this.kod = ($.isArray(e)) ? e[0] : (e.kod === undefined ? e.id : e.kod)
	}
...
class CKodVeAdi extends CKod {
    static { window[this.name] = this; this._key2Class[this.name] = this }
	static get kodSaha() { return 'kod' }
	static get adiSaha() { return 'aciklama' }
...
class CKodAdiVeEkBilgi extends CKodVeAdi {
	...
	constructor(e) {
		...
		if ($.isArray(e)) {
			...
			if (e.ekBilgi !== undefined)
				this.ekBilgi = e.ekBilgi
		}
	}
}
...
class CKodAdiVeMadde extends CKodAdiVeEkBilgi {
	...
	constructor(e) {
		...
		if (!$.isArray(e)) {
			if (e.madde !== undefined)
				this.madde = e.madde;
		}
	}

	get madde() { return this.ekBilgi }
	set madde(value) { return this.ekBilgi = value }
}
...
class CKodAdiVeOran extends CKodAdiVeEkBilgi {
	...
	get oran() { return this.ekBilgi }
	set oran(value) { return this.ekBilgi = value }
}
...
class PayVeBaz extends CKodVeAdi {
	...
	constructor(e) {
		...
		if (!this.aciklama) {
			const {oran} = this;
			if (oran && oran.baz != null)
				this.aciklama = `${asInteger(oran.pay)}/${asInteger(oran.baz)}`;
		}
	}
}
