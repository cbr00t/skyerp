class TekSecim extends CIO {
	...
	set kaListe(value) { const savedValue = this._kaListe; this._kaListe = value; if (!(this.kaDict && value == savedValue)) { this.kaDictOlustur() } }
	get secilen() {
	...
	kaDictOlustur(e) {
		...
		for (const ka of kaListe) { const {question} = ka; kaDict[ka.kod] = ka; if (question && question.length > 2) { questionDict[question.substr(0, question.length - 2)] = ka } }
	}
