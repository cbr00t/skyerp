class MQStm extends MQDbCommand {
	constructor(e) {
		...
		$.extend(this, {
			with: ($.isPlainObject(e.with)
							? new MQWith(e.with) : e.with)
						|| new MQWith(),
	...
	fromGridWSArgs(e) {
		...
		if (pageNum != null && pageSize && !asBool(e.rowCountOnly)) {
			...
			let limit /*= this.limit*/ = offset == null ? null : (pageSize || 0);
			if (limit != null)
				limit += 5
			const top = limit == null ? null : ((offset || 0) + limit);
			// const top = ((offset || 0) + (limit || 0));
			// this.limit = limit;
...
class MQTmpTable extends MQDbCommand {
	constructor(e) {
		...
		this.sahalar = [];
		const _liste = e.sahalar || e.liste;
	...
	addAll(coll) {
		const _liste = coll ? (coll.sahalar || coll.liste) : null;
		if (_liste)
			coll = _liste;
	...
	buildString(e) {
		...
		const sahalarClause = this.sahalar.map(item => item.toString()).join(', ');
