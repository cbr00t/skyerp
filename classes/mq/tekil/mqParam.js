...
class MQParam extends MQTekil {
	...
	constructor(e) {
		e = e || {}; super(e);
		if (e.isCopy) return; $.extend(this, { _promise: new $.Deferred(), kullanim: e.kullanim || {}, });
		this.paramciInit(e)
	}
	...
	static formBuilderDuzenle(e) {
		...
		if (builder.isRootFormBuilder) {
			...
			let tabPage_genel = e.tabPage_genel = tabPanel.addTab('genel', 'Genel'), form = tabPage_genel.addForm();
			e.builder = builder = form
		}
		if (paramci) {
			for (const key of ['_id2Item', 'builder', '_root', '_parent', '_inst', '_altInst']) delete paramci[key]
			$.extend(paramci, { inst: e.inst, part: e.part, layout: e.layout });
	...
	async kaydetOncesiIslemler(e) {
		const {paramci} = this;
		if (paramci) { for (const item of paramci.getItemsAndSelf()) { if (item.kaydetOncesi) await item.kaydetOncesi(e) } }
		super.kaydetOncesiIslemler(e)
	}
	async kaydetSonrasiIslemler(e) {
		const {paramci} = this;
		if (paramci) { for (const item of paramci.getItemsAndSelf()) { if (item.kaydedince) await item.kaydedince(e) } }
		await super.kaydetSonrasiIslemler(e)
	}
	static varsayilanKeyHostVarsDuzenle(e) {
		e = e || {}; super.varsayilanKeyHostVarsDuzenle(e);
		const {hv} = e; hv.kod = this.paramKod;
		const {paramci} = this;
	...
	hostVarsDuzenle(e) {
		...
		const result = this.paramHostVars(e); if (!result) return
	...
	setValues(e) {
		...
		this.paramSetValues($.extend({}, e, { rec: result }));
	}
	...
	paramSetValues(e) {
		...
		const {tekSecimDonusum} = this.class, {tekSecimDonusum_receiver} = this;
		for (const key in tekSecimDonusum) {
	...
	static async tanimla(e) {
		...
		try {
			const {paramci} = this;
			if (paramci?.tanimUIArgsDuzenle) paramci.tanimUIArgsDuzenle(e)
			/*let kaydedinceHandler = e.kaydedince;
			e.kaydedince = e => { if (kaydedinceHandler) getFuncValue.call(this, kaydedinceHandler, e); if (paramci?.kaydedince) paramci.kaydedince(e) }*/
