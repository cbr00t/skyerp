class MQYerelParamConfigTanimPart extends MQYerelParamBaseTanimPart {
	...
	constructor(e) {
		e = e || {};
		super(e);
		this.title = e.title || 'Yerel Parametreler (WS)'
	}
	runDevam(e) {
		super.runDevam(e);
		const {layout, inst} = this;
		const txtWSUrl = this.txtWSUrl = layout.find('#wsURL .veri');
		...
		const txtWSProxyServerURL = this.txtWSProxyURL = layout.find('#wsProxyServerURL .veri');
		...
		txtWSProxyServerURL.on('change', evt => {
			...
			value = value.trim();
			elm.value = value;
			inst.wsProxyServerURL = (value || '')
		});
		const txtSQL_server = this.txtSQL_server = layout.find('#sql-server .veri');
		txtSQL_server.val((inst.sql || {}).server || '');
		txtSQL_server.on('change', evt => {
			const elm = evt.currentTarget;
			let value = (elm.value || '').trim();
			elm.value = value;
			const sql = inst.sql = inst.sql || {};
			sql.server = value
			
		});
		const txtSQL_db = this.txtSQL_db = layout.find('#sql-db .veri');
		...
		const txtUzakScriptURL = this.txtUzakScriptURL = layout.find('#uzakScriptURL .veri');
		...
		txtUzakScriptIntervalSecs.on('change', evt => {
			const elm = evt.currentTarget;
			let value = (typeof elm.value == 'string' ? asFloat((elm.value || '').trim()) : asFloat(elm.value)) || null;
			elm.value = value;
	...
	afterRun(e) {
		super.afterRun(e);
		setTimeout(() => { makeScrollable(this.layout); this.txtWSUrl.focus() }, 100)
	}
	/*wndArgsDuzenle(e) {
