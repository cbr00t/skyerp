	...
	static async raporKategorileriDuzenle_detaylar_hmr(e) {
		...
		for (const item of HMRBilgi.hmrIter()) {
			...
			for (const rSaha of rSahalar) {
				const sqlDict = rSaha.sql, def = MQSQLOrtak.sqlServerDegeri(item.numerikmi ? 0 : '');
				for (let i = 2; i <= shdDisi; i++) { sqlDict[i] = def } rSaha.sql = sqlDict;
				if (!rSaha.attr.toLowerCase().endsWith('adi')) { sqlDict[1] = `har.${item.rowAttr}` }
				kat.addDetay(rSaha)
			}
		}
	}
