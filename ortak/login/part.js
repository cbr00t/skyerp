class LoginPart extends Part {
	...
	runDevam(e) {
		...
		const ddLoginTipi = layout.find(`#loginTipi`);
		ddLoginTipi.jqxDropDownList({
			theme: theme, width: '100%', height: 55, itemHeight: 50, dropDownHeight: 200,
			source: app.loginTipleri, valueMember: 'kod', displayMember: 'aciklama',
			renderer: (index, aciklama, kod) => {
				const {partName} = this.class;
				const layoutName = ddLoginTipi.prop('id');
				const result = (
					`<div class="${partName} ${layoutName} list-item flex-row">` +
		...
		const txtUser = layout.find('#user');
		txtUser.off('blur').on('blur', evt => this.loginUserDegisti({ event: evt }));
		const txtPass = layout.find('#pass');
		const btnSSOLogin = this.btnSSOLogin = layout.find('#ssoLogin').jqxButton({ theme: theme, width: false, height: false });
		btnSSOLogin.off('click').on('click', evt => this.loginIstendi({ event: evt, isSSO: true }));
		const btnLogin = layout.find('#islemTuslari #login');
		btnLogin.jqxButton({ theme: theme, width: '100%', height: 60 });
		...
		if (user) { this.loginIstendi(); txtPass.focus() }
		else { this.show(); txtUser.focus() }
	...
	show(e) {
		if (window.boot) window.boot.end()
		const {rootParent, btnLogout} = app;
		for (const elm of [rootParent, btnLogout]) {
			if (elm && elm.length) elm.addClass('jqx-hidden')
		}
		return super.show(e)
	}
	hide(e) {
		const {rootParent, btnLogout} = app;
		for (const elm of [rootParent, btnLogout]) {
			if (elm && elm.length) elm.removeClass('jqx-hidden')
		}
		return super.hide(e)
	}
	...
	async loginUserDegisti(e) {
		...
		if (session?.sessionID) {
			...
			divUserText.html(`[<b>${session.user || ''}</b>] ${session.userDesc || ''}`);
			userTextParent.removeClass('jqx-hidden basic-hidden');
			if (session.isSSO || session.hasSSO)
	...
	loginIstendi(e) {
		...
		const {layout} = this, _session = new Session();
		...
		return app.wsLogin({ isSSO: e.isSSO, session: _session })
			.then(result => {
				...
				createJQXWindow({
					title: 'Oturum Açılamadı', content: errorText,
					args: { isModal: true, width: 600, height: 300 },
					buttons: { TAMAM: e => { e.close(); setTimeout(() => layout.find(`#user`).focus(), 100) } }
				})
			})
	}
	vazgecIstendi(e) {
		const {promise_uiResult} = this; if (promise_uiResult) promise_uiResult.reject({ sessionInfo });
		this.hide()
	}
