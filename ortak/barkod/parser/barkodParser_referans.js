class BarkodParser_Referans extends BarkodParser {
	...
	static get uygunEkOzellikTipSet() {
		let tip2EkOzellik = app.tip2EkOzellik || {}, desteklenenEkOzellikTipSet = this.desteklenenEkOzellikTipSet || {}, result = {};
		for (let [tip, value] of Object.entries(desteklenenEkOzellikTipSet)) {
			if (value) { result[tip] = true } }
		return result
	}
	async parseDevam(e) {
		e = e || {}; let result = await super.parseDevam(e); if (result) { return result }
	// barkod referans
		let {uygunEkOzellikTipSet} = this.class, {barkod} = this, paketKodClause = `coalesce(pak.kod, '')`;
		let paketIcAdetClause = `(case when ref.bkolibarkodmu <> 0 or ${paketKodClause} <> '' then coalesce(upak.urunmiktari, ref.koliicadet, 0) else NULL end)`;
		let sent = new MQSent({
			from: 'sbarref ref',
			fromIliskiler: [
