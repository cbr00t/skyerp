class NumaratorPart extends Part {
	...
	constructor(e = {}) {
		super(e); let {islem, fis} = e
		$.extend(this, { islem, fis })
	}
	runDevam(e) {
		super.runDevam(e); let {layout, fis, numarator} = this, seriNoForm = layout.find('.seriVeNo');
		let btnListedenSec = this.btnListedenSec = seriNoForm.find('#listedenSec').jqxButton({ theme });
		btnListedenSec.on('click', evt => this.listedenSecIstendi({ event: evt }));
		let txtSeri = this.txtSeri = seriNoForm.find('#seri');
		txtSeri.on('change', evt => { let elm = evt.currentTarget; elm.value = fis.seri = elm.value?.toUpperCase() || '' });
		/* if (!numarator?.tip) { txtSeri.removeClass('readOnly'); txtSeri.removeAttr('readonly') } */
		let txtFisNo = this.txtFisNo = seriNoForm.find('#fisNo'); this.fisNo_orjPlaceHolder = txtFisNo.attr('placeholder');
		...
		if (numarator) {
			if (otoNummu && yeniVeyaKopyami) { this.seriVeSonNoBelirle(e) }
				...
				let uiGoster = () => {
					let fisNo = asInteger(fis.no ?? fis.fisNo) || null, noYil = asInteger(fis.noYil) || 0;
					$.extend(numarator, { seri: fis.seri || '', noYil: fis.noYil || 0, sonNo: fisNo });
					let {txtNoYil} = this; if (txtNoYil?.length) { txtNoYil.val(noYil || null) }
	...
	async seriVeSonNoBelirle(e) {
		e = e || {}; let locals = app.getLocals('sonDegerler'), numKod2Seri = locals.numKod2Seri = locals.numKod2Seri || {};
		let {numarator} = this, numKod = numarator.kod, seri = numarator.seri = numKod2Seri[numKod] || '';
		if (!await numarator.yukle()) {
			numarator.seri = '';
			if (!await numarator.yukle()) { $.extend(numarator, { sonNo: 0 }); await numarator.yaz() }
		}
		let _e = $.extend({}, e, { seri: numarator.seri, noYil: numarator.noYil, sonNo: numarator.sonNo + 1 }); this.otoNumGoster(_e)
	}
	numaratorSecildi(e) {
		let rec = e.rec || (e.recs || [])[0]; if (!rec) { return }
		let {fis, numarator, yeniVeyaKopyami} = this, seri = rec.seri || '', noYil = rec.noyil || 0, sonNo = rec.sonno || 0;
		...
		let _e = $.extend({}, e, { noYil: rec.noyil, sonNo: yeniVeyaKopyami ? sonNo : null }); this.otoNumGoster(_e);
		this.txtFisNo.focus()
	}
	otoNumGoster(e) {
		let {seri, noYil, sonNo} = e, {fis, yeniVeyaKopyami, txtNoYil, txtFisNo} = this;
	...
	listedenSecIstendi(e) {
		let {numarator} = this;
		let result = numarator.class.listeEkraniAc({
			tekilmi: false, wndArgsDuzenle: e => {
				let {fis} = this, {sinifAdi} = fis.class, {numYapi} = fis, {wndArgs} = e;
				$.extend(wndArgs,{
