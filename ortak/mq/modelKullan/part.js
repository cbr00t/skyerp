class ModelKullanPart extends Part {
	...
	static get partName() { return 'modelKullan' }
	get jqxSelector() { return this.isDropDown ? 'jqxDropDownList' : 'jqxComboBox' }
	get mfSinif() {
		let value = this._mfSinif
		return value && !value.prototype && (isFunction(value) || value.run)
			? getFuncValue.call(this, this._mfSinif, { sender: this.sender, builder: this.builder, colDef: this })
			: value
	}
	...
	set value(value) {
		let { value: oldValue } = this
		if (value != null && value != oldValue) {
			this.kod = value
			this.input.val(value)
			this.onChange({ type: 'value', args: { value } })
		}
	}
	...
	runDevam(e) {
		...
		if (isDropDown) {
			extend(args, {
				...
				checkboxes: !!coklumu, groups: true,
			...
			extend(args, {
				autoComplete: true, remoteAutoComplete: true, remoteAutoCompleteDelay: 100,
				minLength: 0, multiSelect: !!coklumu,
				search: searchText => {
		...
		let _e = { parentPart, sender: this, builder: this.builder, isDropDown, da, args }
		let { argsDuzenleBlock, jqxSelector } = this
	...
	dataBind() {
		...
		this._inDataBind = true
		let timerKey = '_timer_dataBind'
		clearTimeout(this[timerKey])
		this[timerKey] = setTimeout(() => {
			try { this._inDataBind = false }
			finally { delete this[timerKey] }
		}, 50)
		
		let { widget } = this, source = widget?.source ?? this.source
		if (this.isDestroyed || source == null)
			return
		...
		if (source.dataBind)
			...
			(async () => {
				...
				let newSource = await getFuncValue.call(this, source, {
					sender: this, builder, parentPart, gridPart, gridWidget,
					inst, fis: inst, editCell, editor: editCell?.editor,
					rowIndex: editCell?.row ?? gridWidget?.selectedrowindex,
					dataField: editCell?.datafield,
					get gridRec() {
						let result = this._gridRec
						if (result === undefined) {
							let { gridWidget, rowIndex } = this
							result = this._gridRec = gridWidget?.getrowdata(rowIndex)
						}
						return result
					}
				});
	...
	clear(e) {
		...
		if (coklumu)
			widget[isDropDown ? 'uncheckAll' : 'clearSelection']()
		this.value = null
		return this
	}
	...
	async onChange(e = {}) {
		...
		let { force } = e
		let { args } = e.args ?? (evt ?? {})
		let type = (args?.type ?? e.type ?? evt?.type ?? '').toLowerCase()
		if (!(force || type == 'mouse') && (this.inEvent || this.disableEventsFlag)) {
			if (evt) evt.preventDefault()
			return
		}
		if (!type || (type == 'none' || type == 'keyboard'))
			return
		if (!(force || this.kodAtandimi) && !!this.mfSinif)
			return
		let timeStamp = evt?.timeStamp
		if (timeStamp != null) {
			let { _lastChangeEventTime } = this
			this._lastChangeEventTime = timeStamp
			if (_lastChangeEventTime && (timeStamp - _lastChangeEventTime) < 50)
				return
		}
		...
		try {
			...
			if (isDropDown) {
				...
				if (wItem == null && kod != null) {
					if (coklumu) {
						wItem = []
						let kodSet = isArray(_kod) ? asSet(_kod) : _kod
						for (let _wItem of widget.getItems()) {
							if (kodSet[_wItem.value])
								wItem.push(_wItem)
						}
					}
					else
						wItem = widget.getItem(_kod)
				}
				if (wItem != null) {
					if (coklumu) {
						item = wItem.map(x => x.originalItem ?? x)
						kod = wItem.map(x => x.value ?? x)
					}
					else {
						item = wItem.originalItem ?? wItem
						kod = wItem.value
					}
				}
	...
	veriYuklendi(e) {
		setTimeout(() => {
			let { input, parentPart, widget, veriYukleninceBlock, coklumu, isDropDown } = this
			if (this.isDestroyed || !input?.length)
				return
			
			if (!this.veriYuklendiFlag) {
				this.veriYuklendiFlag = true
				let { value } = this
				if (value != null && !this.kodAtandimi)
					input.attr('data-value', value ?? null)
				if (value) {
					value = makeArray(value)
					if (value?.length) {
						let valueSet = asSet(value)
						let wItems = widget.getItems()
						widget[isDropDown ? 'uncheckAll' : 'clearSelection']()
						this.disableEventsFlag = true;
						for (let i = 0; i < wItems.length; i++) {
							let r = wItems[i]
							let { index: ind, value: v } = r
							if (valueSet[v]) {
								widget.selectIndex(ind ?? i)
								break
							}
						}
			...
			if (veriYukleninceBlock) {
				let _e = extend({}, e, {
					parentPart, sender: this, builder: this.builder,
					get wItems() { return widget.getItems() },
					get source() { return widget.source },
					get recs() { return widget.source?.records }
				})
				veriYukleninceBlock.call(this,  _e)
			}
		}, 100)
	}
	...
	getDataAdapter(e = {}) {
		let {mfSinif} = this
		let {dataAdapterBlock, loadServerDataBlock, loadServerDataEkDuzenleBlock} = this;
		if (!(dataAdapterBlock || loadServerDataBlock || mfSinif))
			return null
		...
		let da
		if (dataAdapterBlock)
			da = getFuncValue.call(this, dataAdapterBlock, { ...e })
		...
		if (!da) {
			da = new $.jqx.dataAdapter({ dataType: wsDataType, url: 'empty.json', data: e }, {
				...
				loadServerData: async (wsArgs, source, callback) => {
					let lastError, sender = this
					let { parentPart, builder, secimler, mfSinif } = this
					let args = parentPart?.args
					// let _e = extend({}, e, { wsArgs: wsArgs, source: source, callback: callback })
					let kodSaha = mfSinif
						? (isArray(mfSinif.idSaha) ? mfSinif.idSaha[0] : mfSinif.idSaha) ?? mfSinif.kodSaha
						: MQKA.kodSaha;
					let adiSaha = mfSinif ? mfSinif.adiSaha : MQKA.adiSaha
					let kodKullanilirmi = mfSinif?.kodKullanilirmi, adiKullanilirmi = mfSinif?.adiKullanilirmi;
					let temps = {}; let {wsArgsDuzenleBlock, ozelQueryDuzenleBlock} = this;
					for (let i = 0; i < 3; i++) {
						try {
							if (wsArgsDuzenleBlock) {
								let _e = { ...e, wsArgs, source, temps }
								await wsArgsDuzenleBlock.call(this, wsArgsDuzenleBlock, _e)
								wsArgs = _e.wsArgs
								temps = _e.temps
							}
							if (ozelQueryDuzenleBlock) {
								wsArgs.ozelQueryDuzenleBlock = ozelQueryDuzenleBlock
								wsArgs.temps = temps
							}
							
							let { tabloKolonlari } = wsArgs
							if (!tabloKolonlari) {
								tabloKolonlari = this._wsArgs_tabloKolonlari
								if (!tabloKolonlari) {
									tabloKolonlari = []
									if (kodKullanilirmi)
										tabloKolonlari.push(gridKolon(kodSaha))
									if (adiKullanilirmi)
										tabloKolonlari.push(gridKolon(adiSaha))
								}
								wsArgs.tabloKolonlari = this._wsArgs_tabloKolonlari = tabloKolonlari
							}
							
							let _e = { parentPart, sender, builder, secimler, callback, args, ...wsArgs }
							let result = loadServerDataBlock
								? await getFuncValue.call(this, loadServerDataBlock, _e)
								: ( mfSinif ? (await mfSinif.loadServerData(_e)) : null )
							...
							let { records: recs } = result
							if (!recs)
								return
							
							let { bosKodAlinirmi, bosKodEklenirmi } = this
							if (!bosKodAlinirmi) {
								let index = recs.findIndex(r => !r[kodSaha])
								if (index != false && index > -1)
									recs.splice(index, 1)
							}
							
							if (bosKodEklenirmi) {
								let rec = recs.find(r => !r[kodSaha])
								if (!rec) {
									rec = { [kodSaha]: '', [adiSaha]: '' }
									recs.unshift(rec)
								}
							...
							if (loadServerDataEkDuzenleBlock) {
								_e.recs = recs
								let _result = await loadServerDataEkDuzenleBlock.call?.(this, _e)
								if (isArray(_result)) {
									recs = _e.recs = _result
									result.records = recs
									if (result.totalrecords != null)
										result.totalrecords = e.totalrecords ?? recs.length
								}
							}
							...
							try { callback(result) }
							catch (ex) { }  // ignore callback errors
							
							// await defer(p => setTimeout(async () => { await this.veriYuklendi(e); p.resolve() }, i * 10))
							await this.veriYuklendi(e)
							break
						}
						catch (ex) {
							lastError = ex
							console.error('modelKullanPart', 'loadServerData', ex, getErrorText(ex))
							await defer(p => setTimeout(() => p.resolve(), i * 100))
						}
					}
					
					if (lastError) {
						let ex = lastError
						if (!(ex?.message?.includes(`undefined (reading '1')`)))
							console.error(ex); displayMessage(getErrorText(ex), 'ComboBox Verisi Alınamadı')
					}
				}
			})
		}
