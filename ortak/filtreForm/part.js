class FiltreFormPart extends Part {
	...
	runDevam(e) {
		e = e || {}; super.runDevam(e); const {layout} = this, input = this.input = this.input ?? layout.find('.input');
		input.val(this.value?.trim()); input.off('focus').on('focus', evt => setTimeout(() => evt.target.select(), 200));
	...
	static hizliBulIslemi(e) {
		...
		for (const css of [css_include, css_exclude]) {
			const elms = layout.find(`.${css}`); for (let i = 0; i < elms.length; i++) {
				const elm = elms.eq(i); for (const item of [elm, elm.parent()]) item.removeClass(css) }
		...
		for (let i = 0; i < elms.length; i++) {
			...
			let uygunmu = !!tokens.length && (
				( tagName == 'INPUT' && !( elm.attr('type') == 'button' || elm.attr('type') == 'hidden' ) ) || tagName == 'TEXTAREA' || tagName == 'LABEL' || tagName == 'LI' ||
				( elm.hasClass('jqx-grid-cell') || elm.parents('.jqx-grid-cell').length ) || ( elm.hasClass('jqx-grid-group-cell') || elm.parents('.jqx-grid-group-cell').length )
			);
			if (uygunmu) { value = (elm.val() || elm.text() || '').toString().toUpperCase(); uygunmu = !!value }
			if (uygunmu) { for (const token of tokens) { if (token && !value.includes(token)) { uygunmu = false; break } } }
			/* const parent = elm.parent(); */ for (const item of [elm]) { item.addClass(uygunmu ? css_include : css_exclude) }
			if (uygunmu) {
