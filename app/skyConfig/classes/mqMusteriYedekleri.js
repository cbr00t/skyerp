class MQMusteriYedekleri extends MQDetayli {
	...
	static rootFormBuilderDuzenle_listeEkrani(e) {
		super.rootFormBuilderDuzenle_listeEkrani(e); const gridPart = e.gridPart ?? e.sender, {secimler, islemTuslari} = gridPart, rfb = e.rootBuilder, layout = rfb.layout = e.layout;
		const islemTuslari_sol = islemTuslari.children('.sol'); islemTuslari_sol.addClass('flex-row'); const fbd_islemTuslari_sol = rfb.addForm({ id: 'islemTuslari_sol', layout: islemTuslari_sol });
		fbd_islemTuslari_sol.addNumberInput({ id: 'gunSayi', etiket: 'Gün Sayısı', min: 0, max: 60, value: secimler.gunSayi.value }).etiketGosterim_yok()
			.degisince(e => {
				...
				setTimeout(() => { const rootLayout = builder.rootPart.layout; rootLayout.css('--header-ek-height', `8px`) }, 0)
	...
	static orjBaslikListesi_argsDuzenle(e) { super.orjBaslikListesi_argsDuzenle(e); const {args} = e; $.extend(args, { rowsHeight: 80 }) }
		...
		e.liste.push(...[
			...
			new GridKolon({
				...
				cellsRenderer: (colDef, rowIndex, columnField, value, html, jqxCol, rec) => {
					const kod = rec.mustKod, aciklama = rec.mustUnvan;
					value = `<span class="kod bold">(${kod})</span>`; if (aciklama) { value += ` <span class="aciklama">${aciklama}</span>` }
					return changeTagContent(html, value)
				}
			}),
			new GridKolon({
				belirtec: 'errorText', text: 'Sorunlar',
				cellClassName: (sender, rowIndex, belirtec, value, rec) => { let result = [belirtec]; if (rec.isError) { result.push('bold', 'bg-very-lightred') } return result.join(' ') },
				cellsRenderer: (colDef, rowIndex, columnField, value, html, jqxCol, rec) => changeTagContent(html, `<div style="margin: -33px 0 0 0; padding: 0; line-height: 20px; overflow-y: auto; font-size: 95%">${html}</div>`)
			})
	...
	static loadServerData(e) {
		const promise = new $.Deferred(), {secimler} = e;
		setTimeout(async e => {
			try {
				const result = await app.tanitimIcinYedekAnalizi($.extend({}, e, {
					tanitimListe: secimler.tanitim.value || '', mustUnvan: (secimler.mustUnvan.value || '').replaceAll('*', '%').replaceAll('?', '_'),
					gunSayi: secimler.gunSayi.value, pattern: secimler.pattern.value
				}));
	...
	static gridVeriYuklendi(e) { super.gridVeriYuklendi(e); const {sender, grid} = e, {fbd_gunSayi} = sender; if (fbd_gunSayi) { fbd_gunSayi.value = sender.secimler.gunSayi.value } grid.resize() }
	static async tanimla(e) {
		const {islem, rec, sender} = e; if (islem == 'sil') { return null }
		const {tanimUISinif} = this; if (!tanimUISinif) { return null }
		const tanitim = rec?.tanitim; let promise = new $.Deferred();
		const part = new tanimUISinif({
			sender, islem, rec, tanitim,
			tamamIslemi: e => { if (sender?.tazele) { sender.tazele(e) } if (promise) { promise.resolve(e) } },
			kapaninca: e => { if (promise) { promise.resolve(e) } }
		});
		part.run(); const result = await promise;
		return { part, result }
	}
}
class MQMusteriYedekDosyalari extends MQDetay {
	static orjBaslikListesiDuzenle(e) {
		super.orjBaslikListesiDuzenle(e);
		const cellClassName_size = (sender, rowIndex, belirtec, value, rec) => { let result = [belirtec]; if (rec.invalid) { result.push('bold', 'red') } return result.join(' ') };
		e.liste.push(...[
	...
	static orjBaslikListesi_argsDuzenle(e) { super.orjBaslikListesi_argsDuzenle(e); const {args} = e; args.showGroupsHeader = true }
	static loadServerData(e) {
		const {parentRec} = e, recs = [],  tarih2Files = parentRec?.files || [];
		for (const files of Object.values(tarih2Files)) { recs.push(...files) }
		return recs
	}
	static gridVeriYuklendi(e) { super.gridVeriYuklendi(e) /* const {sender, grid} = e  grid.jqxGrid('groups', ['tarih']) */ }
}
