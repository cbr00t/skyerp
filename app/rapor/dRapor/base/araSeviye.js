...
class DRapor_AraSeviye_Main extends DAltRapor_TreeGridGruplu {
		...
		if (totalmi) {
			...
			function getRec({ kod, text, pfKod, pfAdi, pfRes, update }) {
				...
				return {
					...
					getYatayDegerler() {
						let pfRes = pfAdi.toLocaleUpperCase()
						return ['BU', 'ÖNCEKİ']
							.map(pre => `${pre} ${pfRes}`)
					},
					duzenle(e = {}) {
						...
						/*if (!empty(recs)) {
							...
							;recs.forEach(r => {
								...
								let v = asDate(r[tk])
								if (!isInvalidDate(v)) {
									let digermi = diger.uygunmu(v)
									yatay = r[yk] = vals[digermi ? 1 : 0]
								}
								if (yatay)
									r[yk] = yatay
							})
	...
	loadServerData_recsDuzenleIlk({ recs } = {}) {
		const PostfixBakiye = 'yurbakiye', PostfixBedel = 'bedel'
		if (!empty(recs) && keys(recs[0]).find(k => k.includes(PostfixBakiye))) {
			;{
	...
	async loadServerData_ilk(e) {
		let { attrSet, yatayAnaliz: yatayAnalizmi } = e
		let tekilKeymi = keys(attrSet).length == 1
		let { raporTanim } = this
		let { yatayBilgi, _ozelAttrSet } = raporTanim
		let { kod: yatayKod, dbmi, donemmi } = yatayBilgi ?? {}
		
		if (yatayAnalizmi && dbmi) {
			let alias_db = 'db'
			let { session } = config, { dbName: buDBName } = session
			let { ekDBListe } = app.params?.dRapor ?? {}
	...
	loadServerData_queryDuzenle_filtreBaglantiYap(e = {}) {
		let { stm, attrSet: orjAttrSet } = e
		let { secimler, tabloYapi, raporTanim = {} } = this
		let { yatayBilgi } = raporTanim
		
		let internal = true
		let attrSet = { ...orjAttrSet }
	...
	async raporTanim_tamamSonrasiIslemler({ raporTanim, raporTanim: { icerik } }) {
		...
		for (let [prefix, brmKey] of entries(miktar2BrmKeys)) {
			let miktarKey = postfix + prefix
			if (icerik[miktarKey] && !icerik[brmKey]) {
				icerik[brmKey] = true
				notify(
					`<b class=royalblue>${miktarKey}</b> sahası nedeniyle <b class=firebrick>${brmKey}</b> kolonu tanıma otomatik eklendi`,
					'Rapor Tanımı', 'warning'
				)
			}
		}
		/*for (let prefix_ek of ['2', '']) {
			let prefixMiktar = `MIKTAR${prefix_ek}`
			let keyBrm = `BRM${prefix_ek}`
			let miktarSet = {}, brmsizMiktarSet = {}
			for (let key in icerik) {
				// ** MIKTAR => MIKTAR2 | MIKTAR2 => MIKTAR22 gibi kontrol eder
				// MIKTAR, MIKTARAD, MIKTARKG, MIKTAR2 ... için BRM/BRM2 eklenir
				if (!(key.startsWith(prefixMiktar) || key.startsWith(`${prefixMiktar}2`) || key.slice(2).startsWith(prefixMiktar)))
					continue
				miktarSet[key] = true
			...
			if (!(empty(brmsizMiktarSet) || icerik[keyBrm])) {
				icerik[keyBrm] = true
				notify(
					`<b class=royalblue>${prefixMiktar}</b> sahası nedeniyle <b class=firebrick>${keyBrm}</b> kolonu tanıma otomatik eklendi`,
					'Rapor Tanımı', 'warning'
				)
			}
		}*/
	...
	donemBagla(e = {}) {
		let { donemBS: tarihBS, tarihSaha, sent, attrSet } = e
		let rapor = this, { tabloYapi, raporTanim } = this
		let { yatayBilgi } = raporTanim
		if (yatayBilgi?.duzenle) {
			let args = { rapor, tabloYapi, raporTanim, attrSet, tarihBS, sent, tarihSaha }
			yatayBilgi.duzenle(args)
			e.donemBS = args.tarihBS
		}
		else if (tarihBS) {
			let { where: wh } = sent
	...
	loadServerData_queryDuzenle_sh({ stm, sent, attrSet }) {
		...
		for (let key in attrSet) {
			switch (key) {
				...
				case 'SHMARKA':
					sahalar.add(
						(hizmetmi ? `${sqlEmpty} markakod` : `${alias}.smarkakod markakod`),
						(hizmetmi ? `${sqlEmpty} markaadi` : 'smar.aciklama markaadi')
					)
					break
				case 'BRM':
					...
					break
				case 'BRM2':
					sahalar.add(hizmetmi ? `${sqlEmpty} brm2` : `${alias}.brm2`)
					break
				case 'BRMORANI':
					sahalar.add(hizmetmi ? `${sqlEmpty} brmorani` : `${alias}.brmorani`)
					break
				case 'STOKRESIM':
	...
	getBrmliMiktarClause(e = {}) {
		...
		return `SUM(case when ${getWhereClause('brm')} then ${miktarClause}` +
						( miktar2Varmi ? ` when ${getWhereClause('brm2')} then ${miktar2Clause}` : '' ) +
						` else 0 end)`
	}
	getDovizmi(e) {
