...
class DRapor_Muhasebe_Main extends DRapor_Donemsel_Main {
	...
	tabloYapiDuzenle(e) {
		...
		result
			...
			.addGrup(new TabloYapiItem().setKA('CERCEVEHESAP', 'Çerçeve Hesap').setMFSinif(DMQMuhHesap).addColDef(new GridKolon({ belirtec: 'cercevehesap', text: 'Çerçeve Hesap', maxWidth: 450, filterType: 'checkedlist' })))
			.addGrup(new TabloYapiItem().setKA('KEBIRHESAP', 'Kebir Hesap').secimKullanilir().setMFSinif(DMQMuhHesap_Kebir).addColDef(new GridKolon({ belirtec: 'kebirhesap', text: 'Kebir Hesap', maxWidth: 450, filterType: 'checkedlist' })))
			.addGrup(new TabloYapiItem().setKA('SINIFHESAP', 'Sınıf Hesap').setMFSinif(DMQMuhHesap).addColDef(new GridKolon({ belirtec: 'sinifhesap', text: 'Sınıf Hesap', maxWidth: 450, filterType: 'checkedlist' })))
	...
	loadServerData_queryDuzenle(e) {
		super.loadServerData_queryDuzenle(e); let muhParam = app.params.muhasebe ?? {}, maliYil = muhParam.maliYil || today().yil;
		let {stm, attrSet} = e, {sent} = stm, {where: wh, sahalar} = sent, alias = 'fis';
		$.extend(e, { sent, alias }); sent.fisHareket('muhfis', 'muhhar').har2MuhHesapBagla(); wh.degerAta(maliYil, `${alias}.maliyil`);
		this.donemBagla({ ...e, tarihSaha: `${alias}.tarih` }); for (const key in attrSet) {
			switch (key) {
