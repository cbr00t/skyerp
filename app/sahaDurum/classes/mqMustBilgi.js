class MQMustBilgi extends MQKAOrtak {
	...
	static orjBaslikListesiDuzenle(e) {
		...
		colDef = liste.find(colDef => colDef.belirtec == 'aciklama'); $.extend(colDef, {
			genislikCh: null, cellsRenderer: (colDef, rowIndex, belirtec, value, html, jqxCol, rec) => {
				rec = rec?.tanim ?? rec;
				const marginStyle = 'margin-inline-end: 20px';
				return changeTagContent(html, ((
					`<div class="flex-row">` +
						`<div class="bold gray" style="${marginStyle}">${rec.kod || ''}</div>` + `<div style="${marginStyle}">${rec.aciklama || ''}</div>` +
						/* `<div class="ek-bilgi gray" style="${marginStyle}">${rec.yore || ''}</div>` + `<div class="ek-bilgi gray">${rec.iladi || ''}</div>` + */
					`</div>`
				)))
			}
		});
		/*const cellsRenderer = (colDef, rowIndex, belirtec, value, html, jqxCol, rec) => {
			rec = $.isPlainObject(rec) ? new MustBilgi(rec) : rec; const newValue = rec[belirtec];
			if (value != newValue) { html = changeTagContent(html, newValue) } return html
		};*/
		liste.push(
		...
		if (user && loginTipi != 'plasiyerLogin') liste.push(new GridKolon({ belirtec: 'plasiyerKod', text: 'Plasiyer', genislikCh: 10, cellClassName: 'darkgray' }))
	}
	static gridVeriYuklendi(e) {
		const {grid} = e, session = config.session ?? app.params.yerel.lastSession ?? {}, {loginTipi, user} = session, groups = [];
		if (user && loginTipi != 'plasiyerLogin') { groups.push('plasiyerKod') } groups.push('iladi')
	...
	static async loadServerData(e) {
		...
		let result = localData.getData(dataKey); if (result == null) {
			...
			const cariEkstre_fisSayac2Rec = {};
			for (const cls of classes) {
				const subDataKey = cls.dataKey; if (!subDataKey) continue; const recs = await cls.loadServerDataDogrudan(e);
				for (const rec of recs) {
					...
					const kod = (
						( rec.mustkod ?? rec.mustKod ?? rec.must ?? rec.kod ) ??
			...
			const AsyncMax = 20; let promises = [];
