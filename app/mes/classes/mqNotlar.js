class MQEkNotlar extends MQSayacliOrtak {
	...
	static ekCSSDuzenle(e) {
		...
		else { const {ekNotLastReadId} = this, {kaysayac: id} = rec; if (id && (!ekNotLastReadId || id > ekNotLastReadId)) { result.push('yeni-not') } }
	...
	static orjBaslikListesi_gridRendered(e) {
		...
		if (type == 'cellsRenderer') {
			const {rec} = e, id = rec.kaysayac, {localData} = app.params; let ekNotLastReadId = this.ekNotLastReadId = asInteger(localData.getData('ekNotLastReadId'));
			if (id && ekNotLastReadId < id) { ekNotLastReadId = id; localData.setData('ekNotLastReadId', ekNotLastReadId); localData.kaydetDefer() }
		}
	}
