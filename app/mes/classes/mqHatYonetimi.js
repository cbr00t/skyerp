class MQHatYonetimi extends MQMasterOrtak {
	...
	static listeEkrani_afterRun(e) { super.listeEkrani_afterRun(e) /*let gridPart = e.gridPart ?? e.sender*/ }
	static listeEkrani_activated(e) { super.listeEkrani_activated(e); let gridPart = e.gridPart ?? e.sender /*; gridPart.tazeleDefer()*/ }
	static rootFormBuilderDuzenle_listeEkrani(e) {
		super.rootFormBuilderDuzenle_listeEkrani(e); let rfb = e.rootBuilder, gridPart = e.gridPart ?? e.sender ?? rfb?.part;
		/*let fbd_islemTuslari = rfb.addForm('islemTuslari', e => e.builder.part.islemTuslariPart.layout);
		let fbd_sol = fbd_islemTuslari.addForm('sol', e => e.builder.parent.children('.sol')), fbd_sag = fbd_islemTuslari.addForm('sol', e => e.builder.parent.children('.sag'));*/
		this.fbd_listeEkrani_addCheckBox(rfb, 'cokluSecimFlag', 'Çoklu').onAfterRun(e => {
			let {builder} = e, {rootPart, layout} = builder, input = layout.children('input'), {grid, gridWidget} = rootPart; rootPart.fbd_cokluSecim = builder;
			let cokluSecimDegisti = value => { grid.jqxGrid('selectionmode', value ? 'multiplecells' : 'singlecell'); try { gridWidget.clearselection() } catch (ex) { } }
			if (rootPart.cokluSecimFlag) { input.prop('checked', true); cokluSecimDegisti(true) }
			input.on('change', evt => { let value = rootPart.cokluSecimFlag = $(evt.currentTarget).is(':checked'); cokluSecimDegisti(value) })
		});
		this.fbd_listeEkrani_addCheckBox(rfb, 'otoTazeleFlag', 'Oto Tazele').onAfterRun(e => {
			...
			input.on('change', evt => {
				let value = app.otoTazeleFlag = app.otoTazeleFlag = !!$(evt.currentTarget).is(':checked');
				let fbd_grupsuz = builder.parentBuilder.id2Builder.grupsuzmu; if (fbd_grupsuz) { fbd_grupsuz.updateVisible() }
		...
		this.fbd_listeEkrani_addCheckBox(rfb, 'grupsuzmu', 'Grupsuz').onAfterRun(e => {
			...
			input.on('change', evt => { let value = rootPart.grupsuzmu = $(evt.currentTarget).is(':checked'); e.action = 'toggle'; rootPart.tazele(e) })
		});
		rfb.addForm('kronometre').addCSS('flex-row')
			...
			.onAfterRun(e => {
				let {builder} = e, {rootPart, layout} = builder, elmValue = layout.find('#value'), btnReset = layout.find('#reset');
				let timerKey = 'timer_kronometre', kronometre = this.kronometre = this.kronometre || {}; elmValue.attr('placeholder', 'Kronometre');
				elmValue.on('click', evt => {
					let {kronometre} = this; let running = !kronometre.running;
					if (running) {
						...
						kronometre[timerKey] = setInterval(kronometre => {
							...
							let value = kronometre.value = timeToString(now() - kronometre.startTS, false, true); elmValue.html(value)
						}, 90, kronometre);
				...
				btnReset.jqxButton({ theme }).on('click', evt => {
					let {kronometre} = this; if (!kronometre) { return }
					...
					let value = kronometre.value = null; kronometre.running = false; elmValue.html(value); btnReset.addClass('jqx-hidden')
				})
			});
		rfb.onAfterRun(e => {
			let gridPart = e.builder.part, hizliBulLayout = gridPart.bulPart?.layout?.children('input');
	...
	static islemTuslariDuzenle_listeEkrani(e) {
		...
		e = $.extend({}, e);  let {liste} = e, {sabitHatKodVarmi} = app, gridPart = e.parentPart, butonlarPart = e.part; e.recs = gridPart.selectedRecs;
		let items = [
		...
		let ekSagButonIdSet = butonlarPart.ekSagButonIdSet = butonlarPart.ekSagButonIdSet || {}; $.extend(ekSagButonIdSet, asSet(items.map(item => item.id)))
	}
	static orjBaslikListesi_argsDuzenle(e) {
		super.orjBaslikListesi_argsDuzenle(e); let {args} = e, gridPart = e.gridPart ?? e.parentPart ?? e.sender; gridPart.rowNumberOlmasin();
		$.extend(args, { showFilterRow: false, showGroupsHeader: false, groupsRenderer: (...args) => this.orjBaslikListesi_renderGroupsHeader(...args) })
	}
	static orjBaslikListesi_getPanelDuzenleyici(e) { let getCellLayoutIslemi = e => this.gridCell_getLayout(e); return new GridPanelDuzenleyici({ getCellLayoutIslemi }) }
	...
	static orjBaslikListesi_renderGroupsHeader_grupText(text, group, expanded, groupInfo) {
		let /*kod2EkNotlarRec = this.ekNotlarYapi?.HT || {}*/ allSubItems = [], {sabitHatKodVarmi} = app;
		let fillSubItems = info => {
		...
		let rec = allSubItems[0], {hatKod} = rec; /*, ekNotlarRec = kod2EkNotlarRec[hatKod] || {}; */
	...
	static orjBaslikListesi_panelGrupAttrListeDuzenle(e) { super.orjBaslikListesi_panelGrupAttrListeDuzenle(e); let {liste} = e; liste.push('grupText') }
	static orjBaslikListesi_groupsDuzenle(e) {
		super.orjBaslikListesi_groupsDuzenle(e); let gridPart = e.gridPart ?? e.sender, {liste} = e;
		if (!gridPart.recs?.length || !(gridPart.grupsuzmu || app.otoTazeleFlag)) { let grupListe = this.orjBaslikListesi_panelGrupAttrListe || []; if (grupListe?.length) { liste.push(...grupListe) } }
	}
	static orjBaslikListesi_panelUstSeviyeAttrListeDuzenle(e) { super.orjBaslikListesi_panelUstSeviyeAttrListeDuzenle(e); let {liste} = e; liste.push('hatKod', 'hatAdi') }
	static orjBaslikListesi_hizliBulFiltreAttrListeDuzenle(e) {
		super.orjBaslikListesi_hizliBulFiltreAttrListeDuzenle(e); let {gridPart, liste} = e;
		liste.push('hatKod', 'hatAdi', 'tezgahKod', 'tezgahAdi', 'perKod', 'perIsim', 'ip', 'isListe')
	}
	static onSignalChange(e) {
		let gridPart = e.gridPart ?? e.sender; e.action = 'otoTazele';
	...
	static async loadServerData_internal(e) {
		e = e || {}; let gridPart = e.gridPart ?? e.sender, {wsArgs} = e, action_otoTazeleFlag = e.action == 'otoTazele';
		let tezgahKodSet = {}, tezgahKod2Rec = {}, isID2TezgahKodSet = {}, promise_tezgah2SinyalSayiRecs;
		let hatIdListe = app.sabitHatKodVarmi ? app.sabitHatKodListe : $.makeArray(gridPart.hatKod), {excludeTezgahKod} = gridPart;
		...
		if (recs) {
			...
			for (let rec of recs) {
				for (let [key, newKey] of Object.entries(donusum)) { if (rec[newKey] == null) { rec[newKey] = rec[key]?.trimEnd(); delete rec[key] } }
		...
		if (recs) {
			if (!empty(tezgahKodSet)) {
				let sent = new MQSent({
					from: 'messinyal sny',
					where: { inDizi: Object.keys(tezgahKodSet), saha: 'sny.tezgahkod' },
			...
			for (let rec of _recs) {
				let {hatKod, tezgahKod, isID} = rec
				if (excludeTezgahKod && tezgahKod == excludeTezgahKod)
					continue
				let {isListe} = tezgahRec
				let tezgahRec = tezgahKod2Rec[tezgahKod] ?? { ...rec }
				if (!tezgahKod2Rec[tezgahKod]) {
					tezgahKod2Rec[tezgahKod] = tezgahRec
					recs.push(tezgahRec)
					isListe = tezgahRec.isListe = []
				}
				tezgahRec.isSaymaInd = (tezgahRec.isSaymaInd || 0) + (rec.isSaymaInd || 0)
				tezgahRec.isSaymaSayisi = (tezgahRec.isSaymaSayisi || 0) + (rec.isSaymaSayisi || (isID ? 1 : 0))
				if (isID) {
					/*let {isToplamOlasiSureSn, isToplamBrutSureSn, isToplamDuraksamaSureSn} = rec; rec.oee = isToplamOlasiSureSn ? Math.min(Math.round((isToplamBrutSureSn - isToplamDuraksamaSureSn) * 100 / isToplamOlasiSureSn), 100) : 0*/
					let {oemgerceklesen, oemistenen} = rec
					rec.oee = oemistenen ? roundToFra(Math.max(oemgerceklesen * 100 / oemistenen, 0), 2) : 0
					delete rec.isListe
					isListe.push(rec)
					(isID2TezgahKodSet[isID] ||= {})[tezgahKod] = true
				}
			}
		}
		if (!action_otoTazeleFlag && tezgahKod2Rec && !empty(isID2TezgahKodSet)) {
			for (let [isId, tezgahKodSet] of entries(isID2TezgahKodSet)) {
				isId = asInteger(isId)
				let rec
				try {
					rec = await app.wsGorevZamanEtuduVeriGetir({ isId })
					if (!rec?.bzamanetudu)
						rec = null
				}
				catch { }
				if (!rec)
					continue
				for (let tezgahKod in tezgahKodSet) {
					rec = tezgahKod2Rec[tezgahKod]
					if (rec)
						rec.zamanEtuduVarmi = true
				}
			}
		}
		if (promise_tezgah2SinyalSayiRecs && tezgahKod2Rec) {
			...
			for (let {tezgahkod: tezgahKod, bsanal: sanalmi, sayi} of _recs) {
				let rec = tezgahKod2Rec[tezgahKod]
				if (!rec)
					continue
				let key = sanalmi ? 'sanal' : 'cihaz'
				let sinyalSayilar = rec.sinyalSayilar = {}
				sinyalSayilar[key] = (sinyalSayilar[key] || 0) + (sayi || 0)
			}
		}
		if (recs) {
			for (let rec of recs) {
				let {hatKod, hatAdi} = rec, styles_bgImg_url = [], imageInfos = [ { align: 'left' }, { align: 'center', postfix: '-01' }, { align: 'right', postfix: '-02' } ]
				for (let {align, postfix} of imageInfos) {
					let id = `hat-${hatKod}`; if (postfix) { id += postfix } let url = `${app.getWSUrlBase()}/stokResim/?id=${id}`;
					styles_bgImg_url.push(`url(${url}) ${align} center no-repeat`)
				}
				let styles_bgImg_size = styles_bgImg_url.map(x => 'contain'), styles_bgImg = [`background: ${styles_bgImg_url.join(', ')}`, `background-size: ${styles_bgImg_size.join(', ')}`];
	...
	static orjBaslikListesi_gridRendered(e) {
		super.orjBaslikListesi_gridRendered(e); let {gridPart} = e, {grid, gridWidget} = gridPart, table = gridWidget?.table; if (!table) { return }
		let ustAltFormlar = table.find(`[role = row] > * .ust-alt`);
		if (ustAltFormlar?.length) { for (let key of ['mousedown', 'touchstart']) { ustAltFormlar.on(key, evt => { app.otoTazeleTempDisable() }) } }
		let buttons = table.find(`[role = row] > * button`); if (buttons?.length) {
			buttons.jqxButton({ theme }).off('click').on('click', async evt => {
				...
				this._lastButonClickEventTime = now(); let target = evt.currentTarget; await new $.Deferred(p => setTimeout(() => p.resolve(), isTouchDevice() ? 200 : 100));
				if (gridPart.cokluSecimFlag) {
					...
					let {fbd_cokluSecim} = gridPart; if (fbd_cokluSecim) { fbd_cokluSecim.layout.children('input').prop('checked', false) }
				}
				$.extend(e, { event: evt, gridPart }); let id = target.id, grup_hatKod = $(target).parents('tr').data('hatkod');
				...
				if (!rec) { return } let {tezgahAdi} = rec, {boundindex, visibleindex} = parentRec, ctrlFlag = evt?.ctrlKey;
				/*let hatKod = $(target).parents('tr').data('hatkod'), hatRec = hatKod ? gridPart.recs.find(rec => rec.hatKod == hatKod) : null;
	...
	static topluXMenuIstendi(e) {
		e = e || {}; let {parentRec} = e, {sabitHatKodVarmi} = app, hatKod = e.hatKod = e.hatKod ?? parentRec?.hatKod;
	...
	static personelSecIstendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, {gridWidget} = gridPart;
		let _recs = e.recs ?? gridPart.getSubRecs({ gridPart, cells: gridWidget.getselectedcells().filter(cell => cell.datafield[0] == '_') });
		return MQPersonel.listeEkraniAc({
			tekil: true, args: { _recs }, secince: async e => {
				let perKod = e.value, {sender} = e, tezgahKodListe = sender._recs?.map(rec => rec.tezgahKod);
				if (tezgahKodListe?.length) {
					let promises = []; for (let tezgahKod of tezgahKodListe) { promises.push(app.wsPersonelAta({ tezgahKod, perKod })) }
		...
		try {
			...
			return MQHat.listeEkraniAc({
				tekil: true, args: { exclude_hatKod, _recs }, title: `<b class="royalblue">${_recs.length} adet Tezgahı</b> <span class="darkgray">şu Hat'a taşı:</span>`, secince: async e => {
					let hatKod = e.value, {sender} = e, tezgahKodListe = sender._recs?.map(rec => rec.tezgahKod); if (!tezgahKodListe?.length) { return }
					...
					try {
						...
						for (let tezgahKod of tezgahKodListe) {
							promises.push(app.wsSiradakiIsler({ tezgahKod }).then(isRecs => {
								let isIdListe = isRecs.map(rec => rec.issayac).join(delimWS); if (!isIdListe?.length) { return }
	...
	static makineDurumIstendi(e) { let recs = e.recs ?? (e.rec ? [e.rec] : []); for (let rec of recs) { let {tezgahKod} = rec; new MakineYonetimiPart({ tezgahKod }).run() } }
	...
	static xIslerIstendi(e) {
		...
		for (let rec of recs) { let {hatKod, hatAdi, tezgahKod, tezgahAdi} = rec; mfSinif.listeEkraniAc({ args: { hatKod, hatAdi, tezgahKod, tezgahAdi, hatBazindami }}) }
	}
	static topluXIstendi(e) {
		let {id} = e, gridPart = e.gridPart ?? e.sender ?? e.builder?.rootBuilder?.part; let recs = gridPart?._lastRecs;
		let hatKodListe = e.hatKodListe ?? (e.hatKod ? [e.hatKod] : []), wsArgs = {};
		...
		let hatBazindami = !!e.hatKod, recsCount = !hatBazindami ? 0 : recs?.length, islemAdi = islemKod2Adi[id] ?? id;
	...
	static ozetBilgi_getLayout(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, recs = gridPart?._lastRecs; if (!recs) { return null }
		...
		let textList = []; for (let [hat, durum2Sayi] of Object.entries(hat2Durum2Sayi)) {
		...
		let ekTextList = []; if (!$.isEmptyObject(durNeden2TezgahKod)) {
			...
			for (let [neden, kodListe] of Object.entries(durNeden2TezgahKod)) { ekTextList.push(`<tr class="data"><td>${neden}</td><td>${kodListe.length}</td></tr>`) }
	...
	static ekBilgiIstendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, rec = e.rec ?? gridPart.selectedRec, {tezgahKod, tezgahAdi} = rec;
		if (rec) {
			createJQXWindow({
	...
	static bekleyenIsEmirleriIstendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart;
		let rec = e.rec ?? gridPart.selectedRec ?? {}; let {hatKod} = rec; if (!hatKod) { return }
		MQBekleyenIsEmirleri.listeEkraniAc({ args: { hatKod } })
	}
	static ekNotlarIstendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, hepsimi = e.hepsi ?? e.hepsimi;
		let rec = e.rec ?? gridPart.selectedRec ?? {}; let {hatKod} = rec;
	...
	static ekNotEkleIstendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, rec = e.rec ?? gridPart.selectedRec ?? {};
		let hatKod = rec.hatKod ?? '', tezgahKod = rec.tezgahKod ?? ''; if (!(hatKod || tezgahKod)) { return }
		let inst = new MQEkNotlar({ hatKod, tezgahKod }); return inst.tanimla({ islem: 'yeni' })
	}
	static dokumanYukleIstendi(e) {
		e = e || {}; let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, rec = e.rec ?? gridPart.selectedRec ?? {};
		let hatKod = rec.hatKod ?? ''; if (!hatKod) { return } let resimId = `hat-${hatKod}-01`, islemAdi = 'Hat Resim Yükleme';
	...
	static async dokumanSilIstendi(e) {
		e = e || {}; let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, rec = e.rec ?? gridPart.selectedRec ?? {};
		let hatKod = rec.hatKod ?? ''; if (!hatKod) { return } let resimId = `hat-${hatKod}`, islemAdi = `<b color="indianred">Resim SİL</b>`;
	...
	static async ekBilgiSilItendi(e) {
		let gridPart = e.gridPart ?? e.sender ?? e.parentPart ?? e.builder?.rootBuilder?.parentPart, recs = e.recs ?? gridPart.selectedRecs ?? []; if (!recs?.length) { return }
		let tezgahIdListe = recs.map(rec => rec.tezgahKod);
		try { await app.wsEkBilgiTopluSifirla({ tezgahIdListe }); gridPart.tazele(e) }
		catch (ex) { console.error(ex); hConfirm(getErrorText(ex)) }
	}
	static openContextMenu(e) {
		let noCheckFlag = e.noCheck ?? e.noCheckFlag, gridPart = e.gridPart = e.gridPart ?? e.sender ?? e.parentPart, gridWidget = e.gridWidget = gridPart.gridWidget;
		let cells = e.cells = gridWidget.getselectedcells().filter(cell => cell.datafield[0] == '_');
		let {title} = e; let recs = e.recs = (e.recs ?? gridPart.getSubRecs(e)).filter(rec => !!rec);
		let parentRec = e.parentRec = e.parentRec ?? gridPart.selectedRec, rec = e.rec = (recs || [])[0];
	...
	static gridCell_getLayout(e) {
		let gridPart = e.gridPart ?? e.sender, rec = e.rec ?? {}, isListe = rec.isListe ?? [], grupsuzmu = gridPart.grupsuzmu || app.otoTazeleFlag;
		let {hatKod, hatAdi, tezgahKod, tezgahAdi, perKod, perIsim, sinyalKritik, duraksamaKritik, durumKod, durumAdi,
			   durNedenKod, durNedenAdi, ip, siradakiIsSayi, ekBilgi, zamanEtuduVarmi, sinyalSayilar} = rec;
		let {kritikDurNedenKodSet} = app.params.mes, kritikDurNedenmi = kritikDurNedenKodSet && durNedenKod && durumKod == 'DR' ? kritikDurNedenKodSet[durNedenKod] : false;
		let isBilgiHTML = this.gridCell_getLayout_isBilgileri(e);
		let sinyalHTMLListe = [], toplam = 0; for (let key of ['cihaz', 'sanal']) {
	...
	static gridCell_getLayout_isBilgileri(e) {
		e = e ?? {}; let _now = now(), rec = e.rec ?? e.inst ?? {}, isListe = rec.isListe ?? [], isBilgiItems = [], {sinyalKritik, maxAyrilmaDk} = rec;
		let grafikPart = new GaugeGrafikPart(), colors = grafikPart.colors ?? [];
		for (let i = 0; i < isListe.length; i++) {
			...
			let basZamanTS = is.basZamanTS ? asDate(is.basZamanTS) : null, isToplamBrutSureSn = basZamanTS ? asInteger((_now.getTime() - basZamanTS.getTime()) / 1000) : null;
