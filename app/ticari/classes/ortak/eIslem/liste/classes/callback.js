class EIslemAkibet_Callback extends CObject {
	...
	ekBilgiGoster(e) {
		...
		let source = _e => this.getSource($.extend({}, e, _e));
		if (part) { part.source = source; part.tazele() }
		else {
			let {parentPart, tabloKolonlari} = this
			let islemAdi = this.islemAdi || 'e-İşlem';
			part = this.part = new EIslemAkibetPart({ parentPart, title: `${islemAdi} Sonucu`, tabloKolonlari, source })
			part.run()
		}
	}
	getSource(e) {
		...
		let recs = []; if (source) {
			...
			for (let rec of source) {
				let _rec = rec.rec || {}, efAyrimTipi = (rec.efAyrimTipi ?? _rec.efayrimtipi) || 'A';
				let code = rec.code ?? rec.ResponseCode, message = rec.message ?? rec.errorText ?? rec.ResponseDescription;
				$.extend(rec, {
	...
	ekIslem(handler) { let {ekIslemler} = this; ekIslemler.push(handler); return this }
