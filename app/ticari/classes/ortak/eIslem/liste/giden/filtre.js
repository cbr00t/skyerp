class GidenEIslemFiltre extends EIslemFiltre {
		...
		const sentEkle = __e => {
			...
			const sent = new MQSent({
				...
				sahalar: [
					...
					`fis.kaysayac`, `fis.almsat`, `fis.iade`, `fis.ayrimtipi`, `fis.tarih`, `fis.fisnox`, `fis.ticmust mustkod`, `car.birunvan`, `fis.efatuuid`, `fis.efgonderimts`, `fis.efatonaydurumu`,
		...
		(() => {
			...
			const alias = 'fis', sent = new MQSent({
				...
				sahalar: [
					...
					`'' mustkod`, `'' birunvan`, `fis.efatuuid`, `fis.efgonderimts`, `fis.efatonaydurumu`, `0 sonucbedel`,
		...
		(() => {
			...
			const sent = new MQSent({
				...
				sahalar: 
					...
					`fis.mustahsilkod mustkod`, `car.birunvan`, `fis.efatuuid`, `fis.efgonderimts`, `fis.efatonaydurumu`, `0 sonucbedel`, `car.vkno vkn`, 'car.efatsenaryotipi'
				]
			});
			uni.add(sent)
		})();
	...
	tbWhereClauseDuzenle(e) {
		...
		const _e = $.extend({}, e, { where: new MQWhereClause() }); this.tbWhereClauseDuzenle_cari(_e);
		const cariWh = _e.where; if (cariWh && !$.isEmptyObject(cariWh.liste)) {
			if (psTip == 'ST') { e.where = null; return }
			wh.birlestir(cariWh)
		}
	}
	tbWhereClauseDuzenle_basit(e) {
		e = e || {}; const {alias, aliasVeNokta} = e;
		const psTip = e.psTip || e.psTipStr || e.psTipKod, wh = e.where;
		if (psTip == 'SM') { this.tbWhereClauseDuzenle_sutMakbuz(e) }
		else { this.tbWhereClauseDuzenle_ortak(e) }
	}
	tbWhereClauseDuzenle_ortak(e) {
		e = e || {}; const {alias, aliasVeNokta} = e;
		const wh = e.where; wh.add(`${aliasVeNokta}silindi = ''`, `${aliasVeNokta}ozelisaret <> '*'`);
	...
	tbWhereClauseDuzenle_eIslem(e) {
		e = e || {}; const {eIslemTipSet} = e; if (eIslemTipSet && (eIslemTipSet.E || eIslemTipSet.A)) { eIslemTipSet[''] = true }
		const {alias, aliasVeNokta} = e, wh = e.where;
		...
		wh.birKismi(this.eIslemBirKismi, `${aliasVeNokta}efayrimtipi`);
	...
	tbWhereClauseDuzenle_pifFis(e) {
		e = e || {}; const pifGenelAyrimlar = [], faturaAyrim = this.faturaAyrimSecim.tekSecim;
		const fatTipHepsimi = faturaAyrim.hepsimi, magazami = faturaAyrim.bumu, faturami = faturaAyrim.digermi;
		if (fatTipHepsimi || faturami) { pifGenelAyrimlar.push('', 'IK', 'KN', 'FS') }
		else if (fatTipHepsimi || magazami) { pifGenelAyrimlar.push('PR') }
