class HMRBilgi extends CIO {
	...
	static get belirtecListe() {
		...
		if (!result) {
			...
			const _hmr = app.params.stokGenel.hmr;
			for (const key in _hmr) {
				if (_hmr[key])
					result.push(key)
			}
		}
	...
	static get hmrEtiketDict() {
		let result = this._hmrEtiketDict;
		if (!result)
			result = this._hmrEtiketDict = app.params.stokGenel.hmrEtiket;
		return result
	}
	...
	static get belirtec2Bilgi() {
		...
		if (!result) {
			...
			for (const belirtec in _result) {
				const item = _result[belirtec];
				item.belirtec = belirtec;
				if (!belirtecSet[belirtec])
					continue;
				const etiket = hmrEtiketDict[belirtec];
				if (etiket)
					item.etiket = etiket;
	...
	static pTanimDuzenle(e) {
		...
		for (const item of this.hmrIter()) {
			const cls = item.numerikmi ? PInstNum : PInstStr;
			pTanim[item.ioAttr] = new cls(item.rowAttr);
			const {adiAttr} = this;
			if (adiAttr)
	...
	static *hmrIter(e) {
		const {belirtec2Bilgi, belirtecSet} = this;
		const DefaultGenislikCh = 20;
		for (const belirtec in belirtec2Bilgi) {
			...
			yield $.extend(item, {
				// ioAttr: item.kami ? `${belirtec}Kod` : belirtec,
				get defaultValue() { return item.numerikmi ? 0 : '' },
				asGridKolon(e) {
					e = e || {};
					let result = this._gridKolon;
					if (result === undefined) {
						const {kami, mfSinif} = item;
						if (kami && !mfSinif)
							result = null;
						else {
							result = this._gridKolon = 
								kami
									? mfSinif.getGridKolonGrup({
										belirtec: item.belirtec, kodAttr: item.ioAttr, adiAttr: item.adiAttr,
										kodEtiket: item.etiket || ioAttr,
										text: item.etiket, adiGenislikCh: DefaultGenislikCh
									})
									: new GridKolon({
										belirtec: item.ioAttr, text: item.etiket,
										genislikCh: DefaultGenislikCh
									});
							if (result)
							...
							if (kami && mfSinif) {
								result.stmDuzenleyiciEkle(e =>
									mfSinif.hmr_queryEkDuzenle(e));
								result.degisince(e =>
									mfSinif.hmrSetValuesEk(e));
								
								let colDef = result;
								if (colDef && colDef.kaKolonu)
									colDef = colDef.kaKolonu;
								mfSinif.hmrTabloKolonDuzenle({ orjColDef: result, colDef: colDef })
							}
						}
					}
					return result
				...
				asGridGosterimKolonlar(e) {
					e = e || {};
					let result = this._gridGosterimKolonlar;
					if (result === undefined) {
						const {kami, mfSinif} = item;
						if (kami && !mfSinif)
							result = null;
						else {
							const {rowAttr, etiket, adiAttr} = item;
							const mfSinif = item.mfSinif || MQHMR;
							const kodKolonu = new GridKolon({
								 belirtec: rowAttr, text: etiket,
								 genislikCh: asInteger(DefaultGenislikCh / 2)
							});
							if (item.numerikmi)
							...
							if (kami && mfSinif) {
								if (adiAttr) {
									const mfAlias = mfSinif.tableAlias;
									const mfAliasVeNokta = mfAlias ? `${mfAlias}.` : '';
									result.push(new GridKolon({
										belirtec: adiAttr, text: `${etiket} Adı`,
										genislikCh: DefaultGenislikCh,
										sql: `${mfAliasVeNokta}${mfSinif.adiSaha}`
									}))
								}
								
								for (const colDef of result)
									mfSinif.hmrTabloKolonDuzenle({ orjColDef: colDef, colDef: colDef })
							}
				...
				asRaporKolonlari(e) {
					e = e || {};
					let result = this._raporKolonlari;
					if (result === undefined) {
						const alias = e.alias || 'har';
						const aliasVeNokta = alias ? `${alias}.` : '';
						const {belirtec, rowAttr, ioAttr, adiAttr, etiket, numerikmi, kami, mfSinif} = item;
	...
	setValues(e) {
		...
		for (const item of this.hmrIter()) {
			...
			if (adiAttr) {
				const value = rec[adiAttr];
				if (value != null)
					this[adiAttr] = value
			}

			if (kami && mfSinif)
				mfSinif.hmrSetValuesEk($.extend({}, e, { inst: this, rec: rec }));
		}
	}
}
