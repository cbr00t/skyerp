class SatisKosulKapsam extends CObject {
	...
	static get tipListe() {
		return [
			'tarih', 'must', 'plasiyer', 'plasiyerBolge', 'plasiyerTip',
			'sube', 'subeGrup', 'cariTip', 'cariKosulGrup'
		]
	}
	...
	*/
	static get uygunmuKontrol() {
		let {_uygunmuKontrol: result} = this
		if (result == null) {
	...
	static get tip2RowAttrListe() {
		let {_tip2RowAttrListe: result} = this
		if (result == null) {
	...
	constructor(e = {}, _alimmi) {
		super(e)
		if (!empty(e))
			this.setValues({ rec: e, alim: e?.alim }, _alimmi)
	}
	setValues(e = {}, _alimmi) {
		...
		if (subeIcinOzeldir != null && !subeIcinOzeldir)
			tipListe = tipListe.filter(tip => !(tip == 'sube' || tip == 'subeGrup'))
		for (let tip of tipListe) {
			let bs = (
				rec[`${tip}Kod`] ?? rec[`${tip}kod`] ?? rec[tip] ??
				mustRec?.[`${tip}Kod`] ?? mustRec?.[`${tip}kod`] ?? mustRec?.[tip]
			)
			if (bs != null) {
				bs = bs.basi === undefined && bs.sonu === undefined
					? new CBasiSonu({ basi: bs, sonu: bs })
					: new CBasiSonu(bs)
			}
			if (bs == null) {
			...
			if (dateTipSet[tip]) {
				...
				for (let [key, value] of entries(bs)) {
					if (value)
	...
	uygunlukClauseDuzenle({ alias, where: wh, alim }) {  /* this: diger */
		...
		for (let tip of tipListe) {
			if (uygunmuKontrol && !uygunmuKontrol.sql?.[tip])
				continue
