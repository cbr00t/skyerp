class DemirbasHareketci extends Hareketci {
	...
	static mstYapiDuzenle({ result }) {
		...
		result.set('shkod', ({ sent, kodClause, mstAlias, mstAdiAlias }) =>
			sent
				.fromIliski(`demmst ${mstAlias}`, `${kodClause} = ${mstAlias}.kod`)
	...
	uniDuzenleOncesi({ sender: { finansalAnalizmi, eldekiVarliklarmi } = {}, stm, uni }) {
		...
		if (eldekiVarliklarmi) {
			...
			;{
				...
				stm.with
					.add(sent.asTmpTable('demfiyat'))
			}
			;{
				let sent = new MQSent(), { where: wh, sahalar } = sent
				sent
					.fromAdd('demfiyat dfiy')
					.innerJoin('dfiy', 'demceyhesap dhes', ['dfiy.demkod = dhes.demkod', `dhes.vtayrim = ''`])
				sahalar.add(
					'dfiy.demkod',
					`MAX(dhes.hesapyili * 10 + dhes.ceyrekno) sonceyrekyil`
				)
				sent.groupByOlustur()
				stm.with
					.add(sent.asTmpTable('demsonhesap'))
			}
			;{
				let sent = new MQSent(), { where: wh, sahalar } = sent
				sent
					.fromAdd('demsonhesap dson')
				...
				sahalar.add(
					'dson.demkod',
					`ROUND(dhes.ydssabitkiymet / dhes.hesmiktar, 2) brmdegerlenmis`
				)
				stm.with
					.add(sent.asTmpTable('demsabitkiymet'))
			}
			...
			;{
				...
				stm.with
					.add(sent.asTmpTable('demsonucfiyat'))
			}
		}
	}
    ...
    static varsayilanHVDuzenle({ hv, sqlNull, sqlEmpty, sqlZero }) {
		...
		extend(hv, {
			kayittipi: `'DM'`, shTipi: `'D'`,
			shkod: 'har.demirbaskod',
    ...
    uygunluk2UnionBilgiListeDuzenleDevam(e) {
        ...
        this.uniDuzenle_gcVeTrf(e).uniDuzenle_irsFat(e)
			.uniDuzenle_genelDekont(e)
    }
			...
			demGiris$demCikis$demTrf: [    // Transferin ÇIKIŞ kısmı alınır
				new Hareketci_UniBilgi()
					.sentDuzenleIslemi(({ sent }) => {
						...
						let gcTipListe = [
							( uygunluk.demGiris ? 'G' : null ),
							( uygunluk.demCikis ? 'C' : null ),
							( uygunluk.demTrf ? 'T' : null )
						].filter(Boolean)
						sent
							.fisHareket('stfis', 'stdemirbas')
							.fromIliski('stkyer yer', 'har.detrefyerkod = yer.kod')
							.fromIliski('carmst car', 'fis.irsmust = car.must')
					...
					.hvDuzenleIslemi(({ hv, sqlEmpty, sqlZero }) => {
						let wt_anaIslAdi = [
							`WHEN fis.gctipi = 'G' THEN (
								CASE fis.ozeltip
									...
									ELSE 'Dem. Giriş'
								END)`,
							`WHEN fis.gctipi = 'C' THEN (
								CASE fis.ozeltip
									WHEN 'SY' THEN 'Sayım Fişi'
									ELSE 'Dem. Çıkış'
								END)`,
							`WHEN fis.gctipi = 'T' THEN (
								CASE fis.ozeltip
						...
						extend(hv, {
							...
							miktar: 'har.miktar', fiyat: 'har.fiyat',
				...
				new Hareketci_UniBilgi()
					...
					.hvDuzenleIslemi(({ hv, sqlEmpty, sqlZero }) => {
						extend(hv, {
							...
							yerkod: 'har.detyerkod', refkod: 'har.detyerkod', refadi: 'yer.aciklama',
							miktar: 'har.miktar', fiyat: 'har.fiyat', fisaciklama: 'fis.cariaciklama'
						})
					})
			]
		})
				...
				new Hareketci_UniBilgi()
					...
					.hvDuzenleIslemi(({ hv, sqlEmpty, sqlZero }) => {
						...
						extend(hv, {
							...
							anaislemadi: `(${iadeVeATClause} + ' ' + ${irsFatClause})`,                                  // Alım İADE Fatura ...
							yerkod: 'har.detyerkod', refkod: 'fis.must', refadi: 'car.birunvan',
							miktar: 'har.miktar', fiyat: 'har.fiyat', fisaciklama: 'fis.cariaciklama'
						})
					})
			]
		...
		extend(liste, {
			genelDekont: [
				new Hareketci_UniBilgi()
					...
					.hvDuzenleIslemi(({ hv, sqlEmpty, sqlZero }) => {
						extend(hv, {
							gc: `'G'`, anaislemadi: `'Genel Dekont'`,
							yerkod: 'har.yerkod', miktar: 'har.miktar',
							fiyat: sqlZero, fisaciklama: 'fis.aciklama'
						})
					})
			]
		})
	...
	static maliTablo_secimlerYapiDuzenle({ result }) {
		...
		extend(result, {
			sube: DMQSube, subeGrup: DMQSubeGrup,
			mst: DMQDemirbas, grup: DMQDemGrup,
			anaGrup: DMQDemAnaGrup, masraf: DMQMasraf
		})
	}
