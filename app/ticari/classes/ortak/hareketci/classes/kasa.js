		...
		extend(liste, {
			devir: [
				new Hareketci_UniBilgi().sentDuzenleIslemi(({ sent }) => {
					...
					wh.fisSilindiEkle()
						.degerAta('KS', 'fis.fistipi')
				}).hvDuzenleIslemi(({ hv }) => {
					extend(hv, {
						kasakod: 'har.kasakod', kayittipi: `'KSDEV'`,
						islemadi: `'Kasa Devir'`
					})
				})
			],
			banka: getUniBilgiler_banka('KB'),
			kasaCari: getUniBilgiler_banka('KC'),
			hizmet: getUniBilgiler_banka('KH'),
