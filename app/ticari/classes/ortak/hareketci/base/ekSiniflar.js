class Hareketci_UniBilgi extends CObject {
	...
	static get ortakArgs() {
		...
		if (result == null)
			result = this._ortakArgs = { sqlNull: 'NULL', sqlEmpty: `''`, sqlZero: '0' }
		return result
	}
	get sent() {
		let sender = this, {_sent: result, temps} = this
		if (result === undefined) {
			let e = { sent: new MQSent(), temps }
			this.sentDuzenle(e)
			result = this._sent = e.sent
		}
		if (isFunction(result))
			result = this._sent = result.call(this, { sender, temps })
		return result
	}
	set sent(value) { this._sent = value }
	get hv() {
		let sender = this, {_hv: result, temps} = this
		if (result === undefined) {
			let e = { hv: {}, temps }
			this.hvDuzenle(e)
			result = this._hv = e.hv
		}
		if (isFunction(result))
			result = this._hv = result.call(this, { sender, temps })
		return result
	}
	...
	sentDuzenle(e) {
		let {sentDuzenleyici: handler, class: { ortakArgs }} = this
		if (handler) {
			let {sent} = e, _e = { ...e, sender: this, sent, ...ortakArgs }
			handler.call(this, _e)
			sent = this.sent = _e.sent
		}
		return this
	}
	hvDuzenle(e) {
		let {hvDuzenleyici: handler, class: { ortakArgs }} = this
		if (handler) {
			let {hv} = e, _e = { ...e, sender: this, hv, ...ortakArgs }
			handler.call(this, _e)
			hv = this.hv = _e.hv
		}
		return this
	}
	setSent(value) { this.sent = value; return this }
