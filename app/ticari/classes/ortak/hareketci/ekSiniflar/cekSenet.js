...
class CSIslemler extends CObject {
	...
	constructor(e) { super(e); this.setBelgeTipi(e) }
	...
	csHareketBilgileriDuzenle({ result }) {
		let {tekilIslemSinif: cls} = this.class, belgeTipi = asSet($.makeArray(this.belgeTipi));
		result.push(...[
				/* Alacak Çek-Senet */
			(belgeTipi.AC || belgeTipi.AS ? [
	...
	getPortfoyVeReferansTanimlari(e) {
		let girismi = typeof e == 'object' ? e.girismi ?? e.giris : e;
		let selectors = Object.keys(this.class.anahtarcilar), {csHareketBilgiler: harBilgiler} = this, result = {};
		{
			let selector2Anahtarci = {}; for (let selector of selectors) { selector2Anahtarci[selector] = girismi ? 'giris' : 'cikis' }
		...
		{
			let selector2Anahtarci = {}; for (let selector of selectors) { selector2Anahtarci[selector] = girismi ? 'cikis' : 'giris' }
			let selector2Anah2Tipler = this.getXTipVePortfoyDict({ selector2Anahtarci, sadeceGercekOlanmi: true });
			let prefix = 'ref'; this.xportfSahalariDuzenle({ result, prefix, selector2Anah2Tipler })
		}

		/* !! finanaliztipi eklenecek */
		return result
		
			// giriş ise asıl portföy, aksinde ref portföy bilgisi aynı olanlar gruplanır
		tipVePortfoyAynilar = getXTipVePortfoyDict(csHareketDizi
									, (girismi ? girisTipAnahtarci : cikisTipAnahtarci)
									, (girismi ? girisPortfoyAnahtarci : cikisPortfoyAnahtarci)
									, true);
		xportfSahalariDuzenle('', tipVePortfoyAynilar);
	
			// giriş ise asıl ref portföy, aksinde asıl portföy bilgisi aynı olanlar gruplanır
		tipVePortfoyAynilar = getXTipVePortfoyDict(csHareketDizi
									, (girismi ? cikisTipAnahtarci : girisTipAnahtarci)
									, (girismi ? cikisPortfoyAnahtarci : girisPortfoyAnahtarci)
									, false);
		xportfSahalariDuzenle('ref', tipVePortfoyAynilar)
	}
	...
	getFisTipiClause({ kosulcu }) {
		let {belgeTipi} = this, {csHareketBilgiler: harBilgiler} = this;
		let sqlDegeri = value => MQSQLOrtak.sqlServerDegeri(value);
		let tiKodClause = 'RTRIM(fis.fistipi + fis.iade)', caseClause = `(case tipKod`;
	...
	/**@ belgeTipi: [AC, AS, BC, BS] */
	setBelgeTipi(e) { this.belgeTipi = e?.belgeTipi ?? e; return this }
	borcCek() { return this.setBelgeTipi('BC') } alacakCek() { return this.setBelgeTipi('AC') }
	borcSenet() { return this.setBelgeTipi('BS') } alacakSenet() { return this.setBelgeTipi('AS') }
}
