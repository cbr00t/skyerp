...
class SBTabloDetay extends MQDetay {
	...
	get secimlerStr() {
		...
		if (secimler) {
			...
			for (let [tip, sec] of secimler) {
				let liste = [], e = { liste }
				sec.ozetBilgiHTMLOlustur?.(e)
				if (!liste.length)
					continue
				let {etiket, grupKod} = sec, html = ''
				if (grupKod)
					etiket = grupListe[grupKod].aciklama || etiket
	...
	secimlerOlustur(e) {
		...
		for (let [tip, yapi] of entries(tip2SecimMFYapi)) {
			if (empty(yapi))
				continue
	...
	async hareketKartiGoster({ rapor, raporTanim }) {
		e = { ...arguments[0] }
		let { session } = config, { dbName: buDBName } = session
		let { sahaAlias: bedelAlias, yatayDegerler, yatayDegerSet, secimler } = rapor
		let { tarihBS: donemBS } = secimler
		let { yatayAnalizVarmi, yatayAnaliz = {} } = raporTanim
		let { aciklama: yatayEtiket, ekBilgi: { zorunluKodAttrListe: yatayAttrListe } = {} } = yatayAnaliz
...
class SBTabloGridci extends GridKontrolcu {
	...
	tanimla(e) {
		...
		let { islem } = e
		if (islem == 'sil')
			return this.tanimKaydet(e)
		
		let gridRec = e.gridRec ?? e.rec
		if (!gridRec && (islem == 'degistir' || islem == 'kopya'))
			return null
		
		let rfb = this.getRootFormBuilder(e)
		rfb.run()
		return rfb
	...
	async tanimKaydet({ islem, sender: gridPart, gridWidget: w, inst: fis, detay: det, eskiDetay: eDet }) {
		gridPart = gridPart?.gridPart ?? gridPart
		w ??= gridPart.gridWidget
		eDet ??= gridPart.selectedRec
		let { detaylar } = fis
		try {
			switch (islem) {
				case 'yeni': case 'degistir': case 'kopya': {
					try {
						...
						if (result && isString(result))
							result = { isError: true, errorText: result }
						if (result?.isError)
							throw result
					}
					catch (ex) { throw ex }
			...
			switch (islem) {
				case 'yeni': case 'kopya': {
					let i = eDet ? detaylar.indexOf(eDet) + 1 : 'last'
					w.addrow(null, det, i)

					if (i == 'last')
						i = gridPart.totalRecs
					w.clearselection()
					w.selectrow(i)
					break
				}
				case 'degistir': {
					w.updaterow(eDet?.uid, det)
					break
				}
				case 'sil': {
					w.deleterow(eDet?.uid)
					break
				}
			}
	...
	getRootFormBuilder(e) {
		let { islem, sender: gridPart, gridRec: eskiDetay } = e
		let { gridWidget } = gridPart
		let { fis } = this, { class: fisSinif } = fis
		e.gridWidget ??= gridWidget
		if (islem == 'yeni')
			eskiDetay = null
		...
		if (islem == 'kopya')
			deleteKeys(detay, 'okunanHarSayac', 'seq')
		let islemAdi = `${islem[0].toUpperCase()}${islem.slice(1)}`
		let title = e.title = islemAdi
		
		let rfb = new RootFormBuilder().asWindow(title)
			.setInst(detay).setParentPart(gridPart)
			.addStyle_fullWH('calc(var(--full) - 10px)')
	...
	yukariAsagi(e) {
		let {_repeatButton_delayMS: delayMS} = SBTablo
		let timerKey = '_timer_yukariAsagi'
		clearTimeout(this[timerKey]);
		this[timerKey] = setTimeout(e => {
			try { this._yukariAsagi(e) }
			finally { delete this[timerKey] }
		}, delayMS - 15, e)
	}
