...
class DAltRapor_Satislar_Main extends DAltRapor_TreeGridGruplu {
	...
	tabloYapiDuzenle(e) {
		...
		$.extend(grup, {
			...
			STGRP: { ka: new CKodVeAdi({ kod: 'STGRP', aciklama: 'Stok Grup' }), colDefs: [new GridKolon({ belirtec: 'grup', text: 'Stok Grup', maxWidth: 300, filterType: 'checkedlist' })] },
			STOK: { ka: new CKodVeAdi({ kod: 'STOK', aciklama: 'Stok' }), colDefs: [new GridKolon({ belirtec: 'stok', text: 'Stok', maxWidth: 400, filterType: 'input' })] },
			...
			CARI: { ka: new CKodVeAdi({ kod: 'CARI', aciklama: 'Cari' }), colDefs: [new GridKolon({ belirtec: 'cari', text: 'Cari', maxWidth: 400, filterType: 'input' })] },
	...
	async loadServerDataInternal(e) {
		const {gruplamalar, maxRow} = e; let query = await app.sqlExecTekilDeger({
