...
class DAltRapor_GridGruplu extends DAltRapor_Grid {
	...
	async loadServerData(e) {
		...
		gridPart._promise_kaFix = (async () => { for (let rec of recs) { for (let prefix of kaPrefixes) { this.fixKA(rec, prefix) } } })();
		let {sortAttr} = this.class; if (sortAttr) { recs.sort((a, b) => { a = a[sortAttr] || 0; b = b[sortAttr] || 0; return a > b ? -1 : a < b ? 1 : 0 }) }
		return recs
	}
	loadServerDataInternal(e) { return null }
	async gridVeriYuklendi(e) {
		super.gridVeriYuklendi(e); let {fbd_grid, gridPart} = this, {grid, gridWidget} = this; let {_lastGruplamalar} = gridPart;
		let {gruplamalar} = gridPart, gruplamaVarmi = !$.isEmptyObject(gruplamalar), colDefs = this.tabloKolonlari; let {gridGrupAttrSet, gruplamaKAListe} = this.class;
		...
		let belirtec2Kolon = {}; for (let colDef of colDefs) { belirtec2Kolon[colDef.belirtec] = colDef }
		let icerikColsSet = {}, tip2Belirtecler = {}; let count = 0; for (let colDef of colDefs) {
			let {belirtec} = colDef, grup = colDef.userData?.grup; if (!grup) { icerikColsSet[belirtec] = colDef }
			if (grup != null && gridGrupAttrSet[grup] && gruplamalar[grup]) { (tip2Belirtecler[grup] = tip2Belirtecler[grup] || []).push(belirtec); count++ }
		}
		for (let belirtecler of Object.values(tip2Belirtecler)) { if (belirtecler?.length > 1) { belirtecler.splice(0, -1) } }
		...
		await gridPart._promise_kaFix; gridWidget.beginupdate(); try {
			if (anahLastGruplamalar == null || anahGruplamalar != anahLastGruplamalar) {
				let tabloKolonlari = colDefs.filter(colDef => { let {belirtec} = colDef, grup = colDef.userData?.grup; return !icerikColsSet[belirtec] && (grup == null || !!gruplamalar[grup]) });
				let attrSet = asSet(tabloKolonlari.map(colDef => colDef.belirtec));
				let {gruplama2IcerikCols} = this.class; for (let [gruplama, belirtecler] of Object.entries(gruplama2IcerikCols)) {
					let gruplamaVarmi = gruplamalar[gruplama]; if (!gruplamaVarmi) { continue }
					for (let belirtec of belirtecler) {
						if (attrSet[belirtec]) { continue } let colDef = belirtec2Kolon[belirtec]; if (colDef == null) { continue }
		...
		let groups = []; if (Object.keys(gruplamalar).length >= 2) {
			for (let belirtecler of Object.values(tip2Belirtecler)) { if (belirtecler?.length) { let belirtec = belirtecler[1] || belirtecler[0]; groups.push(belirtec) } } }
		if (groups.length) { grid.jqxGrid('groups', groups) } /*if (groups.length) { for (let belirtec of groups) { gridWidget.hidecolumn(belirtec) } }*/
	}
	gruplamalarIstendi(e) {
		...
		for (let {kod, aciklama} of gruplamaKAListe) {
			let btn = fbd_content.addForm(kod).setLayout(e => {
				let {builder} = e, {id} = builder;
	...
	gruplamalar_butonTiklandi(e) {
		let {id, evt, gruplamalar} = e, target = $(evt.currentTarget), flag = target.jqxToggleButton('toggled');
