class DAltRapor_TreeGrid extends DAltRapor {
	...
	tazeleSonrasi(e) {
		...
		this.rapor?.signal('tazeleSonrasi', e)
	}
	hizliBulIslemi(e) {
		let {gridPart} = this;
...
class DAltRapor_TreeGridGruplu extends DAltRapor_TreeGrid {
	...
	ozetBilgiRecsOlustur(e) {
		let { raporTanim, ozetBilgi, gridPart: { gridWidget } } = this
		let { grupAttr, icerikAttr, icerikColDef } = ozetBilgi
		if (!grupAttr)
			return
		
		let { secilenVarmi, ozetMax } = raporTanim
		if (!(secilenVarmi && ozetMax)) {
			ozetBilgi.recs = []
			return
		}
		
		let sevRecs = e.recs ?? gridWidget.getRows()
		let deger2Bilgiler = {}
		for (let sev of sevRecs) {
			let value = sev[icerikAttr]
			if (value) 
				(deger2Bilgiler[value] ??= []).push(sev)
		}

		let { sortdirection: sortDir } = gridWidget.base
		let tersSiraliDegerler = keys(deger2Bilgiler)
			.map(x => asFloat(x))
			.sort((_a, _b) => {
				let tersmi = !sortDir?.ascending
				let a = tersmi ? _b : _a
				let b = tersmi ? _a : _b
				return (
					a == b ? 0 :
		...
		let digerRec = {
			[grupAttr]: `<b class="royalblue">Diğer</b>`,
			[icerikAttr]: 0
		}
		...
		if (digerRec[icerikAttr]) {
			let { aggregates: agg } = icerikColDef ?? {}
			if (agg?.includes('avg')) {
				digerRec[icerikAttr] = digerSayi
					? roundToFra((digerRec[icerikAttr] || 0) / digerSayi, icerikColDef.tip?.fra ?? 0)
					: 0
			}
		...
		try {
			if (!isPanelItem) {
				this._timer_progress = setTimeout(async () => {
					...
					globalThis.progressManager?.setProgressMax(15 * tazeleHideProgress_minCount)
			...
			let {tabloKolonlari, tabloYapi, ozetBilgi} = this
			let {secilenVarmi, attrSet, grup, icerik} = raporTanim
			let tip2ColDefs = {}, belirtec2Tip = {}
			for (let colDef of tabloKolonlari) {
				let {belirtec, userData} = colDef, {kod: tip} = userData || {}
				if (tip) {
					(tip2ColDefs[tip] ??= []).push(colDef)
					belirtec2Tip[belirtec] = tip
				}
			}
			let _colDefs = keys(icerik ?? {}).flatMap(kod => tip2ColDefs[kod])
			let colDefs = [
			...
			for (let tip in attrSet) {
				if (ozetBilgi.grupTipKod && ozetBilgi.icerikTipKod)
					break
				if (!ozetBilgi.grupTipKod && !tabloYapi.toplam[tip])
					ozetBilgi.grupTipKod = tip
				else if (!ozetBilgi.icerikTipKod && tabloYapi.toplam[tip])
					ozetBilgi.icerikTipKod = tip
			}
			...
			hideProgress(); hConfirm(getErrorText(ex), this.class.aciklama)
			throw ex
		}
		finally {
			let {rapor, tazeleHideProgress_waitMS} = this
			let totalCount = ++rapor.tazeleCount
			clearTimeout(this._timer_progress); delete this._timer_progress
			if (!tazeleHideProgress_minCount || totalCount >= tazeleHideProgress_minCount) {
				setTimeout(() =>
					globalThis.progressManager?.progressEnd(), 0)
				setTimeout((
					{ sender }) => hideProgress(),
					tazeleHideProgress_waitMS, { sender: this }
				)
			}
		}
	}
