...
class TSSHDDetay extends TSDetay {
	...
	get eSHText() {
		...
		if (result === undefined) {
			let eBilgi = this.eBilgi || {}, {efstokkod, efstokadi} = eBilgi;
	...
	constructor(e) {
		e = e || {}; super(e); let {shSahaPrefix} = this.class;
	...
	static pTanimDuzenle(e) {
		super.pTanimDuzenle(e); let {pTanim} = e; $.extend(pTanim, {
			...
			dagitDipIskBedel: new PInstNum(), dagitDipNakBedel: new PInstNum()
		});
	...
	static orjBaslikListesiDuzenleDevam(e) {
		let {shEtiket, shKodSaha, shAlias} = this, {liste} = e;
		liste.push(
	...
	static orjBaslikListesiDuzenleSon({ liste }) {
		super.orjBaslikListesiDuzenleSon(...arguments)
		liste.push(new GridKolon({ belirtec: 'ekaciklama', text: 'Açıklama', minWidth: 100, maxWidth: 300 }).noSql())
	}
	static raporQueryDuzenle(e) {
		super.raporQueryDuzenle(e); let {sent, fisSinif, attrSet} = e;
		if (fisSinif.ticarimi) {
			if (attrSet.detayKdvOrani)
				sent.har2KDVBagla()
			if (attrSet.detayTevkifatOranx)
	...
	static loadServerData_queryDuzenle(e) {
		...
		for (let item of HMRBilgi) {
			...
			let { table, tableAlias, kodSaha, adiSaha } = mfSinif
			if (stokGibimi) {
				sent.fromIliski(
					`${table} ${tableAlias}`,
	...
	static tekilOku_detaylar_queryDuzenle_ticari(e) {
		...
		if (yildizlimi) {
			let orjKdvKodClause = yildizlimi ? this.getOrjKdvKodClause(e) : `${aliasVeNokta}kdvhesapkod`
			sahalar.add(`${orjKdvKodClause} orjkdvkod`)
		}
		else
			sahalar.add(`${aliasVeNokta}kdvhesapkod orjkdvkod`)
	}
	async disKaydetOncesiIslemler(e = {}) {
		await super.disKaydetOncesiIslemler(e)
		let { fis } = e
		let { miktar, fiyat, brutBedel, netBedel, kdvKod, kdvOrani } = this
		let { ticarimi } = fis?.class ?? {}
		let _e = { ...e, ticarimi }
	...
	hostVars(e) {
		;['fiyat', 'brutBedel', 'netBedel'].forEach(k =>
			this[k] = (this[k] || 0))
	...
	hostVarsDuzenle(e = {}) {
		...
		let { fis, hv } = e
		let { shKodSaha, shAdiSaha, hizmetmi } = this.class
		let { stokmu, siparismi } = fis?.class ?? {}
		hv[shKodSaha] = this.shKod
		
		if (siparismi || hizmetmi)
			delete hv.detyerkod
		else
			hv.detyerkod = this.getYerKod({ fis })
		
		let { fiyat, brutBedel, netBedel, hmr } = this
		extend(hv, {
			ekranverifiyat: fiyat,
			belgefiyat: fiyat,
			belgebrutbedel: brutBedel,
			belgebedel: netBedel
		})
		if (stokmu)
			deleteKeys(hv, 'brutbedel', 'kkegyuzde')
		
		if (hmr) {
			hmr.hostVarsDuzenle(e)
			for (let {rowAttr, ioAttr} of hmr)
				hv[rowAttr] = this[ioAttr]
		}
	}
	setValues(e = {}) {
		super.setValues(e)
		let { rec, fis } = e
		let { hmr, class: { shKodSaha, shAdiSaha, hizmetmi } } = this
		let { stokmu, siparismi } = fis?.class ?? {}
		let { paketicadet: paketIcAdet } = rec
		
		extend(this, {
			shKod: rec.shKod || rec.shkod || rec[shKodSaha],
			shAdi: rec.shAdi || rec.shadi || rec[shAdiSaha],
			brm: rec.brm, paketIcAdet,
			kdvDegiskenmi: asBool(rec.kdvDegiskenmi),
			adiDegisirmi: asBool(rec.adiDegisirmi),
			takipNo: rec.dettakipno || '',
			takipAdi: rec.takipadi
		})
		
		if (stokmu)
			this.brutBedel = rec.belgebrutbedel
		
		if (!(siparismi || hizmetmi))
			extend(this, { yerKod: rec.detyerkod ?? '', yerAdi: rec.yeradi })
		
		if (hmr) {
			hmr.setValues(e)
			for (let {ra, ia} of hmr)
				this[ia] = rec[ra]
		}
	}
	ticariHostVarsDuzenle(e) {
		let { fis, hv } = e
		let { iskYapi, kdvKod, kdv, stopaj } = this
		let { dagitDipIskBedel, dagitDipNakBedel } = this
		if (!kdvKod)
			kdv = this.kdv = this.kdvOrani = 0
		
		extend(hv, {
			kdvhesapkod: kdvKod || '',
			perkdv: kdv || 0,
			perstopaj: stopaj || 0,
			dipiskonto: dagitDipIskBedel || 0, dipnakliye: dagitDipNakBedel || 0
		})
		
		this.ekVergiYapi.ticariHostVarsDuzenle(e)
		let iskHVEkle = ({ belirtec, rowAttrPrefix, oranMax }) => {
			let oranlar = iskYapi[belirtec]
			for (let i = 0; i < oranMax; i++) {
				let oran = oranlar[i] || 0
				let rowAttr = `${rowAttrPrefix}oran${i + 1}`
				hv[rowAttr] = oran
			}
		}
	...
	ticariSetValues(e) {
		...
		extend(this, {
			...
			dagitDipIskBedel: rec.dipiskonto || 0, dagitDipNakBedel: rec.dipnakliye || 0
		});
		this.ekVergiYapi.ticariSetValues(e);
		/* let {sabitIskOranMax, kampanyaIskOranMax} = app.params.fiyatVeIsk;*/
		let {iskYapi} = this, iskSetValues = e => {
			let {belirtec, rowAttrPrefix, oranMax} = e, oranlar = iskYapi[belirtec] = [];
			for (let i = 0; i < oranMax; i++) {
				let rowAttr = `${rowAttrPrefix}oran${i + 1}`, oran = rec[rowAttr] || 0;
				oranlar.push(oran)
			}
		};
		for (let item of TicIskYapi.getIskYapiIter()) { iskSetValues({ belirtec: item.key, rowAttrPrefix: item.belirtec, oranMax: item.maxSayi }) }
	...
	iskYapiPropertyleriOlustur(e) {
		let {iskYapi} = this; if (!iskYapi) { return }
		for (let item of iskYapi.class.getIskYapiIter()) {
			let {key, selector} = item;
			Object.defineProperty(this, selector, {
				get: () => { let {iskYapi} = this; return iskYapi[key] || [] },
				set: value => { let {iskYapi} = this; iskYapi[key] = value || []; }
			})
		}
	}
}
class TSStokHizmetDetay extends TSSHDDetay {
	...
	static getAdiDegisirmiClause(e) { let {shAlias} = this; return `${shAlias}.adidegisir` }
	static pTanimDuzenle(e) {
		super.pTanimDuzenle(e); let {pTanim} = e;
		$.extend(pTanim, { stopajKod: new PInstStr() })
	}
	static orjBaslikListesiDuzenleDevam2(e) {
		super.orjBaslikListesiDuzenleDevam2(e); let {liste} = e;
		liste.push(
	...
	static tekilOku_detaylar_queryDuzenle_ticari(e) {
		super.tekilOku_detaylar_queryDuzenle_ticari(e); let {aliasVeNokta} = this, {sent} = e, {fis} = e, {yildizlimi} = fis;
		sent.fromIliski(`vergihesap sver`, `${aliasVeNokta}stopajhesapkod = sver.kod`);
		sent.sahalar.add(`sver.belirtec stopajbelirtec`);
		if (yildizlimi) {
			let orjStopajKodClause = yildizlimi ? this.getOrjStopajKodClause(e) : `${aliasVeNokta}stopajhesapkod`;
...
class TSStokDetayOrtak extends TSStokHizmetDetay {
	...
	static raporQueryDuzenle(e) {
		super.raporQueryDuzenle(e); let {sent, fisSinif, attrListesi} = e;
		...
		for (let attr of attrListesi) {
			let Prefix = 'stokOem';
			if (attr && attr.startsWith(Prefix)) {
				let oemSiraStr = attr.substring(Prefix.length), oemAlias = `soem${oemSiraStr}`;
				sent.leftJoin({ alias: 'har', table: `stokoem ${oemAlias}`, on: [`har.stokkod = ${oemAlias}.stokkod`, `${oemAlias}.oemsira = ${oemSiraStr}`] })
			}
		}
	}
	static orjBaslikListesiDuzenleHMR(e) {
		super.orjBaslikListesiDuzenleHMR(e); let {tableAlias: alias} = this, {liste} = e;
		for (let item of HMRBilgi.hmrIter()) {
			let colDefs = item.asGridGosterimKolonlar({ alias }); if ($.isEmptyObject(colDefs)) { continue }
	...
	static tekilOku_detaylar_queryDuzenle_ticari(e) {
		...
		if (yildizlimi) {
			let orjOtvKodClause = yildizlimi ? this.getOrjOtvKodClause(e) : `${aliasVeNokta}otvhesapkod`;
	...
	miktar2Hesapla(e) {
		super.miktar2Hesapla(e); let {brm2, brmOrani} = this;
		if (brm2 && brmOrani) { this.miktar2 = roundToStokFra(this.miktar * brmOrani, brm2) }
		return this
	}
	hmrPropertyleriOlustur(e) {
		e = e ?? {}; let {hmr} = this; if (!hmr) { return }
		for (let {ioAttr, adiAttr} of hmr.hmrIter()) {
...
class TSHizmetDetay extends TSStokHizmetDetay {
	...
	static getDetayTable(e) { let fisSinif = e.fisSinif ?? e.fis?.class; return fisSinif.tsHizmetDetayTable }
	static pTanimDuzenle(e) { super.pTanimDuzenle(e); let {pTanim} = e; $.extend(pTanim, { konaklamaKod: new PInstStr() }) }
	static raporQueryDuzenle(e) {
		super.raporQueryDuzenle(e); let {sent} = e;
	...
	static loadServerData_queryDuzenle(e) { super.loadServerData_queryDuzenle(e); let {sent} = e; sent.sahalar.add('NULL miktar2', 'NULL brm2', 'NULL brmorani') }
	static tekilOku_detaylar_queryDuzenle_ticari(e) {
		super.tekilOku_detaylar_queryDuzenle_ticari(e); let {aliasVeNokta} = this, {sent} = e;
	...
	ticariHostVarsDuzenle(e) { super.ticariHostVarsDuzenle(e); let {hv} = e; $.extend(hv, { konaklamahesapkod: this.konaklamaKod || '' }) }
	ticariSetValues(e) {
		super.ticariSetValues(e); let {rec} = e;
		$.extend(this, {
...
class TSDemirbasDetay extends TSSHDDetay {
	...
	static getDetayTable(e) { let fisSinif = e.fisSinif ?? e.fis?.class; return fisSinif.tsDemirbasDetayTable }
	static raporQueryDuzenle(e) {
		super.raporQueryDuzenle(e); let {sent, fisSinif} = e;
...
class TSAciklamaDetay extends TSDetay {
	...
	static getDetayTable(e) { let fisSinif = e.fisSinif ?? e.fis?.class; return fisSinif.tsAciklamaDetayTable }
